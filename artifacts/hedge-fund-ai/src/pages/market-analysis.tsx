...
/* ── 매수/보유/매도 신호 ─────────────────────────────────────────────────── */
type SignalType = "강한 매수" | "매수" | "보유" | "매도" | "강한 매도";

function computeSignal(r: IndexResult): {
  signal: SignalType;
  reasons: string[];
  confidencePct: number;
} {
  const ret   = r.predictedReturn3d;
  const acc   = r.rolling30dDirAcc;
  const agree = r.agreementSignal;

  const upAgree   = agree === "up";
  const downAgree = agree === "down";
  const highAcc   = acc >= 60;
  const midAcc    = acc >= 52;

  let signal: SignalType = "보유";
  const reasons: string[] = [];

  if (upAgree && ret > 0.8 && highAcc) {
    signal = "강한 매수";
    reasons.push(`3일 내 +${ret}% 상승 예측 (강세)`);
    reasons.push("GBDT·LSTM 두 모델 상승 합의");
    reasons.push(`최근 6주 적중률 ${acc}% — 고신뢰 구간`);
  } else if (upAgree && ret > 0.2 && midAcc) {
    signal = "매수";
    reasons.push(`3일 내 +${ret}% 상승 예측`);
    reasons.push("두 AI 모델 상승 방향 합의");
    reasons.push(`적중률 ${acc}% 기반 판단`);
  } else if (downAgree && ret < -0.8 && highAcc) {
    signal = "강한 매도";
    reasons.push(`3일 내 ${ret}% 하락 예측 (약세)`);
    reasons.push("GBDT·LSTM 두 모델 하락 합의");
    reasons.push(`최근 6주 적중률 ${acc}% — 고신뢰 구간`);
  } else if (downAgree && ret < -0.2 && midAcc) {
    signal = "매도";
    reasons.push(`3일 내 ${ret}% 하락 예측`);
    reasons.push("두 AI 모델 하락 방향 합의");
    reasons.push(`적중률 ${acc}% 기반 판단`);
  } else {
    signal = "보유";
    if (agree === "neutral") reasons.push("두 AI 모델 방향 불일치 — 관망 권고");
    else if (Math.abs(ret) <= 0.2) reasons.push(`예측 변화폭 ${ret}% — 유의미한 방향성 부재`);
    else reasons.push(`적중률 ${acc}% — 신호 강도 부족`);
    reasons.push("뚜렷한 추세 형성 전까지 현 포지션 유지");
  }

  // 신뢰도 점수: 적중률(0~100) × 동의 강도 가중치
  const agreeBonus = upAgree || downAgree ? 1.15 : 0.75;
  const confidencePct = Math.min(99, Math.round(acc * agreeBonus));

  return { signal, reasons, confidencePct };
}

function ActionSignalCard({ result }: { result: IndexResult }) {
  const { signal, reasons, confidencePct } = computeSignal(result);

  const cfg = {
    "강한 매수": {
      bg: "bg-red-500/12 border-red-500/40",
      badge: "bg-red-500 text-white",
      text: "text-red-400",
      bar: "bg-red-500",
      icon: "🔴",
      desc: "강하게 매수를 고려할 시점이에요",
    },
    "매수": {
      bg: "bg-red-500/7 border-red-400/30",
      badge: "bg-red-400/20 text-red-400 border border-red-400/40",
      text: "text-red-400",
      bar: "bg-red-400",
      icon: "📈",
      desc: "매수를 고려해볼 수 있어요",
    ...
    "보유": {
      ...
      icon: "⏸️",
      desc: "지금은 보유하며 관망하는 게 좋아요",
    },
    "매도": {
      bg: "bg-blue-500/7 border-blue-400/30",
      badge: "bg-blue-400/20 text-blue-400 border border-blue-400/40",
      text: "text-blue-400",
      bar: "bg-blue-400",
      icon: "📉",
      desc: "매도를 고려해볼 수 있어요",
    ...
    "강한 매도": {
      bg: "bg-blue-500/12 border-blue-500/40",
      badge: "bg-blue-500 text-white",
      text: "text-blue-400",
      bar: "bg-blue-500",
      icon: "🔵",
      desc: "강하게 매도를 고려할 시점이에요",
    },
  }[signal];

  const allSignals: SignalType[] = ["강한 매도", "매도", "보유", "매수", "강한 매수"];
  const activeIdx = allSignals.indexOf(signal);
    ...
    <div className={cn("rounded-2xl border p-4 space-y-3", cfg.bg)}>
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-[11px] text-muted-foreground/70 font-medium">AI 투자 판단</p>
            <p className={cn("text-sm font-bold", cfg.text)}>{cfg.desc}</p>
          </div>
        </div>
        <span className={cn("text-sm font-bold px-3 py-1.5 rounded-full", cfg.badge)}>
          {signal}
        </span>
      </div>
          ...
          <div key={s} className="flex-1 flex flex-col items-center gap-1">
            <div className={cn(
              ...
              i === activeIdx
                ? cfg.bar
                : i < activeIdx
                  ? (activeIdx >= 3 ? "bg-red-500/25" : "bg-blue-500/25")
                  : "bg-muted/60",
            )} />
            <span className={cn(
              "text-[9px] font-medium hidden sm:block",
              i === activeIdx ? cfg.text : "text-muted-foreground/40",
            )}>
              {s.replace("강한 ", "강")}
            </span>
          </div>
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between text-[10px] text-muted-foreground/60">
          <span>AI 신뢰도</span>
          <span className={cn("font-bold", cfg.text)}>{confidencePct}%</span>
        </div>
        <div className="h-1.5 bg-muted/60 rounded-full overflow-hidden">
          <div className={cn("h-full rounded-full transition-all", cfg.bar)} style={{ width: `${confidencePct}%` }} />
        </div>
      </div>

      {/* 면책 고지 */}
      <p className="text-[10px] text-muted-foreground/40 leading-relaxed">
        ⚠️ 이 판단은 AI 모델 예측 기반 참고 정보입니다. 실제 투자 결정은 본인의 판단과 책임하에 이루어져야 합니다.
      </p>
    </div>
  );
}
      ...
      <AnimatePresence>
        {(status?.kospi || status?.ready) && status?.kospi && status?.kosdaq && (
          ...
          >
            ...
            {/* ── 매수/보유/매도 판단 카드 ─────────────────────────────── */}
            {current && <ActionSignalCard result={current} />}
