...
export default function SignUpScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error) { setErrorMsg(error.message ?? "회원가입에 실패했습니다"); return; }
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (signUp.status === "complete") {
        await signUp.finalize({ navigate: ({ decorateUrl }) => router.replace(decorateUrl("/") as any) });
      }
    } catch (e: any) {
  ...
  const handleGoogle = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg(e?.message ?? "Google 가입에 실패했습니다");
    } finally {
        ...
        <ScrollView contentContainerStyle={[s.container, { paddingTop: insets.top + 20, paddingBottom: insets.bottom + 40 }]}>
          <View style={s.topRow}>
            <TouchableOpacity onPress={() => router.back()} style={s.backBtn}>
              <Feather name="arrow-left" size={20} color="#0d1421" />
            </TouchableOpacity>
          </View>
          ...
          <Text style={s.subtitle}>{email}로{"\n"}전송된 6자리 코드를 입력하세요</Text>
          ...
          <TouchableOpacity style={s.linkBtn} onPress={() => signUp.verifications.sendEmailCode()}>
    ...
    <KeyboardAvoidingView style={s.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView contentContainerStyle={[s.container, { paddingTop: insets.top + 20, paddingBottom: insets.bottom + 40 }]} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.brand}>
          ...
          <Text style={s.appName}>애빛다 시작하기</Text>
          <Text style={s.appTagline}>AI 투자 분석을 무료로 체험하세요</Text>
        </View>
        ...
        >
          {isFetching ? <ActivityIndicator color="#fff" /> : <Text style={s.primaryBtnText}>계속하기</Text>}
        </TouchableOpacity>

        <View nativeID="clerk-captcha" />

        <View style={s.signupRow}>
          <Text style={s.signupText}>이미 계정이 있으신가요? </Text>
          <TouchableOpacity onPress={() => router.replace("/(auth)/sign-in" as any)}>
            <Text style={s.signupLink}>로그인</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
...
const s = StyleSheet.create({
  ...
  logoText: { fontSize: 28, fontFamily: "Pretendard-Bold", color: "#fff" },
  appName: { fontSize: 22, fontFamily: "Pretendard-Bold", color: "#0d1421" },
  appTagline: { fontSize: 13, color: "#5d6678", fontFamily: "Pretendard-Regular" },
  verifyIcon: {
    width: 72, height: 72, borderRadius: 20, backgroundColor: "#fff9f5",
    alignItems: "center", justifyContent: "center", alignSelf: "center",
    marginBottom: 16, marginTop: 20, borderWidth: 1, borderColor: "#fed7aa",
  },
  title: { fontSize: 22, fontFamily: "Pretendard-Bold", color: "#0d1421", marginBottom: 8 },
  subtitle: { fontSize: 14, color: "#5d6678", fontFamily: "Pretendard-Regular", marginBottom: 24, lineHeight: 22 },
  googleBtn: {
    flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10,
    backgroundColor: "#fff", borderRadius: 12, paddingVertical: 14,
    borderWidth: 1, borderColor: "#dde0ea", marginBottom: 16,
  },
  googleIcon: { fontSize: 18, fontFamily: "Pretendard-Bold", color: "#4285f4" },
  googleText: { fontSize: 15, fontFamily: "Pretendard-SemiBold", color: "#0d1421" },
  dividerRow: { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 16 },
  dividerLine: { flex: 1, height: 1, backgroundColor: "#dde0ea" },
  dividerText: { fontSize: 12, color: "#9aa3b2", fontFamily: "Pretendard-Regular" },
  label: { fontSize: 13, fontFamily: "Pretendard-SemiBold", color: "#5d6678", marginBottom: 6 },
  input: {
    backgroundColor: "#fff", borderRadius: 12, paddingHorizontal: 14, paddingVertical: 13,
    fontSize: 15, fontFamily: "Pretendard-Regular", color: "#0d1421",
  ...
  errorText: { fontSize: 13, color: "#ef4444", fontFamily: "Pretendard-Regular", marginBottom: 10 },
  primaryBtn: {
  ...
  primaryBtnText: { fontSize: 16, fontFamily: "Pretendard-Bold", color: "#fff" },
  disabled: { opacity: 0.5 },
  linkBtn: { alignItems: "center", paddingVertical: 12 },
  linkText: { fontSize: 14, color: "#6366f1", fontFamily: "Pretendard-SemiBold" },
  signupRow: { flexDirection: "row", justifyContent: "center", alignItems: "center", marginTop: 8 },
  signupText: { fontSize: 14, color: "#5d6678", fontFamily: "Pretendard-Regular" },
  signupLink: { fontSize: 14, color: "#6366f1", fontFamily: "Pretendard-SemiBold" },
});
