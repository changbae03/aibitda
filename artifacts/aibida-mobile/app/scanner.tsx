...
function UpsideBadge({ pct, colors }: { pct: number; colors: any }) {
  ...
  return (
    <View style={{ backgroundColor: bg + "22", borderRadius: 12, paddingHorizontal: 8, paddingVertical: 3, borderWidth: 1, borderColor: bg + "44" }}>
      <Text style={{ fontSize: 12, fontFamily: "Inter_700Bold", color: bg }}>+{pct.toFixed(1)}%</Text>
    </View>
  );
}
...
function TodayChange({ pct, colors }: { pct: number | null; colors: any }) {
  ...
  return (
    <View style={{ flexDirection: "row", alignItems: "center", gap: 2 }}>
      ...
      <Text style={{ fontSize: 11, fontFamily: "Inter_600SemiBold", color: isUp ? "#ef4444" : "#3b82f6" }}>
        {isUp ? "+" : ""}{pct.toFixed(2)}%
      </Text>
    </View>
  );
...
function VerdictBadge({ verdict, colors }: { verdict: string; colors: any }) {
  ...
  return (
    <View style={{ backgroundColor: bg + "22", borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2, borderWidth: 1, borderColor: bg + "44" }}>
      <Text style={{ fontSize: 9, fontFamily: "Inter_600SemiBold", color: bg }}>{label}</Text>
    </View>
  );
}
    ...
    >
      ...
      <View style={{ flex: 1, gap: 3 }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
          <Text style={{ fontSize: 14, fontFamily: "Inter_700Bold", color: colors.foreground }}>{item.companyName}</Text>
          <View style={{
            backgroundColor: isKR ? "#3b82f620" : "#8b5cf620",
            borderRadius: 4, paddingHorizontal: 5, paddingVertical: 1,
          }}>
            <Text style={{ fontSize: 9, fontFamily: "Inter_600SemiBold", color: isKR ? "#3b82f6" : "#8b5cf6" }}>
              {isKR ? "KR" : "US"}
            </Text>
          </View>
        </View>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
          <Text style={{ fontSize: 11, fontFamily: "Inter_400Regular", color: colors.mutedForeground, fontVariant: ["tabular-nums"] }}>
            {item.ticker}
          </Text>
          {item.industry ? (
            <>
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPad + 10, borderBottomColor: colors.border }]}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
            ...
            <Text style={{ fontSize: 17, fontFamily: "Inter_700Bold", color: colors.foreground }}>저평가 스캐너</Text>
          </View>
      ...
      <View style={[styles.filterWrap, { borderBottomColor: colors.border, backgroundColor: colors.background }]}>
        ...
        <View style={{ flexDirection: "row", gap: 6, padding: 8, backgroundColor: colors.muted + "60", borderRadius: 10, alignSelf: "flex-start" }}>
          {MARKET_TABS.map(t => (
            ...
            >
              <Text style={{
                fontSize: 12, fontFamily: market === t.key ? "Inter_600SemiBold" : "Inter_400Regular",
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginTop: 8 }}>
          <Text style={{ fontSize: 11, color: colors.mutedForeground, fontFamily: "Inter_400Regular" }}>최소 상승여력</Text>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginTop: 6 }}>
          <Text style={{ fontSize: 11, color: colors.mutedForeground, fontFamily: "Inter_400Regular" }}>정렬</Text>
          <View style={{ flexDirection: "row", gap: 4 }}>
      ...
      {loading ? (
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center", gap: 12 }}>
          ...
          <Text style={{ fontSize: 13, color: colors.mutedForeground, fontFamily: "Inter_400Regular" }}>현재가 조회 중…</Text>
          <Text style={{ fontSize: 11, color: colors.mutedForeground + "80", fontFamily: "Inter_400Regular" }}>종목별 시세를 실시간으로 가져오고 있습니다</Text>
        </View>
      ) : error ? (
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center", gap: 12 }}>
          ...
          <TouchableOpacity onPress={() => load()} style={{ paddingHorizontal: 16, paddingVertical: 8, backgroundColor: colors.primary + "22", borderRadius: 8 }}>
            <Text style={{ color: colors.primary, fontFamily: "Inter_500Medium" }}>다시 시도</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <FlatList
          ...
          ListFooterComponent={
            sorted.length > 0 ? (
              <Text style={{ fontSize: 10, color: colors.mutedForeground + "60", textAlign: "center", paddingVertical: 16, fontFamily: "Inter_400Regular" }}>
