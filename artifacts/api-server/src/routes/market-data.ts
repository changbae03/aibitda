...
// ─── GET /api/market-data/indices — 주요 지수 실시간 조회 ─────────────────────
const _indicesCache: { data: any; expiresAt: number } | null = null as any;
let _indicesCacheObj: { data: any; expiresAt: number } | null = null;

const INDEX_TICKERS = [
  { key: "KOSPI",   symbol: "^KS11",    label: "KOSPI",   currency: "KRW" },
  { key: "KOSDAQ",  symbol: "^KQ11",    label: "KOSDAQ",  currency: "KRW" },
  { key: "SP500",   symbol: "^GSPC",    label: "S&P 500", currency: "USD" },
  { key: "NASDAQ",  symbol: "^IXIC",    label: "NASDAQ",  currency: "USD" },
  { key: "USDKRW",  symbol: "USDKRW=X", label: "USD/KRW", currency: "KRW" },
];

router.get("/indices", async (_req, res) => {
  if (_indicesCacheObj && Date.now() < _indicesCacheObj.expiresAt) {
    return res.json(_indicesCacheObj.data);
  }
  try {
    const results = await Promise.allSettled(
      INDEX_TICKERS.map(async ({ key, symbol, label, currency }) => {
        try {
          ...
          return { key, label, currency, price: null, change: null, changeAbs: null, prevClose: null };
        }
      })
    );
    const data = results.map((r) => (r.status === "fulfilled" ? r.value : null)).filter(Boolean);
    _indicesCacheObj = { data, expiresAt: Date.now() + 5 * 60 * 1000 }; // 5분 캐시
    return res.json(data);
  } catch (err: any) {
    return res.status(500).json({ error: err?.message ?? "indices error" });
  }
});

// ─── GET /api/market-data/ai-briefing — AI 일일 마켓 브리핑 ──────────────────
let _aiBriefingCache: { data: any; expiresAt: number } | null = null;

router.get("/ai-briefing", async (_req, res) => {
  if (_aiBriefingCache && Date.now() < _aiBriefingCache.expiresAt) {
    return res.json(_aiBriefingCache.data);
  }
  try {
    const kstNow = new Date(Date.now() + 9 * 3600 * 1000);
    const dateStr = kstNow.toISOString().split("T")[0];

    const prompt = `오늘은 ${dateStr}(KST)입니다. 글로벌 금융 시장 전문가 관점에서 오늘의 주요 마켓 동향을 아래 JSON 형식으로 반환하세요.

{
  "headline": "오늘 시장을 한 줄로 요약 (30자 이내)",
  "summary": "오늘 글로벌 시장 전반 동향 설명 (2-3문장, 한국어)",
  "keyThemes": [
    { "icon": "📈", "title": "테마명", "desc": "15자 이내 설명" }
  ],
  "signals": [
    { "type": "bullish|bearish|neutral", "text": "투자 시그널 (15자 이내)" }
  ],
  "watchList": ["주목할 섹터 또는 키워드 1", "2", "3"]
}

keyThemes는 3개, signals는 3개, watchList는 3개를 반드시 포함하세요.
JSON만 반환하세요.`;
    ...
    if (jsonMatch) raw = jsonMatch[1] ?? raw;
    const briefing = JSON.parse(raw.trim());

    _aiBriefingCache = { data: briefing, expiresAt: Date.now() + 3 * 60 * 60 * 1000 }; // 3시간 캐시
    return res.json(briefing);
  } catch (err: any) {
    console.error("[ai-briefing] error:", err?.message);
    return res.status(500).json({ error: err?.message ?? "AI 브리핑 생성 실패" });
  }
});
