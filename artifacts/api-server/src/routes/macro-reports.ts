...
Bloomberg 브리프 스타일로 작성하세요. 사람들이 30초 안에 읽고 "감이 잡히는" 보고서가 목표입니다.
긴 문장 대신 핵심 수치와 인사이트를 압축하세요. 섹션별 지시를 엄수하세요.

**이슈 배경** (2~3문장 이내)
이 이슈가 왜 지금 중요한지, 역사적 맥락 한 줄 + 현재 상황 한 줄로 끝내세요.

**핵심 데이터** (불릿 3~5개)
구체적 수치, 날짜, 퍼센트를 포함한 팩트만 불릿으로. 설명 없이 수치 중심으로.

**시장 영향** (불릿 3~5개)
자산군별(주식/채권/환율/원자재) 영향을 ↑↓ 기호와 함께 불릿 한 줄씩. 수혜/피해 섹터 명시.

**한줄 인사이트**
투자자가 지금 당장 기억해야 할 핵심 메시지 딱 한 문장. 구체적이고 액션 지향적으로.

전체 500자 내외로 간결하게 작성하세요. 수식어 최소화, 수치 최대화.
마지막에 아래 JSON을 출력하세요:
<JSON>
