...
import { fetchTopMovers, renderMarketInternals } from "../lib/market-internals.js";
...
async function generateBrief(): Promise<MarketBriefResult> {
  ...
  // 병렬로 데이터 수집 (뉴스 포함)
  const [indexData, fredData, ecosData, pipelineStatus, newsResult, moversResult] = await Promise.allSettled([
    fetchRecentIndexData(),
    fetchFREDMacro(),
    fetchECOSMacro(),
    Promise.resolve(getStatus()),
    fetchMarketNews(),
    // 국내 시장 내부 — 이게 없으면 AI는 가진 것(미국 지수·SOX)으로만 단락을 채운다.
    fetchTopMovers(20),
  ]);
  ...
  const internalsBlock = moversResult.status === "fulfilled" ? renderMarketInternals(moversResult.value) : "";
  if (internalsBlock) console.log(`[market-brief] 국내 시장 내부 주입 (급등 ${moversResult.status === "fulfilled" ? moversResult.value.length : 0}종목)`);
  else console.warn("[market-brief] 국내 시장 내부 없음 — 미국 지표 편중 위험");
...
아래 JSON 형식으로만 응답하세요 (코드블록·설명 없이):
{
  ...
  "storyLine": "왜 미국 시장이 그렇게 움직였는지, 어떤 이슈가 있었는지(뉴스 헤드라인 참고), 그래서 오늘 한국 시장에 어떤 영향이 예상되는지 이야기처럼. SOX·달러 움직임, 주요 섹터(반도체·바이오·2차전지·방산) 영향, 기관·외국인 수급 동향도 포함. (400~600자)\n⚠️ **국내 시장 내부를 먼저 쓰세요.** 미국 지수·SOX·환율은 배경일 뿐, 그것만으로 단락을 채우면 실패입니다. 오늘 한국 시장에서 실제로 무슨 일이 있었는지 — 어떤 종목이 왜 움직였는지, 어느 업종에 자금이 몰렸는지, 공시·실적·수주·정책 발표가 있었는지 — 를 구체적인 종목명과 수치로 쓰세요. 코스피와 코스닥이 다르게 움직였다면 그 이유를 짚고, 대형주와 중소형주의 온도차도 언급하세요. 해외 지표 인용은 전체의 3분의 1을 넘기지 마세요. 매일 같은 프레임을 되풀이하면 실패한 분석입니다. 반드시 2~3개 단락으로 나눠 작성하고 단락 사이에 \\n\\n을 삽입하세요.",
  "marketEvents": [
...
아래 JSON 형식으로만 응답하세요 (코드블록·설명 없이):
{
  ...
  "storyLine": "오늘 하루 어떤 대내외 이슈가 있었고(뉴스 헤드라인 참고), 왜 시장이 그렇게 움직였는지, 내일·이번 주에 어떻게 될 것 같은지 이야기처럼. 국내 기업·정치 이슈와 글로벌 이슈, 주요 섹터(반도체·바이오·2차전지·방산·조선) 동향, 기관·외국인·개인 수급 흐름도 언급. (400~600자)\n⚠️ **국내 시장 내부를 먼저 쓰세요.** 미국 지수·SOX·환율은 배경일 뿐, 그것만으로 단락을 채우면 실패입니다. 오늘 한국 시장에서 실제로 무슨 일이 있었는지 — 어떤 종목이 왜 움직였는지, 어느 업종에 자금이 몰렸는지, 공시·실적·수주·정책 발표가 있었는지 — 를 구체적인 종목명과 수치로 쓰세요. 코스피와 코스닥이 다르게 움직였다면 그 이유를 짚고, 대형주와 중소형주의 온도차도 언급하세요. 해외 지표 인용은 전체의 3분의 1을 넘기지 마세요. 매일 같은 프레임을 되풀이하면 실패한 분석입니다. 반드시 2~3개 단락으로 나눠 작성하고 단락 사이에 \\n\\n을 삽입하세요.",
  "marketEvents": [
...
아래 JSON 형식으로만 응답하세요 (코드블록·설명 없이):
{
  ...
  "storyLine": "왜 오전에 이렇게 움직였는지(뉴스 헤드라인 참고), 국내외 어떤 이슈가 있었는지, 주요 섹터(반도체·바이오·2차전지·방산) 흐름, 기관·외국인 수급 동향, 오후에는 어떻게 될 것 같은지, 마감까지 무엇을 지켜봐야 하는지 이야기처럼. (400~600자)\n⚠️ **국내 시장 내부를 먼저 쓰세요.** 미국 지수·SOX·환율은 배경일 뿐, 그것만으로 단락을 채우면 실패입니다. 오늘 한국 시장에서 실제로 무슨 일이 있었는지 — 어떤 종목이 왜 움직였는지, 어느 업종에 자금이 몰렸는지, 공시·실적·수주·정책 발표가 있었는지 — 를 구체적인 종목명과 수치로 쓰세요. 코스피와 코스닥이 다르게 움직였다면 그 이유를 짚고, 대형주와 중소형주의 온도차도 언급하세요. 해외 지표 인용은 전체의 3분의 1을 넘기지 마세요. 매일 같은 프레임을 되풀이하면 실패한 분석입니다. 반드시 2~3개 단락으로 나눠 작성하고 단락 사이에 \\n\\n을 삽입하세요.",
  "marketEvents": [
...
아래 JSON 형식으로만 응답하세요 (코드블록·설명 없이):
{
  ...
  "storyLine": "이번 주 증시 스토리: 어떤 이슈가 시장을 움직였는지, 주요 섹터(반도체·바이오·2차전지·방산·조선) 흐름, 외국인·기관 수급 특이사항, 다음 주 어떻게 될 것 같은지. 국내·해외 이슈 모두 언급. (400~600자)\n⚠️ **국내 시장 내부를 먼저 쓰세요.** 미국 지수·SOX·환율은 배경일 뿐, 그것만으로 단락을 채우면 실패입니다. 오늘 한국 시장에서 실제로 무슨 일이 있었는지 — 어떤 종목이 왜 움직였는지, 어느 업종에 자금이 몰렸는지, 공시·실적·수주·정책 발표가 있었는지 — 를 구체적인 종목명과 수치로 쓰세요. 코스피와 코스닥이 다르게 움직였다면 그 이유를 짚고, 대형주와 중소형주의 온도차도 언급하세요. 해외 지표 인용은 전체의 3분의 1을 넘기지 마세요. 매일 같은 프레임을 되풀이하면 실패한 분석입니다. 반드시 2~3개 단락으로 나눠 작성하고 단락 사이에 \\n\\n을 삽입하세요.",
  "marketEvents": [
...
- 문체: 친근한 해요체`;
  ...
  const afternoonPrompt = middayPrompt
    ...
    .replace(
      "왜 오전에 이렇게 움직였는지(뉴스 헤드라인 참고), 국내외 어떤 이슈가 있었는지, 주요 섹터(반도체·바이오·2차전지·방산) 흐름, 기관·외국인 수급 동향, 오후에는 어떻게 될 것 같은지, 마감까지 무엇을 지켜봐야 하는지 이야기처럼. (400~600자)\n⚠️ **국내 시장 내부를 먼저 쓰세요.** 미국 지수·SOX·환율은 배경일 뿐, 그것만으로 단락을 채우면 실패입니다. 오늘 한국 시장에서 실제로 무슨 일이 있었는지 — 어떤 종목이 왜 움직였는지, 어느 업종에 자금이 몰렸는지, 공시·실적·수주·정책 발표가 있었는지 — 를 구체적인 종목명과 수치로 쓰세요. 코스피와 코스닥이 다르게 움직였다면 그 이유를 짚고, 대형주와 중소형주의 온도차도 언급하세요. 해외 지표 인용은 전체의 3분의 1을 넘기지 마세요. 매일 같은 프레임을 되풀이하면 실패한 분석입니다. 반드시 2~3개 단락으로 나눠 작성하고 단락 사이에 \\n\\n을 삽입하세요.",
...
아래 JSON 형식으로만 응답하세요 (코드블록·설명 없이):
{
  ...
  "storyLine": "월가 전문가 수준의 심층 시장 분석. 3~4개 단락 (단락 사이 \\n\\n 삽입, 전체 500~700자).\\n⚠️ 첫 단락은 **오늘 실제로 시장을 움직인 것**으로 시작하세요. 금리·연준으로 시작하는 것이 기본값이 아닙니다 — 그날 실적 발표, 개별 종목 급등락, 섹터 로테이션, 지정학 사건, 원자재, 수급이 주인공이면 그것으로 시작합니다.\\n아래 소재 중 **그날 실제로 움직임이 있었던 것만** 골라 쓰세요. 해당 없는 소재를 억지로 채우지 마세요:\\n· 실적 시즌: 어제·오늘 발표된 주요 기업 실적과 가이던스, 그 반응\\n· 개별 종목: 그날 크게 움직인 종목과 이유 (M&A·소송·규제·제품 발표 등)\\n· 섹터 로테이션: 어느 섹터에서 어디로 자금이 옮겨갔는지\\n· 시장 폭(breadth): 소수 대형주 주도인지 광범위한 상승인지, 동일가중 대비\\n· 금리·연준: **새로운 발언·지표·움직임이 있을 때만**. 어제와 같은 이야기는 반복하지 마세요\\n· 원자재·에너지·달러, 지정학·정책\\n⚠️ 매일 같은 프레임(금리 동결 기대감 등)을 되풀이하면 실패한 분석입니다. 오늘만의 이야기를 찾으세요.\\n반드시 수치와 종목명을 풍부하게 인용하고 '왜 그런지' 인과관계를 설명하세요.",
  "marketEvents": [
    { "title": "이슈 제목 (15자)", "impact": "이 이슈가 왜 시장에 영향을 줬는지 구체적 수치·종목명 포함 (70~90자)", "direction": "positive/negative/neutral" }
  ],
  "_marketEvents_규칙": "6~8개. **오늘 실제로 일어난 일**로 채우세요 — 아래는 후보 목록일 뿐이고, 해당 없는 항목은 넣지 마세요. 고정 칸을 억지로 채우면 '별다른 소식 없음' 같은 빈 내용이 들어갑니다. 후보: 개별 기업 실적·가이던스 / 대형주 급등락과 그 이유 / M&A·소송·규제 / 신제품·수주 발표 / 섹터 로테이션 / 실적 시즌 흐름 / 연준·금리(새 발언이나 지표가 있을 때만) / 경제지표 발표 / 원자재·에너지 / 지정학·관세 / 채권·달러 / IPO·자사주·배당. 같은 종류의 이슈로 절반 이상을 채우지 마세요.",
  "macroFactors": [
