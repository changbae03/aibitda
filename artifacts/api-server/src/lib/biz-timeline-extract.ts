...
const BIZ_SUBSECTIONS = [
  ...
  // head의 뒤쪽 대안(영업의 현황·영업설비·재무건전성)은 **금융사 서식**이다.
  // 은행·증권·보험·지주는 제조업의 "주요 제품/생산설비/매출·수주" 대신 이 머리글을 쓴다.
  // 이걸 안 넣으면 금융사는 "1. 사업의 개요"만 잡히고 예금·대출·NIM·점유율을 통째로 놓친다.
  { key: "사업개요",   head: /^\d+\.\s*사업의\s*개요/,                    weight: 1,   qWeight: 0.5 },
  { key: "주요제품",   head: /^\d+\.\s*주요\s*제품/,                      weight: 1.5, qWeight: 1.5 }, // 매출비중은 분기에도 변한다
  { key: "생산설비",   head: /^\d+\.\s*(원재료|생산|영업\s*설비)/,        weight: 1.5, qWeight: 1.5 }, // 생산실적·가동률(금융: 영업설비)
  { key: "매출수주",   head: /^\d+\.\s*(매출|영업의?\s*현황)/,            weight: 2,   qWeight: 2 },   // 매출·수주(금융: 영업의 현황=예금·대출)
  ...
  { key: "기타참고",   head: /^\d+\.\s*(기타\s*참고|재무건전성)/,         weight: 3,   qWeight: 0.5 }, // 산업분석·점유율(금융: 재무건전성)
] as const;
 ...
 */
const SIGNAL = /점유율|비중|시장\s*규모|경쟁|주요\s*고객|매출처|가동률|생산\s*능력|생산\s*실적|수주\s*잔고|연구개발비|증설|신규\s*(사업|수주|계약)|CAPA|M\/S|예수금|예금|대출금|여신|수신|순이자마진|NIM|연체율|BIS|고정이하/;
