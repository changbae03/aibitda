...
export interface KISEtfHolding {
  ticker: string;     // 종목코드
  name: string;       // 종목명
...
export interface KISEtfInfo {
  ...
  holdings: KISEtfHolding[];
}

/**
 * KIS API로 국내 ETF 구성 종목 조회
 * TR_ID: FHKST03030100
 * 6자리 종목코드 (예: 069500)
 */
export async function fetchKISEtfHoldings(code: string): Promise<KISEtfInfo | null> {
  try {
    const token = await getAccessToken();

    const url = new URL(`${BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-etf-portfolio-list`);
    url.searchParams.set("FID_COND_MRKT_DIV_CODE", "J");
    ...
    if (!res.ok) {
      console.warn(`[kis-etf] ${code} 조회 실패: ${res.status}`);
      return null;
    }

    const json = await res.json();
    if (json.rt_cd !== "0") {
      console.warn(`[kis-etf] ${code} 오류: ${json.msg1}`);
      return null;
    }

    const info = json.output ?? {};
    const items: any[] = json.output2 ?? [];
    ...
    // output2에 비중(etpr_cntr_rt) 가 퍼센트(%)로 들어오므로 /100
    const totalWeight = items.reduce((sum, h) => sum + (parseFloat(h.etpr_cntr_rt ?? "0") || 0), 0);

    const holdings: KISEtfHolding[] = items.map((h: any) => ({
      ticker: h.stck_shrn_iscd ?? "",
      name: h.prdt_abrv_name ?? h.stck_shrn_iscd ?? "",
      weight: (parseFloat(h.etpr_cntr_rt ?? "0") || 0) / 100,
      shares: parseInt((h.cstg_qty ?? "0").replace(/,/g, ""), 10) || 0,
    })).filter(h => h.ticker && h.weight > 0);

    const price = parseFloat((info.stck_prpr ?? "0").replace(/,/g, "")) || 0;
    const prevClose = parseFloat((info.stck_sdpr ?? "0").replace(/,/g, "")) || 0;
    const change = price - prevClose;
    const changePercent = prevClose > 0 ? (change / prevClose) * 100 : 0;

    return {
      ticker: code,
      name: info.prdt_abrv_name ?? code,
      price,
      change,
      changePercent,
      nav: parseNum(info.nav),
      expenseRatio: parseNum(info.etf_fee_rt) ? (parseNum(info.etf_fee_rt)! / 100) : null,
      holdings,
    };
  } catch (e: any) {
    console.error(`[kis-etf] ${code} 예외:`, e.message);
    return null;
  }
}
