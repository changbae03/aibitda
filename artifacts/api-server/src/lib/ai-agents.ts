...
**[STEP 1 — 내부 계산, 보고서 미출력] 서버 산출 앵커 확인**
컨텍스트에서 다음 두 섹션을 찾아 읽으세요:
① "[분기별 실적 — 최근 6분기 (fundamentalsTimeSeries)]" — 각 분기 OPM을 직접 확인
② "⛔⛔ [서버 산출 — bottom-up OPM 앵커]" — **서버가 사전 계산한 forward OPM 범위와 연간 영업이익 범위를 확인**

→ 서버 산출 forward OPM 중심값: __%  |  bottom-up 연간 영업이익 범위: __억~__억
→ 이 범위가 이후 분기 추정의 **최우선 기준**입니다. 이탈 시 반드시 명시적 근거를 제시해야 합니다.
...
> **OPM 이탈 근거** (이탈 발생 시만 작성): Q?E OPM __% — 서버 산출 범위 대비 __pp 차이: [원자재·수주·고정비 변화 등 구체적 사유]

⛔ **자기검증**: Q1확정 영업이익 > 0인데 연간계 ≤ Q1이면 물리적 불가 — 즉시 수정
⛔ **자기검증**: 연간계가 서버 산출 bottom-up 범위(STEP 1 ②)보다 크게 낮다면 top-down 역산이 개입된 것 — Q2~Q4 OPM을 서버 산출 기준값으로 재산출하세요.
