...
### 📅 ${new Date().getFullYear()}년 분기별 실적 전망 (bottom-up — 분기 합산이 연간을 결정)

⛔ **이 섹션을 먼저 완성하세요. 연간 표는 이 분기 합산 결과를 받아 작성합니다. top-down(연간 OPM 선설정 → 분기 역산) 방식은 금지.**

**[STEP 1 — 내부 읽기 전용, 보고서에 어떤 형태로도 출력 금지]**
컨텍스트에서 다음 두 섹션을 읽고 수치를 기억하세요:
① "[분기별 실적 — 최근 6분기 (fundamentalsTimeSeries)]" — 각 분기 OPM 확인
② "⛔⛔ [서버 산출 — bottom-up OPM 앵커]" — Q2E/Q3E/Q4E 분기별 OPM과 연간 영업이익 범위 확인
   → 이 값들이 이후 분기 추정의 최우선 기준입니다. 이탈 시 반드시 명시적 근거 필요.
   → ⛔ "서버 산출 forward OPM", "bottom-up 연간 영업이익의 범위", "서버 경고" 등의 문구를 보고서 본문에 절대 출력하지 마세요.

**[STEP 2] Q1(★확정) — 수정 불가**
"[분기별 실적 — 최근 6분기 (fundamentalsTimeSeries)]"에서 가장 최신 분기(★ 최신 확정 분기) 수치를 그대로 기입.
※ "[📊 DART 시계열]" 또는 "[분기별 전망 테이블 — 확정값 직접 사용]" 섹션이 있으면 해당 ★확정★ 행 우선.

**[STEP 3] Q2E~Q4E — OPM 추세 기반 bottom-up 추정**
- **기준**: STEP 1 ②의 서버 산출 forward OPM 범위 (top-down 연간 OPM 적용 금지)
- **계절성 (반드시 적용)**: 서버가 "계절성 데이터 부족" 경고를 출력했거나 Q2~Q4 OPM이 모두 동일하면, 아래 업종별 기본 패턴을 직접 적용하세요.
  - 건설 소재·인프라(파일·철근·레미콘 등): Q2 중립(+0~+0.5%p) / Q3 중립(-0.5~0%p) / Q4 강세(+1.5~+2.5%p)
  - 소비재·유통: Q4 강세(+2%p 이상) / Q1 약세(-1%p)
  - 반도체·IT: Q3 강세(+1%p) / Q1 약세(-1%p)
  - 조선·중공업: 연간 균등 (계절성 낮음)
- **업황·촉매 반영 (필수 — 하반기 호재 가중 반영)**:
  STEP 1 ①에서 수령한 "📅 분기별 반영 시점 표"를 찾아 각 분기 OPM 조정값을 서버 산출 OPM에 가산·차감하세요.
  → 표가 있으면: Q2E OPM = 서버산출OPM + 계절조정 + 촉매OPM조정, Q3E/Q4E도 동일 방식
  → 표가 없으면: 촉매 분석 본문에서 하반기 긍정 이슈(SOC 집행·수주 증가·리스크 해소 등)를 직접 찾아
    분기별 방향(↑/→/↓)과 %p 추정치를 명시적으로 적용하세요
- ⛔ **동일값 금지**: Q2E=Q3E=Q4E OPM이 모두 같으면 계절성·촉매 반영 누락 — 반드시 재검토 후 분기별로 다르게 설정하세요.
- ⚠️ **OPM 이탈 점검**: Q2~Q4 추정 OPM이 서버 산출 범위를 벗어나면 이탈 사유(촉매·업황 등)를 표 아래에 명시

**[STEP 4] 연간계 확정 = Q1(확정) + Q2E + Q3E + Q4E**
...
> **OPM 이탈 근거** (이탈 발생 시만 기재, 없으면 이 블록 전체 삭제): Q?E OPM __% — 추정 기준 대비 __pp 차이: [원자재·수주·고정비·촉매 등 구체적 사유]
> ⛔ 이 블록에 "서버 산출", "forward OPM 중심값", "bottom-up" 같은 내부 레이블을 절대 쓰지 마세요.

⛔ **자기검증**: Q1확정 영업이익 > 0인데 연간계 ≤ Q1이면 물리적 불가 — 즉시 수정
⛔ **자기검증**: 연간계가 STEP 1 ②의 연간 영업이익 범위보다 크게 낮다면 top-down 역산이 개입된 것 — Q2~Q4 OPM을 기준값으로 재산출하세요.

---

⚠️ **아래 연간 표의 ${new Date().getFullYear()}E 열은 위 분기 합산(연간계)을 그대로 옮겨 기입하세요. 별도로 top-down 재산출하지 마세요.**
...
**경로 ① 추가 상승 후 되돌림** — 모멘텀이 살아있어 단기 __원~__원까지 오버슈팅한 뒤, 적정주가 근방 __원대로 되돌아오는 시나리오. 상승이 지속되려면 무슨 조건이 필요하고, 어떤 이벤트·수치가 정점 전환 신호인지 구체적으로 서술.
...
**📅 분기별 반영 시점 (실적 전망 STEP 3 직접 사용):**

아래 표를 반드시 작성하세요. 이슈가 어느 분기부터 실적에 반영되는지, OPM에 몇 %p 영향을 주는지 추정합니다.
수치를 특정할 수 없으면 방향(↑/↓/→)과 이유 1줄로 표기. 빈칸 금지.

| 분기 | 매출 영향 | OPM 조정 | 반영 근거 (시점·조건) |
|------|---------|---------|-------------------|
| Q2E | +__억원 또는 방향 | +__%p 또는 방향 | [수주 인식·출시·계약 효력 발생 등] |
| Q3E | +__억원 또는 방향 | +__%p 또는 방향 | [분기 누적·확대·약화 등] |
| Q4E | +__억원 또는 방향 | +__%p 또는 방향 | [연말 반영·계절성 합산 등] |
