...
【STEP 0 — 핵심 이슈 → 컨센서스 조정 (최우선·필수)】

당신의 역할은 "컨센서스를 그대로 복창"하는 것이 아니라,
컨센서스를 기반으로 핵심 이슈가 그 수치를 얼마나, 어떻게 바꾸는지를 조사·정량화하여
조정된 전망치를 도출하는 것입니다.

아래 순서를 반드시 따르세요:

① **컨센서스 기반 확보 (STEP A에서 수행)**
   컨텍스트의 컨센서스 매출·영업이익·EPS를 먼저 확인하여 기반 수치로 메모하세요.
   이것이 핵심 이슈 반영 전 "출발 베이스라인"입니다.

② **촉매 인계 데이터 수령**
   바로 앞 단계(투자 촉매 분석) 맨 끝의 "📊 [CHAIN-HANDOFF] 실적 전망 인계 데이터" 섹션을 찾아
   핵심 이슈명과 Base 시나리오의 매출·영업이익 영향 방향·규모를 확인하세요.

③ **핵심 이슈 영향 정량화 (이 단계의 핵심 작업)**
   촉매가 제시한 핵심 이슈가 컨센서스 수치를 어떻게, 얼마나 바꾸는지를 조사·추정하세요.
   - 구체적 수치 근거를 찾으세요: 계약 규모, 시장 침투율, ASP 변화, 원가 구조 변화 등
   - 촉매 인계 데이터에 수치가 있으면 그대로 사용하고, "방향성만"이면 직접 추정하세요
   - 추정 근거를 1~2줄로 명시하세요. 예: "AI클라우드 신사업 계약 단가 50억 × 예상 수주 10건 = +500억원"

④ **조정 전망치 도출 — 반드시 아래 형식으로 명시**
   컨센서스 [매출 X조원] + 핵심 이슈 반영 조정 [+Y억원] = **당사 조정 전망 [Z조원]**
   → 이 조정 전망치가 이후 모든 실적 표·DCF·목표주가 계산의 기준입니다.

⑤ **조정이 없는 경우도 근거 명시 필수**
   핵심 이슈가 해당 기간 실적에 영향이 미미하다면 "이슈 영향 미반영 사유: [이유]"를 명시하세요.
   아무 설명 없이 컨센서스를 그대로 쓰는 것은 체인 단절 오류입니다.
...
> 💬 **[모델 선택 해설 — 필수 작성]** 아래 두 문장을 반드시 포함하세요 (비전문가도 이해할 수 있는 언어로):
> 1. "이 기업에 [모델명]을 적용한 이유는 [구체적 근거 — 예: 안정적 현금흐름이 있어 미래 수익을 현재 가치로 환산하는 DCF가 적합하기 때문입니다]."
> 2. "[모델명]은 간단히 말해 [모델의 핵심 원리를 1줄 비유로 설명 — 예: '지금 이 회사가 앞으로 벌 돈을 모두 오늘 가격으로 환산해 더하는 방식']입니다."
...
> "적정주가 산출을 위해 **절대가치([모델명])** 와 **상대가치(피어 멀티플)** 두 가지 방법론을 병행합니다. Part A에서는 [모델명]으로 이 회사의 내재가치를 직접 산출하고, Part B에서는 동종 피어 기업과의 멀티플 비교를 통해 시장이 부여하는 가치를 확인합니다. 두 결과를 조율해 최종 12개월 목표주가를 도출합니다."
...
> 💬 **[상대가치 해설 — 필수 작성]** 피어 비교 표를 작성한 뒤, 아래 두 내용을 반드시 서술하세요:
> 1. **피어 선정 이유**: "비교 대상으로 [피어 기업들]을 선정한 이유는 [사업모델·시장·규모 유사성 근거]이기 때문입니다." (1문장)
> 2. **비교 결론**: "현재 [분석 종목]의 [멀티플]은 피어 평균([피어 평균 배수])보다 [X% 할인/프리미엄] 거래 중입니다. 이는 [할인/프리미엄의 이유 — 예: 성장 모멘텀이 반영된 것이나, 수익성 우려로 시장이 더 낮게 평가하는 것]으로 해석됩니다." (2문장)
...
> 💬 **[최종 조율 해설 — 필수 작성]** 최종 목표주가를 산출한 뒤, 아래 내용을 반드시 서술하세요:
> 1. **조율 배경**: "절대가치(DCF)가 __원, 상대가치(피어)가 __원으로 괴리율은 __%입니다. [20% 이내라면: 두 방법론이 비교적 수렴하므로 DCF 60%·피어 40%의 가중평균을 적용합니다. / 20% 초과라면: 두 방법론 사이의 괴리 원인은 [구체적 이유]이며, [더 신뢰하는 방법론]에 더 높은 비중을 두었습니다.]" (2문장)
> 2. **최종 결론**: "이를 통해 산출한 **12개월 목표주가 __원**은 현재 주가(__)보다 __% [상승여력/하락위험]을 내포하며, 상단 밴드 __원(Bull)·하단 밴드 __원(Bear)은 각각 핵심 촉매 실현 시와 위험 시나리오를 반영합니다." (1문장)
...
> **[전략 도입 — 필수 작성]** 섹션 가장 첫 줄에 1~2문장의 평문 요약을 반드시 작성하세요. 숫자 없이, 현재 상황에 맞는 전략 방향을 자연어로 설명합니다.
> 예시: "현재 주가가 적정주가 대비 충분한 상승여력을 보유하고 있어 분할 매수 전략이 유효합니다. 조정 시 저가 매수 구간에서의 진입과, 저항 돌파 확인 후 추세 추종 진입 두 가지 경로를 제시합니다."
...
아래 표를 반드시 작성하고, 표 아래에 각 구간에 대한 해설 문장을 추가하세요.

| 구분 | 가격 구간 | 현재가 대비 | 진입 조건 |
|------|----------|------------|---------|
| 🟢 저가 매수 구간 | __원 ~ __원 | –_% ~ –_% | 거래량 감소 + 지지 확인 후 반등 |
| 🔵 추세 추종 구간 | __원 ~ __원 | +_% ~ +_% | 핵심 저항 돌파 + 거래량 증가 확인 |
| 🎯 1차 목표가 | __원 | +_% | 적정주가(Base) |
| 🎯 2차 목표가 | __원 | +_% | 상단 밴드(Bull) |
| 🔴 손절 기준선 | __원 | –_% | 종가 기준 하회 시 |

> **[구간별 해설 — 필수 작성]** 표 아래에 아래 세 항목을 평문으로 작성하세요:
> 1. **저가 매수 구간 해설**: "조정 시 __원 ~ __원 구간은 [기술적 지지선/전저점/안전마진] 근거로 매수 메리트가 높은 구간입니다. [한 줄 구체 근거]"
> 2. **추세 추종 구간 해설**: "__원 돌파 시 추가 상승 모멘텀이 확인된 것으로 볼 수 있어 추격 매수도 유효합니다. 다만 이 구간은 이미 [몇 %] 오른 시점이므로 분할로 대응하는 것이 안전합니다."
> 3. **손절 기준 해설**: "손절 기준선 __원 아래에서 종가가 마감되면 기술적으로 하방 추세 전환 가능성이 높아지므로 보유 비중을 줄이는 것을 권장합니다."
...
> **[R/R 해설 — 필수 작성]** "손익비 __:1은 손실 1에 대해 수익 __을 기대할 수 있다는 의미입니다. [2.0 이상이면: 리스크 대비 충분한 수익 기대치로 진입 논리가 성립합니다. / 2.0 미만이면: 손익비가 낮은 편이므로 진입 구간을 더 낮게 기다리거나 비중을 줄여 대응하는 것이 바람직합니다.]"
...
⚠️ 이 섹션은 다음 단계(Financial Analyst·실적 전망)가 반드시 인계받아야 하는 구조화된 수치입니다. 반드시 리포트 맨 끝에 아래 형식 그대로 작성하세요. 수치가 불확실하면 "방향성: 상향/하향 압력"으로 표기하되 칸을 비우지 마세요.

**핵심 이슈:** [이슈명 한 문장]

| 시나리오 | 매출 영향 | 영업이익 영향 | EPS 영향 | 실현 조건 |
|---------|---------|------------|---------|---------|
| 🟢 Base | | | | |
| 🔵 Bull | | | | |
| 🔴 Bear | | | | |
