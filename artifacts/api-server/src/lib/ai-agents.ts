...
⚠️ 기업이 2개 이상의 사업부문(사업부·세그먼트·제품군)을 보유하면 아래 테이블을 반드시 작성하세요.
⚠️ 단일 사업 기업 또는 세그먼트 정보가 전혀 없으면 이 섹션 생략 가능.
⚠️ 각 부문의 **업종 특화 핵심 KPI**를 가정치로 명시합니다 (매출만 쓰지 말 것).

**부문별 매출 및 이익 추정**

| 사업부문 | ${new Date().getFullYear()-1}(실적) | ${new Date().getFullYear()}E | ${new Date().getFullYear()+1}E | 핵심 KPI 가정 |
|---------|-------:|-------:|-------:|------------|
| [부문A] | | | | [KPI: 성장률/ASP/판매량 등] |
| [부문B] | | | | [KPI: 성장률/ASP/판매량 등] |
| [부문C] | | | | [KPI: 성장률/ASP/판매량 등] |
| **전사 합계** | | | | |

**업종별 핵심 KPI 가이드 (해당 업종의 KPI를 "핵심 KPI 가정" 열에 반드시 기재):**
- 반도체(메모리): Bit Growth(%), ASP YoY(%), 영업이익률(%)
- 반도체(비메모리/파운드리): 가동률(%), ASP 변화, 선단 노드 비중(%)
- 자동차: 판매량(만대), ASP(만원/대), 믹스(전기차 비중 %)
- 배터리/2차전지: 출하량(GWh), ASP($/kWh), 영업이익률(%)
- 디스플레이: 출하면적(㎡), 패널 ASP($), OLED 비중(%)
- 은행/금융: NIM(%), 대출성장률(%), 충당금 적립률(%)
- 통신: ARPU(원), 가입자 증가율(%), 5G 비중(%)
- 건설: 수주잔고(조원), 착공면적(㎡), 해외비중(%)
- 유통/소비재: 동일 점포 매출 성장률(%), 온라인 비중(%), GPM(%)
- 바이오: 품목별 매출, 임상 단계, 로열티율(%)
- 기타: 업종 특성에 맞는 가장 중요한 volume·price·margin 지표

**사업부문별 이익 기여도 요약 (1~2줄)**
올해E 기준 가장 큰 이익 기여 부문과 이익 성장의 핵심 드라이버를 간략히 서술하세요.
...
**올바른 재투자 계산법 — 3단계 (순서대로 모두 적용):**

**Step 1: S-to-C 공식값 계산**
S-to-C 공식 재투자 = 매출증분 ÷ S-to-C

**Step 2: 전년 Cascading Floor 적용 (가장 중요)**
전년 재투자가 X조원이었다면 → 당해 연도 재투자 최솟값 = X × 0.70
이 floor는 전년도 값에 연동되므로 연쇄적으로 적용됩니다:
- 2026년 재투자 = 240조 → 2027년 최솟값 = 240 × 0.70 = **168조** (74조 ❌ → 168조로 올려야 함)
- 2027년 재투자 = 168조 → 2028년 최솟값 = 168 × 0.70 = **118조**
- Step 1의 S-to-C 공식값이 이 Floor를 넘으면 공식값 사용, 넘지 못하면 Floor값 사용

**Step 3: 절대 하한 (바닥 중의 바닥)**
Step 2 결과가 아래 기업별 절대 하한보다 낮으면 절대 하한 사용:
- 삼성전자(005930): 절대 최솟값 **55조원/year** (어떤 시나리오에서도 이 밑으로 내려갈 수 없음)
- SK하이닉스(000660): 절대 최솟값 **18조원/year**
- TSMC(TSM): 절대 최솟값 **USD 250억/year**

**최종 재투자값 = MAX(Step1 공식값, Step2 Cascading Floor, Step3 절대 하한)**

⛔ **위반 사례 체크 — 반드시 표 작성 전 자가검증:**
1. 전년 대비 재투자 감소율 계산: (당년 − 전년) ÷ 전년 × 100
2. 이 감소율이 –30% 초과면 → Step2 Floor로 대체 후 재계산
3. 예: 2026년=240조, 2027년 산출값=74조 → 감소율 = –69% → 위반 → 168조로 교체
