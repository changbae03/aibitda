...
【STEP 0 — 촉매 인계 수치 고정 (최우선·필수)】
바로 앞 단계(투자 촉매 분석) 맨 끝의 "📊 [CHAIN-HANDOFF] 실적 전망 인계 데이터" 섹션을 찾아 Base 시나리오의 매출·영업이익·EPS 영향 수치를 확인하세요.
→ 이 수치가 이 단계의 **Base 실적 전망 기준값**입니다. 출발점이 아닌 확정 기준입니다.
→ 컨센서스는 검증 도구일 뿐입니다. 컨센서스가 촉매 수치와 충돌해도 촉매 Base 방향성은 반드시 전망치에 반영하세요. 컨센서스가 촉매 수치를 덮어씌울 수 없습니다.
→ 촉매 인계 수치가 없거나 "방향성: 상향/하향 압력"만 있는 항목: STEP A~C의 컨센서스·분기 데이터로 독립 추정하되, 촉매에서 확인된 방향성은 반드시 근거로 명시하세요.
→ ⛔ 촉매 분석이 매출 증가를 명시했는데 실적 전망이 컨센서스 기준 그대로라면 이는 체인 단절 오류입니다. 반드시 촉매 영향을 전망치에 반영하고 그 차이를 "촉매 반영 조정: +__억원" 형식으로 명시하세요.
