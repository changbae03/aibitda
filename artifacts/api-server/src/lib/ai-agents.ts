...
⑤ **합리성 검증 (내부용 — 보고서에 컨센서스 수치 직접 인용 금지)**
   컨텍스트의 애널리스트 컨센서스·분기 추세를 확인하여 당사 전망치의 방향이 크게 이탈하지 않는지 점검하세요.
   - 당사 전망이 컨센서스 방향과 같고 차이 ±30% 이내: 통과
   - 차이 ±30%~±60%: 추정 로직 근거를 강화하세요
   - **방향 반대(컨센서스 흑자인데 당사 적자, 또는 그 반대)**: ⛔ 이 경우 당사 추정치를 컨센서스 방향으로 반드시 조정하세요.
     조정 후에도 차이가 클 경우에만 보고서 본문에 "시장 전망 대비 보수적(또는 낙관적) 시각" 한 줄로 이유를 기술 (컨센서스 수치 직접 인용 금지)
...
한국 주식시장은 개인투자자 거래 비중이 60–70%에 달하고 테마·뉴스·심리가 단기 주가를 지배합니다. **소제목 없이** 3–4문장 줄글로 자연스럽게 이어서 서술하세요. 아래 4가지 판정을 문장 안에 판정 기호와 함께 녹여 쓰세요.
...
③ **셀온뉴스(Sell-on-News) 위험도** — 확정 발표 시 차익실현 출회 가능성:
   **高** (충분히 반영, 급락 위험) / **中** (일부 반영, 등락 혼재) / **低** (미반영, 추가 상승 여력)

④ **투자자 심리 온도계** — 최근 5일 수급 기반:
   🔴 극단적 탐욕 / 🟠 탐욕 / 🟡 중립 / 🟢 스마트머니 진입

**작성 예시 (이 형식으로):** "이 종목은 조선·방산 테마의 🎯직접 수혜주로, 신조선가 상승과 친환경 선박 전환이라는 실질 매출 드라이버가 테마 프리미엄을 뒷받침하고 있습니다. 테마는 52주 신고가 근접과 언론 집중이 겹치며 🔥과열 국면으로 진입했습니다. 셀온뉴스 위험은 **高**로, 최근 대규모 수주 공시에도 주가가 하락한 선례가 있어 확정 발표 시 차익실현 매물이 집중될 가능성이 큽니다. 최근 5일 수급은 3주체 혼조세를 보이며 🟡중립으로, 단기 방향성 확인 후 진입을 권장합니다."
