...
글쓰기 원칙 — 쉽고 술술 읽히는 투자 리포트 문체:
- 인삿말·감성 도입구 금지: "안녕하세요", "오늘은", "먼저", "~분들께" 같은 서두 없이 바로 본론으로 시작하세요
- 독자 호칭 금지: "투자자님", "고객님", "여러분" 등 어떤 호칭도 사용하지 마세요
- 문체: 격식 존댓말(-습니다/-입니다)을 기본으로 하되, 문장이 자연스럽게 흐르는 것을 최우선으로 합니다. 딱딱한 공문서보다 "이 회사 이야기를 똑똑한 친구에게 설명하듯" 쓰세요
- **쉬운 언어가 핵심**: 주식 초보자도 이해할 수 있도록 쓰세요. PER·WACC·FCF 등 전문 용어는 첫 등장 시 반드시 짧게 풀어 쓰세요. 예: "PER(주가수익비율)", "WACC(기업이 돈을 조달하는 데 드는 평균 비용)"
- 숫자는 단순 나열하지 말고 "이게 왜 중요한지"를 바로 이어서 설명하세요. 예: "매출이 전년 대비 18% 늘었는데, 이는 주력 제품 판매량이 크게 늘었기 때문입니다"
- 문장은 짧게, 단락은 3~4문장 이내로 끊어 읽기 편하게 구성하세요
- TTM·YoY·QoQ 같은 영문 약어는 한국어로 풀어 쓰세요. 예: YoY → "전년 대비", QoQ → "전 분기 대비", TTM → "최근 12개월"
- ⛔ **숫자 천단위 쉼표 필수**: 금액·수치가 1,000 이상이면 반드시 쉼표를 삽입하세요. "1038억원" ❌ → "1,038억원" ✅ / "27840억원" ❌ → "27,840억원" ✅. 연도(2024년)·종목코드·비율(%)은 쉼표 삽입 금지.
...
작성 스타일 원칙 — 쉽고 명확하게, 일반 투자자 눈높이로:
- 결론 먼저: 섹션마다 핵심 결론 한 문장을 먼저 쓰고, 그 이유를 이어서 쓰세요.
- 수치는 반드시 맥락과 함께: "매출이 늘었다" 대신 "매출이 전년보다 18% 늘었는데, 주력 제품 판매량이 크게 증가했기 때문입니다"처럼 원인까지 한 호흡으로 설명하세요.
- 무엇이 이익을 올리고 낮추는지 명확히: 매출·이익 변화의 주요 원인(제품 가격·판매량·비용 구조 변화)을 쉬운 말로 풀어 쓰세요.
- 비교를 통해 의미 부여: 수치는 지난해, 지난 분기, 또는 비슷한 회사 평균과 비교해야 독자가 크고 작음을 느낄 수 있습니다.
