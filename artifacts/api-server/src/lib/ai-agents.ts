...
최근 2~3년 실적 궤적을 2~3문장으로 서술하되 수치 중심으로 작성하세요 (매출·영업이익 CAGR, 턴어라운드 시점, 구조적 변화 등).

### 💹 수익성
⚠️ 마진율은 손익계산서 데이터에서 직접 계산하세요: 매출총이익률 = grossProfit ÷ totalRevenue × 100, 영업이익률 = operatingIncome ÷ totalRevenue × 100, 순이익률 = netIncome ÷ totalRevenue × 100. 연도별 손익계산서(incomeStatementHistory)가 2~4년치 있으면 연도별로 채우세요. ROE는 financialData.returnOnEquity × 100으로 계산하세요.
...
⚠️ 수치를 계산할 수 없는 셀은 "—"으로 표시하고, "데이터 없음"이라는 표현은 사용하지 마세요.

**마진 해설:** 마진 추세의 핵심 원인을 한 문장으로 설명하고, 업종 내 포지셔닝(상위/중위/하위권)을 언급하세요.

### 💰 현금흐름
⚠️ 영업CF = cashflowStatements.totalCashFromOperatingActivities, CAPEX = cashflowStatements.capitalExpenditures의 절댓값, FCF = 영업CF - CAPEX. 모두 최근 연도 데이터를 사용하세요.

- **영업CF:** __조원 | **CAPEX:** __조원 | **FCF:** __조원
- **FCF 전환율:** __% (= FCF ÷ 영업CF × 100 — 70% 이상 우량, 50% 미만 주의)
- **자본집약도:** CAPEX/영업CF = __%  — [자산경량형/자산집약형] 판단 한 줄
- **FCF 품질:** [우량/보통/주의] — 한 문장 해설

### 🏦 재무건전성
⚠️ 부채비율 = totalLiab ÷ totalStockholderEquity × 100. 순현금 = 현금성자산 - 총부채. 유동비율·당좌비율은 financialData에서 인용하세요. 이자보상배율이 계산 불가이면 생략하세요.

- **부채비율(D/E):** __% | **순현금(+)/순부채(-):** __조원
- **유동비율:** __x | **당좌비율:** __x
- **발행주식수:** __주
...
- 현재 주가: __원
- 발행주식수: __주 | 순현금(현금-부채): __조원
- Base EPS (올해E / 내년E): __원 / __원
- Base EBITDA (올해E / 내년E): __조원 / __조원
- Base 영업이익률: __% | Base 매출 (내년E): __조원`,
    },
...
주당 내재가치 = 주주가치(조원) × 1,000,000,000,000 ÷ 발행주식수
...
Base 추정 적용 10년 FCFF. 단위 조원, 발행주식수는 인계 요약에서 직접 인용.
...
주주가치 = 기업가치 합계 + 순현금 = __조원 | **DCF 내재가치: __원**
⚠️ 수치 정합성 검증 후 출력
