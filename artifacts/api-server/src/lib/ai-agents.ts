...
**핵심 질문에 답하세요**: 앞으로 1~3개월 이 종목의 가격이 어떻게 움직일 가능성이 높은가?

먼저 현재가와 밸류에이션 단계에서 산출된 적정주가(Base)를 수치로 비교하고, 괴리율을 명시하세요. (예: "현재가 __원은 적정주가 __원 대비 +__%로 과대평가 구간" 또는 "적정주가 __원 대비 –__% 저평가 구간")

이어서 아래 두 경우 중 해당하는 경우를 선택하여 심층 서술하세요:

**[현재가 > 적정주가 — 과대평가 구간인 경우]**
지금 주가를 끌어올리는 상승 모멘텀의 원인(테마·수급·기대감 등)이 무엇인지 1~2문장으로 설명하세요. 이어서 이 모멘텀이 언제, 어떤 조건에서 수그러들 수 있는지를 구체적으로 서술하세요. 어떤 이벤트(실적 발표 실망, 기관 이탈 가속, 거래대금 급감, 테마 소화 등)가 조정의 트리거가 될 수 있는지 분석하고, 가격이 되돌아올 수 있는 구간(기술적 지지선·적정주가 근방)을 수치로 제시하세요. ⚠️ 적정주가 < 현재가인 상황에서 가격이 계속 오르면 밸류에이션과 맞지 않음을 솔직하게 명시하고, 모멘텀이 지속될 수 있는 조건과 꺾일 시점을 함께 분석하세요.

**[현재가 < 적정주가 — 저평가 구간인 경우]**
현재 상승 모멘텀이 얼마나 더 지속될 수 있는지, 어떤 조건이 상승을 뒷받침하는지 서술하세요. 주요 저항 구간과 모멘텀이 약해질 수 있는 조건을 언급하세요.

마지막으로 투자자가 이 시점에서 주의해야 할 핵심 리스크 1~2가지를 간결하게 명시하세요.

⚠️ **중요 구분**: 적정주가는 12개월 내재가치 기반이며 단기 모멘텀과 다를 수 있습니다. 밸류에이션 기반 적정주가와 단기 모멘텀 전망을 독자가 명확히 구분할 수 있도록 서술하세요.
