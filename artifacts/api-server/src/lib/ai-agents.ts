...
**올바른 계산 방법 (2단계):**
1. **S-to-C 공식 재투자** = 매출증분 ÷ S-to-C (성장 투자분)
2. **재투자 하한선(Floor)** = 전년도 재투자 × 0.70 (최소 보장값)
3. **실제 적용값** = MAX(S-to-C 공식 재투자, 재투자 하한선)

**기업별 Maintenance Capex 하한선 (절대 하한, 매년 유지):**
- 삼성전자(005930): 재투자 최솟값 **55조원/year** (GAA 공정 전환·HBM 증설 최소 라인 유지)
- SK하이닉스(000660): 재투자 최솟값 **18조원/year** (HBM 후공정 + DRAM 공정 유지)
- TSMC(TSM): 재투자 최솟값 **USD 250억/year** (N3·N2 공정 유지 + CoWoS 증설)

**추가 검증 규칙:**
- ⛔ **전년 대비 재투자 –50% 초과 급감 절대 금지** (ex: 240조→74조는 69% 급락 → 즉시 재계산)
- ✅ 연도별 재투자 변화율은 **–30% ~ +40%** 이내로 제한
- ✅ NOPAT 대비 재투자율(= 재투자 ÷ NOPAT): 반도체 성장·유지기 **80–130%** 가 정상 범위 (2년 연속 40% 미만이면 가정 재검토)
- ✅ 예측 기간 전체(2026–2033) S-to-C를 일정 범위(±0.3) 안에서만 조정 — 구간별 급변 금지
