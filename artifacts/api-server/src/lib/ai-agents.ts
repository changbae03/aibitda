...
- 문장 순서: ① 기업명·업종·핵심 사업을 한 문장으로 → ② 이 회사의 대표 제품·서비스를 처음 듣는 사람도 바로 이해할 수 있도록 구체적으로 한 문장(예: "이 회사는 [제품/서비스]를 만들어 [고객/시장]에 판매합니다") → ③ 현재 주가 한 문장 → ④ 핵심 이슈 선언 한 문장 → ⑤ 분석 순서 안내 한 문장
- ⛔ 현재 주가 인용 규칙 (절대 준수): ③ 문장에서 현재 주가는 반드시 컨텍스트의 "=== KIS 실시간 시세 데이터 ===" 섹션에 있는 "현재가:" 수치를 그대로 사용하세요. 기억·추정·다른 섹션 수치로 대체하는 것은 절대 금지입니다. 예: KIS 현재가가 1,068,000원이면 반드시 "1,068,000원"으로 기재합니다.
...
분석 의뢰가 접수됐습니다. 팀장으로서 ① 기업명·핵심사업을 한 문장으로 소개하고, ② 이 회사의 대표 제품이나 서비스가 무엇인지 처음 이 기업을 접하는 사람도 바로 이해할 수 있게 구체적으로 한 문장으로 설명하고(어떤 제품을 만들어서 누구에게 파는지, 혹은 어떤 서비스를 제공하는지), ③ 현재 주가를 한 문장으로 언급하고(⛔ 현재 주가는 정확히 ${startPrice != null ? `${startPrice.toLocaleString("ko-KR")}원` : "컨텍스트의 KIS 현재가"}입니다. 이 숫자 외 다른 숫자를 쓰는 것은 절대 금지), ④ 지금 이 기업의 운명을 가를 핵심 이슈 1가지를 한 문장으로 명확하게 선언하고(예: 삼성전자라면 "HBM 수율 개선과 엔비디아 공급망 진입", SK하이닉스라면 "HBM3E 독점 공급 지속 여부", 에코프로비엠이라면 "전기차 배터리 수요 회복 시점"), ⑤ 본 리서치는 산업·촉매 분석에 이어 실적 전망, 적정주가 산출, 기술적 분석, 최종 결론 순으로 진행됩니다. 총 5~6문장.`,
    },
