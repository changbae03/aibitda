...
/**
 * **실행 순서**다. 화면의 카드 번호와는 다르다 — 카드는 자기 자리가 정해져 있고,
 * 각자 끝나는 대로 채워진다(3번이 먼저 끝나도 1·2번 자리는 그대로 위에 있다).
 *
 * 사업보고서 단계를 **맨 앞에** 둔다. 이 제품이 파는 것이 그 분석이고,
 * 가장 오래 걸리는 단계다. 뒤에 두면 앞 단계를 다 기다린 뒤에야 시작한다 —
 * 실제로 화면 3번인데 실행이 5번째라, 4번 카드가 다 나온 뒤에도 3번이
 * "읽는 중"이었다. 단계 자체는 22초다(자료 준비 4.6초 + 생성, 첫 글자 8.8초).
 *
 * 먼저 돌려서 손해 보는 것도 없다. 이 단계는 공시·재무로 자기 자료를 직접 모으고,
 * 뒤 단계들은 반대로 **사업보고서 분석을 읽고** 쓰게 된다.
 *
 * ⚠️ 웹의 `ANALYSIS_STEPS_ORDER`(lib/agents.ts)가 **다음 단계를 몰아간다.**
 * 서버만 바꾸면 순서가 안 바뀐다 — 두 곳을 함께 고칠 것.
 */
...
- 연도·기간이 들어가는 모든 표는 **최신 연도·분기를 맨 위**에 두고 아래로 갈수록 과거(내림차순)로 배열하세요. 예: 2026 → 2025 → 2024 순
...
역할: 이 기업의 실적·재무 흐름을 숫자로 정리하고, 그 숫자가 무엇을 말하는지 설명합니다.
단순 나열이 아니라 **무엇이 좋아지고 나빠졌는지, 그 변화가 얼마나 큰지**를 짚는 것이 핵심입니다.
숫자의 "왜"(사업 구조 변화·전략 행동)는 다음 단계인 사업보고서 행간 분석이 이어받으므로,
여기서는 근거 없는 사업적 추측을 하지 말고 **앞 단계(산업·촉매)에서 확인된 맥락까지만** 연결하세요.
...
🔗 **앞 단계(실적·재무 분석)를 이어받으세요.** 화면에서 독자는 바로 앞에 숫자를 봤습니다.
이 단계의 임무는 그 숫자의 **"왜"**를 사업보고서에서 찾아 주는 것입니다 — 매출이 왜 늘거나
줄었는지, 마진이 왜 변했는지, 그 뒤에 어떤 사업 구조 변화·투자·계약이 있었는지.
앞 단계와 같은 숫자를 다시 나열하지 말고, **그 숫자를 설명하는 사업적 사실**을 대세요.
...
- 뉴스가 그 지표와 무관하면 억지로 끼우지 말 것. 섹션 9는 이를 종합·판정하는 자리입니다.
...
⏱️ **분량**: 각 섹션은 **3~5문장**으로 씁니다. 표는 꼭 필요할 때만(연도별 비중 등).
같은 말을 다시 풀지 말고, 근거가 없는 섹션은 "공시 미확인" 한 줄로 끝내세요.
길게 쓴다고 더 나은 분석이 아닙니다 — 읽는 사람은 **바뀐 것**만 알고 싶어 합니다.
...
앞의 국면·6렌즈·행간을 모두 합쳐 **이 기업이 지금 전반적으로 좋아 보이는지, 신중히 볼 상태인지**를
투자자가 한눈에 알 수 있게 딱 한 줄로 판정하세요. (매수/매도 권유가 아니라 '상태' 판정입니다)
- 형식: → **종합: [긍정적 / 혼재 / 신중] — [핵심 이유 한 구절]**
- 예: "종합: 긍정적 — 실적이 높은 기대를 실제로 받쳐주고 있음"
- 예: "종합: 혼재 — 성장은 강하나 밸류에이션 부담이 큼"
- 예: "종합: 신중 — 기대는 큰데 실체(현금·수익성)가 아직 못 따라옴"
- 데이터가 부정적이면 정직하게 부정적으로 쓰세요. 좋게 포장하지 마세요.`,
    },
...
⚕️ **바이오 분기 (임상단계 신약개발사면 위 1~12 대신 아래 12개를 점검)**
매출이 미미하거나 영업적자이고 **가치의 대부분이 임상 파이프라인**에 있는 신약개발 바이오텍이면,
매출성장·OPM·ROE·PER는 무의미하므로 아래 바이오 전용 항목으로 대체합니다.
(이미 흑자인 CDMO·바이오시밀러·전통 제약사는 위 일반 12개를 그대로 사용)

⛔ **바이오 status 판정 원칙 (일반 종목과 다르게 봅니다)**: 임상단계 바이오는 매출·이익이 없는 게
정상입니다. "아직 이르다·미확정"을 fail로 몰지 마세요 — 그러면 잘 가는 종목도 전부 fail이 됩니다.
- pass: 그 단계에서 **긍정적 진척** (예: 임상 3상 진입=순항, MoA 검증됨, 특허 확보, 파이프라인 다변화,
  L/O 성사, 현금 런웨이 2년+, TAM 충분)
- warn: 아직 미확정이나 **해당 단계에선 정상 범위** (예: 초기 임상, L/O 협의 단계, 경쟁사 존재)
- fail: **실제 위험 신호일 때만** (현금 소진 임박 1년 미만, 임상 실패·중단, 과도한 오버행, rNPV 과대,
  임상 일정 반복 연기, 경영진 자사주 매도)
- verdict는 "미충족"이 아니라 **진척 상태**로 (예: "임상 3상 순항", "L/O 협의 중", "현금 2년 확보").
1. category: "파이프라인", item: "MoA·과학적 타당성" — 작용기전이 검증됐나. First-in-class(고위험 고수익)냐, 개량·Best-in-class냐
2. category: "파이프라인", item: "파이프라인 다변화" — 단일 후보 의존 vs 플랫폼 기술로 여러 적응증 확장(단일 실패 리스크)
3. category: "파이프라인", item: "특허 장벽(IP)" — 물질·용도 특허가 만료까지 넉넉한가(10~15년+)
4. category: "임상", item: "핵심 임상 단계" — 전임상~3상 어디인가. 2상 완료 시점이 L/O 가능성 최고
5. category: "임상", item: "규제·대조군 우위" — 패스트트랙·희귀약 지정 여부, 표준치료(SOC) 대비 유의미한 데이터
6. category: "검증", item: "빅파마 기술이전(L/O)" — L/O·공동개발 이력(외부 검증 신호). 행간의 제휴 신호 활용
7. category: "밸류에이션", item: "TAM·rNPV 정당성" — 타깃 시장이 충분하고 rNPV가 과대(TAM·Peak 과장) 아닌가
8. category: "해자", item: "경쟁·후발 리스크" — 동일 적응증 글로벌 경쟁사 임상 속도, late-mover 여부
9. category: "현금", item: "Cash Runway" — 증자·CB 없이 최소 1.5~2년+ 임상 지속 가능한가(현금÷연간 소진속도)
10. category: "현금", item: "오버행·희석 리스크" — 미상환 CB·BW·VC 물량 등 잠재 매도·희석 위험
11. category: "기대", item: "기대 선반영 여부" — 국면 지도 기준: 임상 기대가 주가에 과선반영됐나
12. category: "내러티브", item: "정합·레드플래그" — 스토리-데이터 정합 + 감점요인(임상 일정 반복 연기·경영진 자사주 매도·본업과 무관한 사업 다각화)
