...
- 분석 대상이 **연골 재생(DMOAD)** 효과를 목표로 임상을 진행 중이거나, DMOAD 지정을 목적으로 FDA·식약처에 신청한 경우:
  - **단순 통증 완화(진통제·소염제) 대비 가격 프리미엄 30~100% 적용 가능** — 근거: 기존 치료제는 증상만 완화, DMOAD는 질병 자체를 변형시키는 구조적 차별점.
  - **TAM 산정 시 "미충족 수요(Unmet Need)" 최상위 카테고리로 분류** — 현재 FDA 승인 DMOAD 골관절염 치료제 전무(시장 공백).
  - 가격 가정 서술 형식: "DMOAD 검증 시 연 치료비 __원 (기존 진통제 대비 __배 프리미엄 — 연골 재생 메커니즘 차별화 근거)"
  - **밸류에이션 프리미엄 섹션에서 반드시 1~2문장으로 DMOAD 시장 공백과 해당 자산의 위상을 명시**.
  - ⛔ DMOAD 임상을 "단순 관절염 치료제"로 축소 서술 금지.
