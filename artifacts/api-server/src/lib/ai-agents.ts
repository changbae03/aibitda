...
먼저 촉매 분석에서 도출한 핵심 이슈가 이번 실적 전망에 어떻게 반영되었는지 2~3문장으로 연결 서술하세요. 아래 구조를 따르세요:

> **[핵심 이슈 → 실적 연결 고리]**: 촉매 분석의 핵심 이슈("[이슈명]")가 매출·이익에 미치는 경로를 한 문단으로 설명하세요. "이 이슈가 실현되면 [구체적 사업 메커니즘 — 예: 수주 인식 시점, 단가 상승, 원가 구조 변화]으로 이어져, [올해E/내년E] 매출에 [+X% 또는 +Y억원] 기여할 것으로 가정했습니다." 형식으로 인과관계를 명확히 서술하세요.

이어서 위 표 수치를 뒷받침하는 핵심 가정을 불릿으로 서술하세요 (각 불릿에 수치 근거 포함):
- **[가정 항목]**: [수치 가정 내용] — [왜 이 수치인지 논리 1~2줄. 컨센서스 또는 과거 실적과의 비교 포함]
- **[가정 항목]**: ...
- **[가정 항목]**: ...

마지막으로 주요 리스크 1~2가지를 간략히 언급하세요 (수치 포함).
...
**선택 모델:** [모델명] — [선택 이유: 재무 데이터 근거 + 위 기준 중 어느 경우에 해당하는지, 'DCF 사용 금지 조건' 해당 여부 명시. 2~3문장 이내로 완결]

> 💬 **[모델 해설 — 필수 작성]** 아래 한 문장만 작성하세요 (비전문가도 이해할 수 있는 언어로, 위 선택 이유와 중복 금지):
> "[모델명]은 간단히 말해 [모델의 핵심 원리를 1줄 비유로 설명 — 예: '지금 이 회사가 앞으로 벌 돈을 모두 오늘 가격으로 환산해 더하는 방식']입니다."
...
각 시나리오는 (1) 실현 조건, (2) 사업 전개 방향, (3) 주가 시사점 순으로 서술하세요. 재무 수치(매출·영업이익 추정치) 작성 금지 — 대신 사업 모멘텀·경쟁 구도·투자자 심리 변화를 중심으로 해설하세요.

🟢 **Base (핵심 이슈 정상 실현):**
**실현 조건:** [핵심 이슈가 정상 궤도로 전개되는 조건 1~2문장]
**사업 전개:** [이 조건이 충족될 때 사업·경쟁 구도가 어떻게 변화하는지 2~3문장. 시장 점유율·고객사 관계·제품 포지셔닝 등 사업적 변화 중심]
**주가 시사점:** [투자자 심리·밸류에이션 재평가 방향·주가 촉매 관점에서 어떤 의미인지 1~2문장. 구체적 % 수치보다 방향성과 논리를 서술]

🔵 **Bull (초과 달성):**
**실현 조건:** [예상을 상회하는 조건 1~2문장]
**사업 전개:** [초과 달성 시 사업 모멘텀이 어떻게 가속되는지, 시장에서 어떤 포지션 변화가 생기는지 2~3문장]
**주가 시사점:** [투자자들이 이 시나리오를 어떻게 받아들일지, 멀티플 확장 또는 섹터 리레이팅 가능성 등 1~2문장]

🔴 **Bear (미실현·역풍):**
**실현 조건:** [이슈가 실패하거나 역풍이 부는 조건 1~2문장]
**사업 전개:** [실패 시 사업 전략이 어떻게 타격받는지, 경쟁사 대비 포지션이 어떻게 달라지는지 2~3문장]
**주가 시사점:** [투자 심리 위축·멀티플 하락 압력·리스크 요인 재부각 등 1~2문장]
