...
[표 앞 결론] 다음 형식으로 한 문장: "${companyName}은 [기간] 동안 매출 __→__조원(억원)으로 CAGR __%를 기록했으며, 영업이익률은 [개선/악화] 추세다."

| 지표 | ${new Date().getFullYear()-4} | ${new Date().getFullYear()-3} | ${new Date().getFullYear()-2} | ${new Date().getFullYear()-1} | TTM | 추세 |
|------|------:|------:|------:|------:|------:|:----:|
| 매출 (단위) | | | | | | ↑/↓/→ |
| 영업이익 (단위) | | | | | | ↑/↓/→ |
| 순이익 (단위) | | | | | | ↑/↓/→ |
...
⚠️ D/E: balanceSheetHistory totalLiab÷totalStockholderEquity 직접 계산. 없으면 financialData.debtToEquity 사용.
...
할인율(WACC): __% | 파이프라인 검토 기준: 임상 단계별 누적 성공 확률(PoS) 산업 평균 적용

📋 **PoS 업계 평균 참조표 (반드시 이 기준으로 적용)**
| 임상 단계 | 해당 단계→최종 허가 누적 PoS | 항암제(더 낮음) | 희귀질환/바이오시밀러(더 높음) |
|---------|--------------------------|-------------|--------------------------|
| 전임상 (Preclinical) | 5~10% | 3~6% | 8~15% |
| Phase 1 | 10~15% | 5~8% | 15~25% |
| Phase 2 | 25~35% | 12~20% | 35~50% |
| Phase 3 | 55~65% | 35~45% | 65~75% |
| 품목허가 신청(BLA/NDA) | 85~90% | 80~85% | 90~95% |
| 품목허가 완료 | 100% | 100% | 100% |
...
| 순현금(현금-부채, 단위) | |
| 발행주식수 | |
| 기존 사업(영업 중) 가치 산정 방법 | DCF 또는 EV/Sales (해당 시) |
...
**계산 방법**: TAM × 침투율 = Peak Sales → 연도별 매출/CF 추정 → WACC로 할인 → NPV(성공 시) → × PoS = rNPV

---

#### 📊 파이프라인 자산별 상세 계산

**(아래 블록을 파이프라인 자산 개수만큼 반복하세요. 컨텍스트에서 확인된 모든 파이프라인 자산 포함)**
...
|------|------|---------|
| 임상 단계 | | |
| 적용 PoS (모델 가정 섹션의 참조표 기준) | __% | Phase __기준 업계 평균 __% 적용 |
| 대상 환자 수 (국내/글로벌 시장) | | 역학 데이터 또는 AI 지식 기반 추정 명시 |
| 연 치료 비용 (가격 가정) | | 유사 치료제 기준 |
| TAM (총 시장 규모) | | 환자 수 × 치료 비용 |
| 당사 최대 시장 침투율 | __% | 경쟁 환경, 임상 데이터, 글로벌/국내 구분 |
| Peak Sales (당사 귀속분) | | TAM × 침투율 (파트너사 로열티 등 지급 구조 명시) |
| Peak Sales 도달 기준 연도 | __(현재로부터 __년 후) | 허가 예상 + Ramp-up __년 가정 |
| 매출 창출 기간 (특허 만료·독점기간) | | |
| 영업이익률 가정 (Peak 시) | __% | 유사 바이오 제품 근거 |

연도별 매출·현금흐름·현가 (허가 후 기준):

| 허가 후 연도 | 매출(억원) | 영업CF(억원) | PV(억원, WACC __%) |
|------------|-----------|------------|------------------|
| Year 1 | | | |
| Year 2 | | | |
| Year 3 | | | |
...
| [파이프라인 자산 2] rNPV | | | |
| [파이프라인 자산 3~] rNPV | | | |
| 기존 사업(DCF/운영 현금흐름) | | | 영업 중인 사업부 있을 경우 산정 |
| **파이프라인 rNPV 소계** | | 100% | |
| (+) 순현금 (현금 − 금융부채) | | | |
| **주주가치 합계** | | | |
...
- 암묵적 PoS = 내포 가치 ÷ NPV(성공 시) = __%
- 업계 평균(Phase __기준 __%) 대비 [과소/적정/과대] 반영
- 결론: "현재 주가는 핵심 파이프라인의 PoS를 __%로 반영 중이며, 이는 [달성 가능/도전적/과대/과소] 수준이다"
...
⚠️ 컨텍스트에 "피어 그룹 실시간 재무 데이터 (Yahoo Finance)" 섹션이 제공됩니다.
- 시가총액, PER, PBR, EV/EBITDA, EV/매출, ROE, 영업이익률, 순이익률, 매출(TTM) 등 **모든 수치를 해당 섹션에서 직접 인용**하세요. 추정·추측·임의 채움 금지
- 피어 기업명은 반드시 **실제 회사명**으로 작성하세요. "Peer A", "Peer B" 등 플레이스홀더 사용 금지
- 컨텍스트에 "N/A"로 표기된 수치는 표에도 "N/A"로 유지하세요. 임의로 수치를 채우지 마세요
- 컨텍스트에 피어 데이터가 전혀 없을 때만: "컨텍스트에 피어 데이터 미제공 — AI 지식 기반 추정" 명시 후 업종 정상 멀티플 범위를 기준으로 추정하세요
