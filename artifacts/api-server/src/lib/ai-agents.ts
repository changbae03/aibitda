...
위 표의 숫자를 처음 이 기업을 보는 사람도 이해할 수 있도록 아래 구조로 해설하세요. 전문 용어 최소화, 평이한 문장으로 작성하세요.

**[왜 매출이 이렇게 늘어날까요?]**
올해E·내년E 매출 성장의 핵심 이유를 2~3문장으로 설명하세요. "○○ 사업이 커지는 이유는 ~~이기 때문입니다. 구체적으로는 [수주 증가 / 신규 고객사 확보 / 단가 상승 / 시장 점유율 확대] 등이 매출에 기여합니다." 형식으로, 수치(억원 또는 %)를 최소 2개 포함하세요.

**[이익률은 왜 이 수준인가요?]**
영업이익률 가정의 이유를 2문장으로 설명하세요. "영업이익률을 XX%로 가정한 이유는 [원가 구조 / 고정비 레버리지 / 연구개발비 증감] 때문입니다." 형식. 과거 실적 또는 경쟁사 대비 수치를 언급하세요.

**[가장 중요한 변수는 무엇인가요?]**
이 전망이 맞거나 틀릴 수 있는 핵심 변수 2~3가지를 나열하세요. 각 항목을 불릿으로, "만약 [조건]이 달라지면, [매출/이익]이 [어느 방향으로] 바뀔 수 있습니다" 형식으로 서술하세요.
