...
⛔ **[Year 1 매출 서버 앵커 — 최우선 준수]**: 컨텍스트에 "[📌 Year 1 매출 서버 앵커]" 섹션이 있으면:
  - "⛔ DCF Year 1 매출이 XXX 초과하면 즉시 수정 필수" 줄에 명시된 값이 DCF Year 1 매출의 절대 상한입니다.
  - DCF 테이블 Year 1 매출이 이 값을 1원이라도 초과하는 것은 금지.
  - 이 앵커를 무시하고 상위 매출을 사용하면 전체 DCF가 비현실적 수준이 됩니다.

⛔ **성장률 100% 이상 감지 규칙**: 컨텍스트에 매출 성장률이 100%를 초과하는 수치가 있으면:
  - 이는 저점 대비 기저효과(전년 적자→흑자 전환) 또는 Yahoo Finance 데이터 품질 문제 가능성이 높습니다.
  - 실제 매출액(조원/억원) 절대치 변화를 직접 계산해 성장률을 재산출하세요.
  - 예: 2025 매출 97조 → 2026 컨센서스 150조 = **+54.4% (올바른 계산)**
  - ⚠️ "228.8% 성장"처럼 그대로 쓰지 말고 반드시 절대치로 역산 검증
  - EPS 성장률(YoY)은 절대로 DCF 매출 성장률로 사용하지 마세요 (컨텍스트에서 "← DCF에 사용 금지" 표시된 항목)

⛔ **[DCF 영업이익률(OPM) 상한 — 업종별 엄격 적용]**
  반도체·메모리 업종(SK하이닉스·삼성전자·Micron 등):
    - Year 1~3: 컨센서스 OPM 사용 가능, 단 최대 60%
    - Year 4~10 (장기): 최대 45% (경쟁 심화·가격 사이클로 수렴)
    - Terminal Year: 최대 40%
  일반 반도체 설계(팹리스): 최대 40%
  파운드리(TSMC급): 최대 50%
  컨텍스트에서 컨센서스 OPM이 60% 이상으로 제시되는 경우, 이는 1~2년 피크 사이클 추정치이므로 장기 DCF에 그대로 사용 금지.
  OPM이 60% 초과인 경우 사용 전 반드시 "⚠️ 고OPM 사이클 피크 가정 — 장기 지속 가능성 검토" 1줄 명시.
