...
| 고성장 + FCF 음수 (비바이오) | EV/Sales 기반 DCF | 흑자 전환 임박 시 DCF 전환 |
| 바이오 — 임상단계 (미허가, 영업적자) | Pipeline rNPV | 파이프라인 가치가 시총의 대부분인 경우 |
| 바이오 — 상업화 완료 (허가된 주력 제품 있음, 매출 성장 중) | DCF 또는 EV/Sales | 매출·FCF 가시성이 충분한 경우 |
| 바이오 — 파이프라인 + 기존 매출 혼합 | DCF(기존 사업) + rNPV(파이프라인) 합산 가능 | 두 가치가 비슷하게 큰 경우 |
...
- 컨텍스트에 피어 데이터가 없으면: "컨텍스트에 피어 데이터 미제공 — AI 지식 기반 추정" 명시 후 AI가 직접 피어와 멀티플을 추정하세요
