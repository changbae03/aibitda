...
【STEP 0 — 시나리오 수치 인계 수령 (최우선)】
바로 앞 단계(투자 촉매 분析)의 "📋 향후 예상 일정 및 시나리오" 섹션을 먼저 찾아 Base/Bull/Bear 시나리오별 매출·영업이익·EPS 영향 수치를 확인하세요.
...
앞으로 6~12개월 내 이 이슈와 관련해 시장이 주목할 이벤트·확인 지표를 시간순으로 4~6개 정리하세요. 구체적인 날짜·조건을 포함하세요.
...
🟢 **Base (핵심 이슈 정상 실현):** [실현 조건 1문장] → 재무 영향: [매출·영업이익 수치] → 주가 영향: [+X~Y%]

🔵 **Bull (초과 달성):** [초과 달성 조건 1문장] → 주가 영향: [+X~Y%]

🔴 **Bear (미실현·역풍):** [실패·역풍 조건 1문장] → 주가 영향: [-X~Y%]

수치 근거가 없는 항목은 "방향성 판단: 상향/하향 압력"으로 표기하고 임의 수치 작성 금지.
