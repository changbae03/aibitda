...
각 섹션 작성 기준:
- 분량: 섹션당 8~10문장
- 수치: 매출·이익·마진·ROE·ROIC·주당가치 등 구체적 수치를 최소 3개 이상 포함
- 직전 섹션을 참조하는 문구 ("Macro & Industry Analyst에 따르면" 등) 전면 금지
- 소제목은 이모지 + 제목만 사용하고 부제 설명 텍스트는 붙이지 마세요

## 📊 재무 현황

최근 3년 매출·영업이익·순이익 추이를 실제 수치로 직접 인용하고, 매출총이익률·영업이익률·순이익률을 제시하세요. FCF와 영업현금흐름을 수치로 비교하고, FCF 전환율(FCF/영업이익)을 평가하세요. ROE·ROA를 업계 평균 대비 수치로 제시하고, 부채비율·유동비율·순현금(현금-부채) 규모를 서술하세요. 재무구조의 강점과 취약점을 각 1~2가지씩 수치로 뒷받침해 결론 내리세요.

## ⚙️ ROIC & 핵심 가정

최근 3년 평균 ROIC와 Forward ROIC 수치를 제시하고 WACC와 비교해 가치 창출 여부를 판정하세요. ROIC가 WACC를 상회하는지 여부와 그 격차를 명시하고, 이것이 현재 밸류에이션 프리미엄을 정당화하는지 판단하세요. 핵심 이슈를 한 문장으로 선언한 뒤, 아래 표를 Bear/Base/Bull 시나리오로 수치를 채워 작성하세요. 없는 데이터는 추정 기준을 적용하세요.
...
주주가치(조원) = 기업가치 합계 + 순현금 = __조원
주당 내재가치(Base) = __조원 × 1,000,000,000,000 ÷ __주 = __원
Bear Case 주당가치: __원 | Bull Case 주당가치: __원

## 🔄 Reverse DCF & 멀티플 크로스체크

현재 주가가 전제하는 요구 ROIC와 성장 지속 기간을 역산하여 "현재 주가는 ROIC __%, __년 유지를 전제한다"는 문장으로 명시하세요. 이 가정이 글로벌 동종 최상위 대비 달성 가능한지 판정하고, 코리아 디스카운트 해소 가능성과 구조적 리레이팅 근거를 서술하세요. 아래 멀티플 크로스체크 표를 채운 뒤, DCF 내재가치와의 정합성을 평가하고 최종 주당가치 레인지를 제시하세요.
