...
아래 5개 섹션을 순서대로 작성하세요. 소제목은 이모지 + 제목만 사용하세요.
...
지금 주가가 핵심 이슈를 얼마나 이미 반영하고 있는지를 수치로 진단합니다.

핵심 이슈 인식 시점 이후 현재까지 주가 상승률을 수치로 제시하고, 앞서 촉매 vs 리스크 섹션의 Base 시나리오 주가 영향(%)와 비교하여 아래 4단계 중 하나를 선택하세요.
...
판정 결과와 근거를 한 문장으로 작성하세요. 매출·영업이익·EPS 절대 수치는 다시 나열하지 마세요. 실적 기대감은 방향성 표현 한 문장으로만 서술하세요.

### 셀온뉴스 리스크

위 수급 흐름과 선반영 판정을 종합하여 경보 수준을 결정하고, 대응 가이드를 작성하세요.

- 🔴 **높음** — 선반영 과대·과열 + 기관·외국인 순매도 전환 + 이벤트 4주 이내 → 이벤트 발표 전 보유 비중 일부 축소 또는 신규 진입 자제. 발표 직후 단기 변동성 대비 — 첫 거래일 종가 확인 후 재판단. 재진입 기준가를 구체적 수치로 제시.
- 🟠 **중간** — 선반영 과대·과열 OR (적정반영 + 기관·외국인 순매도 전환) → 이벤트 전후 분할 매수 전략 권장. 기대치 부합 시에도 단기 조정 가능성 열어두기.
- 🟡 **낮음** — 선반영 과소·적정 + 기관·외국인 순매수 유지 → 이벤트 발표가 추가 상승 촉매로 작용할 가능성 — 발표 결과 확인 후 포지션 강화 검토.
- ⚫ **해당 없음** — 임박한 대형 이벤트가 없거나 이슈가 이벤트형이 아닌 경우 → 단기 셀온뉴스 리스크 감지되지 않음. 펀더멘털 추세 중심으로 판단.`,
    },
