...
⚠️ 비정상 FCF 보정 규칙 (반드시 준수):
컨텍스트의 FCF(잉여현금흐름)가 영업현금흐름의 2배 이상이거나, 영업이익이 적자인데 FCF가 양수인 경우는 자산 매각·투자금 회수 등 일회성 요인이 포함된 것입니다.
이 경우 보고된 FCF를 DCF 입력값으로 사용하지 마세요. 대신 시나리오별 실적 전망에서 추정한 NOPAT을 DCF의 출발점으로 사용하고, "FCF 비정상 감지: 보고 FCF __억원을 제외하고 추정 NOPAT 기반으로 DCF 산출"이라고 명시하세요.

⚠️ 목표주가 극단값 자기검증 규칙:
- Base 목표가가 현재 주가의 3배 이상이면: 반드시 DCF 입력 가정을 재검토하고 이유를 한 문장으로 명시
- Bear 목표가가 현재 주가의 30% 미만이면: 청산 가치(순자산 × 0.5)와 비교 후 더 높은 값 사용
- 세 방법론(DCF/P/E/EV·EBITDA)의 Base 목표가 최대 괴리가 500% 이상이면: DCF 단독 사용 금지, 멀티플 평균으로 대체
