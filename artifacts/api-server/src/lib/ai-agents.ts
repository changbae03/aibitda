...
⛔ **[STEP 2.5 — OPM 현실성 상한선 검증 (STEP 2 직후 필수 실행)]**

STEP 2에서 산출된 연간 OPM 앵커가 아래 업종별 역사적 상한을 초과하면 → 즉시 상한선으로 강제 조정하고, 표 아래에 "(OPM 상한 적용: [업종] 역사 피크 기준 [X]%로 제한)" 주석을 삽입하세요.

| 업종 | OPM 상한 | 역사적 근거 |
|------|---------|-----------|
| 메모리 반도체 (SK하이닉스·삼성전자 DS부문) | **65%** | 역사 피크 55~60% + HBM 믹스 프리미엄 최대 5pp |
| 파운드리·시스템반도체 (TSMC·DB하이텍 등) | **55%** | TSMC 역사 피크 약 47% |
| 반도체 장비·소재 | **35%** | ASML·한미반도체 역사 피크 수준 |
| 조선·중공업 | **18%** | 한국 조선사 호황기 피크 12~15% |
| 철강·화학·소재 | **25%** | 포스코·LG화학 역사 피크 수준 |
| 소비재·유통·음식료 | **20%** | 구조적 마진 한계 |
| 바이오·제약 (매출발생 기업) | **40%** | 글로벌 빅파마 수준 |
| 소프트웨어·SaaS·플랫폼 (순수) | 상한 없음 | 단, 70% 초과 시 근거 문장 1줄 필수 |
| 기타 업종 | 역사 최고 OPM + 10%p | 컨텍스트 데이터에서 역산 |

적용 예시:
- SK하이닉스 2026E OPM 75% 도출 시 → ⛔ 메모리 반도체 상한 65% 초과 → 65%로 강제 조정
- NVIDIA 2026E OPM 63% 도출 시 → 빅테크·GPU 소프트웨어 혼합 → 상한 없음, 근거 1줄 명시 후 통과
