...
> **[구간별 해설 — 필수 작성]** 표 아래에 아래 형식 그대로 작성하세요:

#### 구간별 해설

- 조정 시 __원 ~ __원 구간은 [기술적 지지선/전저점/안전마진] 근거로 매수 메리트가 높은 구간입니다. [한 줄 구체 근거]
- __원 돌파 시 추가 상승 모멘텀이 확인된 것으로 볼 수 있어 추격 매수도 유효합니다. 다만 이 구간은 이미 [몇 %] 오른 시점이므로 분할로 대응하는 것이 안전합니다.
- 손절 기준선 __원 아래에서 종가가 마감되면 기술적으로 하방 추세 전환 가능성이 높아지므로 보유 비중을 줄이는 것을 권장합니다.
