...
${companyName}이 속한 산업을 아래 4개 섹션으로 분析하세요.
...
필드별 작성 기준:
- summary: 아래 6개 관점을 자연스럽게 연결한 5~7문장 줄글로 작성하세요. 마크다운 볼드(**) 없이, 주식 초보자도 읽을 수 있는 쉬운 말로, 반드시 각 단계의 핵심 수치를 포함하세요.
  ① **핵심 이슈 + 현재 상황**: "[핵심 이슈]가 지금 이 기업 주가 방향을 결정하고 있으며, 현재 [진행 단계 — 초기/가속/성숙]에 와 있습니다." — 이슈가 무엇인지, 지금 어디쯤 왔는지 한 문장
  ② **산업·경쟁 포지션**: 이 기업이 속한 산업에서 구조적으로 유리한 점 또는 주의해야 할 점 한 문장 (경쟁 우위·시장 점유·마진 차별화 등 수치 포함)
  ③ **실적 전망**: 이 이슈가 실적에 미치는 영향과 올해E·내년E EPS(또는 영업이익) 수치를 포함한 한 문장
  ④ **밸류에이션·목표가**: "실적 기반 적정주가는 [X원/달러]로, 현재가 대비 [Z]% [상승/하락] 여력" 한 문장 — 목표가 수치 필수
  ⑤ **핵심 촉매 또는 리스크**: 앞으로 가장 중요한 확인 포인트(긍정 촉매 또는 핵심 리스크) 한 문장 — 구체적 시점·조건 포함
  ⑥ **최종 판정 근거**: 이 모든 근거를 종합한 판정 이유와 투자자가 지금 취해야 할 행동 한 문장
  ⚠️ Sell/Strong Sell인 경우: 매수 권유 문구 절대 금지. ①을 "현재가 X원은 적정주가 Y원 대비 Z% 과대평가 상태입니다."로 시작하고, ⑤에 과열 해소 조건과 재관심 기준가(예: __원 ~ __원)를 포함하세요.
