...
**[STEP 1] 아래 수치를 이전 단계에서 반드시 인용하세요.**
- 적정주가(Base): __원 / 상단 밴드(Bull): __원 / 하단 밴드(Bear): __원
- 현재가: __원
- Base upside = (적정주가 - 현재가) / 현재가 × 100 = __%

**[STEP 2] 아래 기준에 따라 전략 유형을 선택하고, 선택하지 않은 섹션은 완전히 생략하세요.**

▶ Base upside < –10%  →  [A. 매도 대응 전략]만 작성 (매수 관련 내용 일체 금지)
▶ Base upside >= –10% →  [B. 매수 진입 전략]만 작성

현재 종목의 Base upside: __% → 따라서 **[A / B 중 선택]** 전략을 작성합니다.
...
- R/R 역전: 상방 +__% vs 하방 –__% → **하방 압력 우세, 신규 진입 비합리적**

---

### [B. 매수 진입 전략] ← Base upside >= –10%일 때만 작성

**현재가와 진입 구간 관계 판단 (케이스 선택)**

**[케이스 A] 현재가 ≤ 저가 매수 구간 상단 (이미 매수 구간 진입)**
- "저가 매수 구간(__원 ~ __원)에 **현재 진입 중** — 지금 현재가(__원)가 이 구간에 위치."
- 구간 이탈(하향) 시 손절, 구간 상단 이탈(상향) 시 추세 추종 전략으로 전환

**[케이스 B] 현재가가 저가 구간 위, 추세 추종 구간 아래 (대부분 케이스)**
...
**[케이스 C] 현재가 > 상단 밴드 (소폭 과열, upside는 –10% 이상)**
