...
⚠️ 모델 선택 우선 규칙 (아래 순서로 적용):
1. **바이오/제약/의료기기 기업** + 영업적자(영업이익 < 0) + 임상 파이프라인 보유 → **Pipeline rNPV 필수** (DCF 사용 금지)
2. 매출 발생 중인 성숙 기업 + FCF 양수 + 비바이오 → DCF (NOPAT/FCFF)
3. 고성장 기업 + FCF 음수 + 비바이오 → EV/Sales 기반 DCF
4. 금융주(은행/보험/증권) → DDM (Gordon Growth)
5. 부동산/인프라 → NAV
6. 순현금 대형주 → DCF + 순현금 가산

**선택 모델:** [모델명] — [이유 한 줄 — 위 규칙 번호 명시]
