...
⛔ **[서버 계산 DCF 매출 출발점 — 최우선 준수]**:
  ...
  - 이 값을 초과하는 Year 1 매출은 어떤 경우에도 금지입니다.
...
⛔ **[Year 2 이후 성장률 — 반드시 현실적 수렴 적용]**:
  ...
  **Year별 현실적 성장률 가이드**:
  ...
  - Year 2: **Year 1 성장률의 30~50%로 감소**. 예: Year 1이 +55%이면 Year 2는 +15~25%
...
═══ 극단값 양방향 가드레일 ═══
⚠️ 목표가 ≥ 현재가 × 2.5 (upside 150% 이상) → 내부 재검토 체크리스트 실행:
  1. WACC가 성숙 대형주 기준 10% 이상인지 확인
  2. g가 2.0% 이하인지 확인
  3. Terminal Value 비중이 80% 이하인지 확인
  4. [🔒 서버 계산 DCF 매출 출발점] 섹션의 Year 1 확정값을 초과하지 않았는지 확인
  5. NOPAT이 EBIT × (1-세율)로 계산됐는지 확인 (세전 영업이익 사용 시 33% 과대)
  → 위 5개 항목 점검 후 수정된 목표가를 최종값으로 사용 (보고서에 "재검토" 언급 없이 수정값만 제시)
⚠️ 목표가 ≤ 현재가 × 0.5 → "목표가 극단적 하락 감지" — 피어 할인 근거 또는 부채 구조 재검토 필수
⚠️ upside 80% 초과 → 반드시 구조적 성장·카탈리스트 근거를 보고서에 명시
...
아래 절대 상한을 초과하는 배수는 이상치로 즉시 제외:
- EV/EBITDA: 30배 초과 → 이상치 (수익성 왜곡 or 적자 전환 가능성)
- PER(Fwd): 60배 초과 → 이상치
- EV/Sales: 업종 한국 벤치마크 상단의 2배 초과 → 이상치
  예: 반도체 EV/Sales 한국 벤치마크 최대 3.5x → 7.0x 초과이면 이상치

**[STEP 2 — 중간값 상대 필터 (후적용)]**
유효 피어의 중간값(median) 계산 후:
- 해당 배수가 **median의 2.0배 초과** → 고배수 이상치 제외
- 해당 배수가 **median의 0.3배 미만** → 저배수 이상치 제외 (단, 구조적 할인 기업이면 유지 후 별도 설명)

이상치 처리:
- 표에는 포함하되 "(이상치 제외)" 표기 후 해당 기업은 **피어 중간값 계산에서 제외**
- 이상치 제외 후 남은 피어 수가 2개 미만이면: 대체 피어를 추가 선정
- **이상치가 있으면 피어 평균 대신 중간값(median)을 적용 배수 기준으로 사용 (필수)**
...
→ 적용 방법: 글로벌 피어 median 배수 × (1 − 구조적 할인율) = **한국 적용 배수**
→ 예: 글로벌 피어 EV/EBITDA median 14x × (1 − 12%) = **12.3x 적용**
→ 단, **한국 국내 피어(코스피·코스닥)를 우선 사용하면 이 추가 할인 불필요**

---

⚠️ **적용 배수 자의성 방지 규칙**:
- 피어 median과 다른 배수 적용 시: **구체적 근거 2가지 이상** 명시 필수
- 적용 배수가 피어 median(이상치 제외 후)의 1.5배 초과 → "프리미엄 과대 경고" 표시
