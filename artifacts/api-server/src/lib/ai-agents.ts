...
> **[상황 해설 — 필수 작성]** 왜 현재가가 과열 구간인지 2~3문장으로 설명하세요. 밸류에이션 기준(적정주가와의 괴리), 기술적 신호(이평선 이격·RSI 수준), 최근 주가 급등의 배경을 연결해 독자가 "왜 지금 사면 안 되는가"를 직관적으로 이해할 수 있도록 서술하세요.
...
> **[차익실현 해설 — 필수 작성]** 1차·2차 차익실현 가격을 왜 그 수준으로 설정했는지 2문장으로 설명하세요. 기술적 저항선·상단 밴드·과거 고점 등 근거를 연결하고, 전량 매도가 아닌 단계적 축소를 권하는 이유(불확실성 대응)를 간결하게 덧붙이세요.

**재관심 기준가 (과열 해소 후 재검토)**
- 재관심 구간: __원 ~ __원 (적정주가 __원 기준 ± 안전마진 _%)
  - 조건: 이 구간 도달 + 실적 가시성 개선 확인 필수

> **[재관심 해설 — 필수 작성]** 재관심 구간에서 다시 매수를 검토할 수 있는 조건을 2문장으로 설명하세요. 단순히 가격만 내려온다고 매수가 합리적인 게 아니라, 어떤 펀더멘털·이벤트 조건이 동반돼야 하는지를 명확히 서술하세요.
...
> **[R/R 해설 — 필수 작성]** "현재 위치에서 상방은 +__%에 불과한 반면 하방은 –__%로, 손익비가 __:1로 매수자에게 불리합니다. [핵심 하방 트리거 중 가장 현실적인 1가지]가 현실화될 경우 하단 밴드까지의 추가 하락 가능성도 열려 있어, 지금 신규 진입보다 과열 해소를 기다리는 것이 합리적입니다."
...
> **[해설 — 필수 작성]** 아래 세 가지를 각각 2~3문장의 줄글(running prose)로 작성하세요. 불릿 나열 금지. 독자가 표의 숫자만 봐서는 알 수 없는 맥락과 판단 근거를 전달하는 것이 목적입니다.

**저가 매수 구간 근거**: 왜 __원 ~ __원이 매력적인 진입 구간인지 서술하세요. 기술적 지지선(전저점·이평선)과 안전마진(적정주가 대비 할인율) 근거를 연결하고, 이 구간에서 거래량이 어떻게 나타나야 신뢰도 있는 지지로 볼 수 있는지 덧붙이세요.

**추세 추종 구간 논리**: __원 돌파가 왜 의미 있는 신호인지 서술하세요. 기술적 저항 돌파의 의미, 거래량 확인이 왜 필요한지, 그리고 이미 상승한 시점임에도 추격 진입이 합리적인 조건을 설명하세요.

**손절 기준선 설정 이유**: __원 아래 종가 마감이 왜 손절 신호인지 서술하세요. 해당 가격이 기술적으로 어떤 의미를 가지는지(지지선 붕괴·추세 전환 신호 등)와, 손절 없이 보유 시 발생할 수 있는 리스크를 간결하게 설명하세요.
