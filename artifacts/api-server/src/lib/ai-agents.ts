...
역할: 산업 구조와 경쟁 지형을 꿰뚫어 보고, 이 기업이 그 속에서 어디에 서 있는지를 명확하게 전달합니다.
원칙: 각 섹션은 핵심만 간결하게. 길게 설명하지 않습니다. 한 섹션에 3~5문장이면 충분합니다.
${COMMON_RULES}`,
      userPrompt: `${baseContext}${previousContext}

${companyName}이 속한 산업을 아래 4개 섹션으로 분석하세요. 각 섹션은 핵심만 간결하게, 3~5문장으로 끝내세요. 장황하게 설명하지 마세요.
...
지금 이 산업을 움직이는 가장 중요한 흐름 1~2가지를 짚으세요. 수요 변화, 기술 전환, 정책 충격 등 지금 이 순간 판을 바꾸고 있는 힘.

## ⚔️ 경쟁 구도 — 누가 이기고 누가 지는가

이 산업의 주요 플레이어 구도와 승패를 가르는 핵심 변수를 서술하세요. 진입장벽, 차별화 요소, 지금 강자와 약자가 뒤바뀌는 이유.

## 📍 기업 포지션 — ${companyName}은 어디에 서 있는가

위 분석을 바탕으로 ${companyName}의 현재 위치를 냉정하게 평가하세요. 이 산업의 논리상 이 기업에 유리한 것과 불리한 것을 각각 한두 가지씩 명확히 짚으세요.`,
    },
