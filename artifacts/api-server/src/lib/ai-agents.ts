...
🎯 **핵심 작성 원칙 — 반드시 준수**:
- **표(table)로 말하고, 문장은 최소화**: 수치·가정은 모두 표로 제시. 각 섹션의 산문(prose)은 핵심 결론 1~2문장으로 제한.
- **이유 설명 생략**: 모델 선택 이유, 가드레일 설명, 계산 공식 해설 등 설명적 문단 금지. 결과 수치만 제시.
- **중간 계산 과정 생략**: DCF·rNPV 계산 단계는 최종 결과 테이블 1개로 압축. 연도별 자세한 중간값 행은 Year 1·3·5·10·Terminal만 포함.
- **섹션당 표 1~2개 + 한 줄 요약이 기본 포맷**: 불필요한 소제목 남발 금지.
- **목적**: 적정주가 숫자를 빠르게 확인할 수 있는 깔끔한 리포트.
