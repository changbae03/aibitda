...
한국 주식시장에서는 "가는 종목이 더 간다" — 개인투자자(전체 거래의 60–70%) 주도의 모멘텀이 한번 형성되면 펀더멘털 이상으로 주가가 추가 상승하는 구조적 특성이 있습니다. 피어 멀티플 선택과 Bull 밴드 설정에 이 심리를 반드시 반영하세요.

[모멘텀 강도 판정 — 이전 단계 '한국 시장 심리 진단' 결과를 참조]

- 🔥 강한 모멘텀: 테마 직접 수혜(🎯) + 수명 주기 과열(🔥) + 심리 온도계 탐욕/극단적 탐욕(🟠/🔴) 중 2가지 이상 해당
  → 피어 멀티플 상위 25%(Q3) 적용
  → Bull 밴드 = 가중평균 목표가 × 1.25~1.40 (모멘텀 프리미엄 반영)
  → 보고서에 "강한 모멘텀 감지 — 피어 멀티플 Q3, Bull 밴드 모멘텀 프리미엄 반영" 1줄 명시 필수

- 🌱 초기 모멘텀: 직접/간접 수혜 + 수명 주기 초입(🌱) + 스마트머니 진입(🟢) 해당
  → 피어 멀티플 중앙값~Q3 중간 적용
  → Bull 밴드 = 가중평균 목표가 × 1.15~1.25 (소폭 확장)
  → 보고서에 "초기 모멘텀 형성 — 피어 멀티플 중앙값 상단 적용" 1줄 명시

- 📉 모멘텀 소화·소멸: 수명 주기 소화/소멸(📉/❄️) 또는 테마 편승(❌) + 과열반영
  → 피어 멀티플 중앙값 이하(Q1~중앙값) 적용
  → Bull 밴드 = 기본값 × 1.10~1.15 (확장 없음)
  → 보고서에 "모멘텀 소화 구간 — 피어 멀티플 보수적 적용, 펀더멘털 가치 복귀 중" 1줄 명시

- ⚪ 신호 없음 / USD 종목: 피어 멀티플 중앙값 적용 (기존 방식 유지)

[Bull 밴드 모멘텀 확장 규칙 — 상단 밴드의 한국 시장 현실 반영]
한국 테마주의 단기(1~3개월) 고점은 펀더멘털 적정주가를 30~50% 상회하는 경우가 흔합니다. 이를 Bull 밴드에 반영하세요.
- 강한 모멘텀 + 직접 수혜: ×1.35~1.40 (테마 최고조 구간 반영)
- 강한 모멘텀 + 간접 수혜: ×1.25~1.30 (테마 프리미엄 일부 반영)
- ⛔ Bull 밴드가 현재가를 하회하면: 모멘텀 프리미엄 없이 기본 × 1.15 적용

[모멘텀 프리미엄 소멸 리스크 — Bear 시나리오 반드시 반영]
모멘텀 프리미엄은 단기(1~6개월) 성격입니다. Bear 시나리오에 반드시 포함하세요:
- 셀온뉴스 위험도 高 → Bear 시나리오 확률을 +5~10%p 상향 조정
- 테마 수명 주기 과열(🔥) → Bear 시나리오에 "테마 소멸 시 멀티플 급격 수축 → 피어 Q1 이하 적용 가능" 명시
- 모멘텀 프리미엄 반영 시 Bear 시나리오 서술에 반드시 한 문장 추가:
  "테마 소멸·수급 이탈이 겹치면 모멘텀 프리미엄이 빠르게 사라지며, 이 경우 [Base 목표가의 60~80% 수준]까지 단기 급락 위험이 있습니다."

[보고서 기재 원칙]
⚠️ 모멘텀 판정을 반드시 한 줄 명시: "모멘텀 강도: [강함/초기/소화·소멸/없음] — 피어 멀티플 [Q3/중앙값 상단/중앙값 이하/중앙값] 적용"
⚠️ Bull 밴드 계수가 기본(×1.15)과 다른 경우: "Bull 밴드는 모멘텀 프리미엄 반영으로 ×[X.XX] 적용" 을 밴드 표 하단에 주석으로 명시
⛔ 모멘텀 근거 없이 Bull 밴드를 × 1.15 초과로 설정하는 것은 금지

═══ 한국 시장 특화 모듈 6 — 코리아 디스카운트/프리미엄 체계화 (KRW 종목 필수) ═══

한국 주식은 글로벌 피어 대비 구조적 할인(코리아 디스카운트)을 받아왔으나, 특정 섹터는 오히려 글로벌 프리미엄을 받습니다. 피어 비교 시 이를 반드시 반영하세요.

[코리아 디스카운트 구조]
코스피·코스닥 전체: 글로벌 동종 피어 대비 평균 20~30% PER 할인 (주요 원인: 지정학적 리스크, 지배구조 불투명, 낮은 주주환원)
⚠️ 글로벌 피어 배수를 한국 종목에 직접 적용할 경우: 아래 디스카운트/프리미엄 기준을 반드시 적용하세요.

[섹터별 코리아 디스카운트/프리미엄 기준]
...
| K-플랫폼·IT (카카오·네이버) | -20~-30% 디스카운트 | 규제 리스크, 지배구조, 글로벌 확장성 제한 |
| K-뷰티·소비재 (LG생활건강·아모레 등) | -5~-15% 디스카운트 | 중국 의존도 리스크, 브랜드 글로벌화 진행 중 |
| K-엔터·콘텐츠 (하이브·SM·CJ ENM) | 0~+20% 프리미엄 | K-팝 글로벌 팬덤 프리미엄, IP 자산 가치 |
| 자동차·부품 (현대차·기아) | -10~-20% 디스카운트 | 지배구조 할인, 사이클 변동성 |
| 금융·은행 | -20~-30% 디스카운트 | 低 주주환원, 지정학 리스크, NIM 압박 |

[적용 원칙]
1. 피어 비교에 글로벌 동종기업을 사용하는 경우: 위 디스카운트/프리미엄 기준을 적용한 조정 배수를 사용하세요.
   예: 글로벌 방산 PER 평균 23x → K-방산 적용 배수 = 23x × (1 + 0.15~0.25) = 26~29x
2. 국내 피어만 비교하는 경우: 코리아 디스카운트 추가 적용 불필요 (이미 동일 할인 적용됨)
3. 국내·글로벌 피어 혼용 시: 글로벌 피어에만 디스카운트 조정 적용, 국내 피어는 무조정

⚠️ 보고서에 반드시 한 줄 명시: "코리아 [디스카운트/프리미엄] 적용: 글로벌 피어 [X]x → 조정 배수 [Y]x ([+/-Z%] 적용)"

[코리아 디스카운트 해소 트리거 — 목표주가 상방 요인]
아래 조건 중 하나 이상 해당하면 Bull 시나리오에 "코리아 디스카운트 해소 시나리오" 를 추가하고 목표가에 반영하세요:
- 주요 글로벌 기관(MSCI EM 비중 확대)의 한국 비중 확대 공시
- 지배구조 개선 (이사회 독립성 강화, 자사주 소각, 밸류업 공시)
...
| 구분 | 내용 |
|------|------|
| 장기 추세 (52주 밴드) | 52주 최고 __원 / 최저 __원, 현재 위치 __% (52주 밴드 내 상대적 위치) |
| 중기 추세 (주봉) | 20주선/60주선 데이터가 있으면 지지·저항 여부 서술. 없으면 52주 밴드 내 위치·최근 1~3개월 가격 방향·단기 추세 맥락을 근거로 "상승 흐름 유지 중" / "조정 구간 진행 중" / "횡보 구간" 중 하나를 선택하고 1문장으로 서술. ⛔ "데이터 미제공" 단독 기재 금지. |
...
**[KRW 종목 전용 — 한국 시장 특화 기술 패턴]** currency=KRW인 경우에만 아래 항목을 기술적 분석 말미에 추가하세요. currency=USD이면 생략하세요.

- 📈 단기 급등 패턴 확인: 52주 밴드 내 현재 위치가 85% 이상(52주 고점 근접)이거나 최근 1개월 수익률이 +20% 이상이면 "단기 급등 구간 — 추격 매수 자제, 눌림목 진입 전략 권장"으로 명시하세요.
- 🔥 상한가·연속 급등 패턴: 컨텍스트에 상한가(+29~30%) 또는 2거래일 이상 연속 5% 초과 상승 기록이 있으면, 상한가 다음날 패턴(상승 지속 vs 갭 하락 후 되돌림)을 1문장으로 명시하고 "상한가 이후 단기 변동성 확대 구간"임을 경고하세요.
- 📊 거래대금 신호: KIS 거래대금 추세 데이터가 있으면, 거래대금이 평소 대비 3배 이상 급증한 경우 "테마 과열 신호 — 거래대금 급증 후 급락 전환 위험"을 명시하세요. 거래대금이 급감 중이면 "관심 이탈 — 모멘텀 소화 구간"으로 표기하세요.
- ⚡ 개인 수급 기술 신호: 수급 섹션의 개인 순매수·순매도 방향을 기술적 신호로 재해석하세요. 개인이 연속 순매수 중이면 "개인 추격 매수 구간 — 단기 과매수 가능성", 개인 순매도 전환이면 "개인 차익실현 신호 — 단기 저항 구간"으로 명시하세요.
...
| 52주 최고가 / 최저가 | |
| 52주 밴드 위치 (하단 기준 %) | |
| 현재 PER / 추정 PER | 현재 PER = 현재가 ÷ 최근 확정 EPS. 추정 PER = 현재가 ÷ 직전 단계(실적 전망) CHAIN-HANDOFF에서 인계된 **해당연도E EPS** (⛔ 독자적으로 EPS 재산출 또는 다른 출처 사용 금지 — 리포트 내 EPS 수치 불일치 유발) |
| PBR | |
...
Base upside가 –10% 미만이면 [A. 매도·재진입 전략]을 작성하고, –10% 이상이면 [B. 매수 진입 전략]을 작성하세요. 선택하지 않은 섹션은 완전히 생략하세요.
※ 지시사항 끝.

---

### [A. 매도·재진입 전략] ← Base upside < –10%일 때만 작성

섹션 첫 줄에 현재 상황을 1~2문장 줄글로 요약하세요. "현재가는 적정주가 대비 X% 과대평가 — 기보유자는 단계적 차익실현, 미보유자는 진입 금지" 형식.

**📡 핵심 매매 신호 (3줄 요약)**

| 신호 | 조건 | 행동 |
|------|------|------|
| 🔴 매도·비중 축소 | 현재가 __원 이상 유지 / 적정주가(__원) 대비 __% 과대평가 | 보유 물량의 __%p 단계적 축소 |
| 📈 재진입 검토 | 주가 __원 ~ __원 도달 + [핵심 촉매 조건 1가지] | 소량 분할 매수 재개 |
| 🛑 손절·완전 청산 | __원 아래 종가 마감 또는 [악재 이벤트 조건] | 전량 청산 |
...
| 하단 밴드(Bear) | __원 | 적정주가 대비 추가 __% 하락 가능 |
...
재진입 조건을 **가격 조건**과 **펀더멘털 조건** 두 가지로 나눠 각각 1~2문장으로 서술하세요.

- **가격 조건**: 어느 가격대까지 내려왔을 때 기술적으로 매력적인지(지지선·이평선 기준)
- **펀더멘털 조건**: 단순 하락만으로는 부족하고, 어떤 이벤트·실적·밸류에이션 회복이 동반돼야 하는지
...
섹션 첫 줄에 1~2문장 평문 요약을 작성하세요. 예시: "현재 주가는 적정주가 대비 충분한 상승여력을 보유해 분할 매수 전략이 유효합니다. 조정 시 저가 매수 구간 진입과 저항 돌파 후 추세 추종 두 경로를 제시합니다."

**📡 핵심 매매 신호 (3줄 요약)**

| 신호 | 조건 | 행동 |
|------|------|------|
| 🟢 진입 | __원 ~ __원 도달 + 거래량 감소 후 반등 확인 | 분할 매수 시작 |
| 🎯 1차 매도 | __원 (+__%) 도달 | 보유 물량의 50% 차익실현 |
| 🛑 손절 | __원 아래 종가 마감 | 전량 손절 |
...
| 🎯 1차 목표가 | __원 | +_% | 적정주가(Base) — 절반 차익실현 |
| 🎯 2차 목표가 | __원 | +_% | 상단 밴드(Bull) — 잔여 물량 정리 |
| 🔴 손절 기준선 | __원 | –_% | 종가 기준 이탈 시 전량 손절 |
...
한국 주식시장은 개인투자자 거래 비중이 60–70%에 달하고 테마·뉴스·심리가 단기 주가를 지배합니다. 펀더멘털 분석만으로는 단기 흐름을 설명하기 어려운 구조적 특성이 있습니다. 아래 4가지 항목을 각각 1–2문장으로 간결하게 진단하세요.

### 테마주 여부

뉴스 컨텍스트의 키워드와 최근 주가 흐름을 근거로 이 종목이 현재 시장 테마(우주항공, K-방산, AI·로봇, 2차전지, 원자력·SMR, 바이오, 반도체 소부장, 수소, 조선 등)에 연결되어 움직이고 있는지 판단하세요. 아래 중 하나를 선택하고 1문장 근거를 제시하세요.

- 🎯 직접 수혜: 해당 산업·기술을 직접 영위 — 테마 프리미엄이 펀더멘털로 뒷받침 가능
- 🔗 간접 연관: 관련 산업이지만 실제 매출 기여 제한적 — 테마 소멸 시 조정 위험 있음
- ❌ 테마 편승: 연관성이 낮고 심리·수급만으로 상승 — 이슈 소멸 시 원위치 가능성 高

### 테마 수명 주기

뉴스 빈도, 거래대금 추이, 수급 흐름을 종합해 이 테마/이슈가 어느 단계에 있는지 판정하고 핵심 근거를 1문장으로 제시하세요.

- 🌱 초입: 이슈 부각 초기, 거래대금 아직 낮음, 기관·외국인 미진입
- 🔥 과열: 거래대금 급등, 개인 쏠림, 언론 집중, 52주 신고가 근접 또는 돌파
- 📉 소화: 고점 대비 조정 중, 거래대금 감소, 기관·외국인 이탈 시작
- ❄️ 소멸: 테마 뉴스 단절, 개인 매도 우위, 기본 밸류에이션으로 복귀 중

### 셀온뉴스(Sell-on-News) 위험도

핵심 이슈가 공시·발표·뉴스로 확정되는 시점에 차익실현 매물이 출회할 가능성을 평가하세요. **高 / 中 / 低** 중 하나 + 근거 1문장.

- 高: 이슈가 이미 주가에 충분히 반영됨 — 확정 발표 후 재료 소멸 급락 위험
- 中: 이슈 일부 반영 — 발표 내용에 따라 등락 혼재
- 低: 이슈가 아직 미반영 또는 연속 촉매 존재 — 발표 후 추가 상승 여력

### 투자자 심리 온도계

최근 5일 수급 데이터(외국인·기관·개인 순매수)를 기반으로 아래 중 하나를 선택하고, 단기 변동성 함의를 1문장으로 추가하세요.

- 🔴 극단적 탐욕: 개인 순매수 폭발 + 외국인·기관 동시 이탈 → 급등 후 급락 패턴 경계
- 🟠 탐욕: 개인 주도 매수 + 기관 소규모 참여 → 테마 지속 시 상승, 뉴스 소멸 시 변동성 확대
- 🟡 중립: 3주체 혼조 또는 소규모 거래 → 방향성 확인 후 진입 권장
- 🟢 스마트머니 진입: 외국인·기관 순매수 우위 + 개인 매도 → 구조적 상승 신호
...
한국 주식 시장에서는 펀더멘털 이외에 수급·테마·투자자 심리가 단기~중기 주가에 유의미한 영향을 줍니다.
앞 단계 수급 동향 및 한국 시장 심리 진단 데이터를 바탕으로 Base target_price에 소폭 보정을 적용하세요.

보정 기준 — 수급 신호 (가장 강한 신호 1개 선택):
...
추가 보정 — 한국 시장 심리 진단 연동 (수급 보정에 중첩 적용, 합산 상한 ±7%):
- 촉매 분석 단계에서 "테마 편승(❌)" + "🔴 과열반영" + "셀온뉴스 高" 판정 시: 추가 -2~3% (테마 거품 반영)
- 촉매 분석 단계에서 "직접 수혜(🎯)" + "🌱 초입" + "과소반영" 판정 시: 추가 +2~3% (테마 프리미엄 인정)
- 촉매 분석 단계에서 "🔴 극단적 탐욕" 심리 온도계 판정 시: 추가 -1~2% (과매수 조정 반영)
- 촉매 분석 단계에서 "🟢 스마트머니 진입" 판정 시: 추가 +1~2% (구조적 상승 신뢰)
- 위 조건 미해당 또는 신호 불명확: 추가 보정 없음

보정 규칙:
- 수급 보정 + 심리 보정 합산 ±7% 이내로 제한, 보정 후 100원 단위 반올림
- 보정 결과는 target_price_rationale의 마지막 문장에 자연스러운 문장으로 녹여 쓰세요.
  예) "수급 흐름상 기관·외국인 동반 매수세가 확인되어 목표가를 소폭 상향 조정했습니다."
  예) "테마 편승 성격과 과열반영 신호를 반영하여 목표가를 보수적으로 조정했습니다."
  ⛔ "수급·심리 보정: +N%" 또는 "STEP B" 같은 내부 표현을 그대로 출력하는 것은 절대 금지합니다.

또한, 한국 주식 투자 기간(investment_period) 결정 시 테마주 수명 주기를 반드시 반영하세요:
- 테마 "🔥 과열" 또는 "📉 소화" 단계 판정 → investment_period "단기" 우선 고려
- 테마 "🌱 초입" 단계 + 직접 수혜 → investment_period "중기" 또는 "장기" 적용 가능
