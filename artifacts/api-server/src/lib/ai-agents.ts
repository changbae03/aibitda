...
| 사업부문 | ${new Date().getFullYear()-1}A | YoY | ${new Date().getFullYear()}E | YoY | ${new Date().getFullYear()+1}E | YoY |
|---------|------:|-----:|------:|-----:|------:|-----:|
| [부문A] | | | | | | |
| [부문B] | | | | | | |
| [부문C] | | | | | | |
...
| 사업부문 | ${new Date().getFullYear()-1}A OP | OPM | ${new Date().getFullYear()}E OP | OPM | ${new Date().getFullYear()+1}E OP | OPM |
|---------|------:|-----:|------:|-----:|------:|-----:|
| [부문A] | | | | | | |
| [부문B] | | | | | | |
| [부문C] | | | | | | |
...
- Base upside = (적정주가 - 현재가) / 현재가 × 100 = __%

⚠️ **⛔ SELL / STRONG SELL 여부를 먼저 판정하세요 (Base upside 기준)**
- Base upside **–10% 미만**(Sell 또는 Strong Sell)이면 → **[매도 대응 전략]** 섹션을 작성하세요 (매수 진입 전략 작성 금지)
- Base upside **–10% 이상**(Hold/Buy/Strong Buy)이면 → **[매수 진입 전략]** 섹션을 작성하세요

---

### ⚠️ [매도 대응 전략] — Sell / Strong Sell 판정 시 작성

**현재 상황 진단**
- 현재가(__원)는 적정주가(__원) 대비 __%**과열(과대평가)** 상태
- 상단 밴드(__원)도 이미 __% 상회 — 밸류에이션 전방위 과열

**보유자 대응 전략 (단계적 차익실현)**
- **1차 차익실현 기준**: __원 (현재 수준 / 상단 밴드 수준) — 현재가 기준 보유 비중 __% 축소 고려
  - 근거: 적정주가 대비 __% 프리미엄, 기술적 과매수 (RSI __ 이상 / 이평선 이격 __%)
- **2차 차익실현 기준**: __원 (추가 상승 시) — 추가 __% 축소
  - 근거: 단기 모멘텀 추가 과열 구간

**신규 진입 자제 구간 & 재관심 기준**
- ⛔ **신규 매수 자제**: 현재가(__원)는 과열 구간 — 신규 롱 진입 시 하락 리스크 過大
- **재관심 기준가**: __원 ~ __원 (적정주가 __원 ± 안전마진 _% 수준)
  - 이 구간 도달 + 실적 가시성 개선 확인 후 재검토 권고

**과열 해소 시나리오별 하방 경로**
- Base 하락 경로: 현재가 → 적정주가(__원) — 하락 여지 __%
- Bear 심화 시: 하단 밴드(__원)까지 추가 하락 가능성 (현재가 대비 –_%)
- 핵심 하방 트리거: [실적 미스 / 금리 상승 / 모멘텀 소멸 등 구체적 조건]

**R/R (리스크·리워드 역전 분석)**
- 상방 여지: +_% (현재가→Bull 상단 밴드)
- 하방 리스크: –_% (현재가→Bear 하단 밴드)
- 현 시점 R/R = 상방:하방 = _:_ → **하방 압력이 우세한 비대칭 구조**

---

### [매수 진입 전략] — Hold / Buy / Strong Buy 판정 시 작성

**현재가와 진입 구간의 관계를 먼저 판단하여 아래 케이스 중 해당하는 것을 선택하세요.**
...
- ⛔ 현재가 아래에 있는 구간을 "앞으로 이 구간에 도달해야 진입"하는 것처럼 쓰는 것 금지
...
- **추세 추종 진입 구간**: __원 ~ __원 (현재가 대비 +%~+% — 상방 돌파 확인 후 진입)
  - ⚠️ "보수적 진입" 명칭 금지: 현재가보다 높은 구간은 "추세 추종(모멘텀) 진입"
  - 근거: 핵심 저항선 __원 돌파 + 이평선 정배열 전환

**[케이스 C] 현재가 > 상단 밴드이나 upside가 –10% 미만이 아님 (소폭 과열)**
- "현재가(__원)가 상단 밴드(__원) 상회 — 신규 진입 시 단기 과열 리스크. 보유 중이면 일부 차익 실현 검토."
- 재진입 참고 기준: 상단 밴드(__원) 지지 확인 후 재접근

**리스크 관리 기준선 (참고용)**

⚠️ 아래 수치는 투자 권유가 아닌 리스크 시나리오 참고 분석입니다.

- 1차 하방 지지 구간: __원 ~ __원 (근거: 기술적 지지선)
- 핵심 하방 기준선: __원 (근거: 하단 밴드 vs 중요 지지선 중 보수적 값)
- 리스크 재검토 기준: 종가 기준 __원 하회 시
...
- R/R = (1차 목표가 - 진입가) / (진입가 - 최종 손절가)
- 예) 진입가 180,000원 / 목표가 283,000원 / 손절가 170,000원 → R/R = 10.3:1
- ⛔ 현재가 기준 R/R과 진입 구간이 다르면 진입 구간 중간값 기준으로 통일
- 리스크/리턴 비율(R/R): __:1
