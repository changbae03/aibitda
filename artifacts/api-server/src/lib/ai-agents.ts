...
실적 전망 인계 요약의 Base 수치를 기반으로 DCF 가정을 수립합니다.

**WACC 산출:** Rf __%  |  β __  |  ERP __%  →  CoE __%  |  CoD(after-tax) __%  |  E%/D% __/__  →  **WACC __%**

ROIC 분석: 최근 3년 평균 ROIC __%, Forward ROIC __%로 WACC 대비 스프레드 __%p. ROIC > WACC 여부와 밸류에이션 프리미엄 정당화 여부 한 문장.
...
위에서 수립한 WACC, Terminal g, Sales-to-Capital, 매출 CAGR을 사용하세요.
Base 추정 적용 10년 FCFF. 발행주식수는 인계 요약에서 직접 인용.
⚠️ 단위: 인계 요약의 단위를 그대로 사용 (예: 억원 → 모든 컬럼 헤더에 "(억원)" 표기)
...
주주가치 = 기업가치 합계 + 순현금 = __(단위) | **주당 내재가치 = __원/주** (단위 변환 명시)
⚠️ 수치 정합성 검증 후 출력

### 🔄 Reverse DCF — 현재 주가가 말하는 것

3문단 줄글. 다모다란 방식으로 현재 주가가 내포하는 암묵적 성장 기대를 역산합니다.

첫째: WACC 고정 후 현재 주가 정당화에 필요한 매출 CAGR·Terminal ROIC·영업이익률을 역산해 Base 가정 대비 비교.
둘째: "시장은 현재 이 기업에 대해 ___을 기대하고 있다" 형식으로 달성 가능성 판단.
셋째: 가장 도전적인 가정을 특정하고 upside/downside 비대칭 서술. 안전마진 여부 결론.
