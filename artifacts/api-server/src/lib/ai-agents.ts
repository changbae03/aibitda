...
Q3. 아래 **P/B-ROE 모델 우선 적용 조건**에 해당하는가?
  ...
  - ROE가 안정적이고 예측 가능한 대형 성숙주 (삼성전자, 현대차, 포스코, SK하이닉스 등 코스피 시총 상위)
...
═══ DCF 성장률 합리성 가드레일 (비현실적 목표가 핵심 원인 방지) ═══
⚠️ DCF 10년 명시적 기간의 **연간 매출/NOPAT/FCFF 성장률**은 아래 상한을 반드시 준수하세요:
  - Year 1~2: 컨센서스 추정치 우선 (단, 최대 60%p)
  - Year 3~5: 최대 35%
  - Year 6~10: 최대 15%
  - 10년 평균 CAGR: **최대 25%** (이를 초과하면 과성장 가정)

⛔ **성장률 100% 이상 감지 규칙**: 컨텍스트에 매출 성장률이 100%를 초과하는 수치가 있으면:
  - 이는 저점 대비 기저효과(전년 적자→흑자 전환) 또는 데이터 오환산 가능성이 높습니다.
  - 실제 매출액(조원/억원) 절대치 변화를 직접 계산해 성장률을 재산출하세요.
  - 예: 2025 매출 97조 → 2026 컨센서스 150조 = **+54.4% (올바른 계산)**
  - ⚠️ "228.8% 성장"처럼 그대로 쓰지 말고 반드시 절대치로 역산 검증
...
⚠️ 목표가 ≥ 현재가 × 2 (upside 100% 이상) → 즉시 다음 순서로 내부 재검토 (보고서에 "재검토" 언급 없이 수정된 최종값만 제시):
  ...
  5. **DCF에 사용한 Year 1~5 평균 성장률이 30% 초과**인지 확인 → 초과 시 성장률을 25%로 상한 조정 후 재계산 필수
     - 특히 컨센서스 수치 100%+ 를 그대로 DCF에 사용했는지 확인 (기저효과 착시 주의)
  → 위 5개 항목 점검 후 수정된 목표가를 최종값으로 사용
...
═══ 재투자(Reinvestment) 계산 규칙 — 업종별 차등 적용 ═══

**[범용 공식]**
재투자 = MAX(매출증분÷S-to-C, 전년재투자×0.85, 기업별절대하한)
→ 어떤 업종이든 재투자는 전년 대비 최대 15%만 감소 가능 (0.70 적용 금지)

**[자산집약적 업종 추가 규칙 — 반도체·에너지·통신·철강·화학·조선]**
이 업종은 구조적으로 CAPEX가 증가하거나 유지되는 사이클에 있습니다.
① 컨텍스트에 실제 CAPEX 수치(가이던스 포함)가 있으면 → 해당 금액을 재투자 하한으로 사용
② CAPEX 데이터가 없으면 → 재투자 = MAX(매출증분÷S-to-C, **전년재투자×0.95**, 기업별절대하한)
   (자산집약 업종에서 재투자는 전년 대비 최대 5%만 감소 가능)
③ **AI/HBM·반도체 투자 확장 사이클(2024~2028 예상)**: 재투자는 Year 1~5 동안 감소 금지 — 전년 수준 유지 또는 증가

⛔ **재투자 급감 감지 (필수 점검)**: 전년 대비 재투자가 30% 이상 감소하면:
  → 해당 연도 재투자를 "전년재투자×(업종 하한 배수)"로 강제 조정
  → 보고서에 "⚠️ 재투자 급감 감지 — 업종 특성 반영하여 조정" 1줄 명시
  → 반도체 예시: 2026년 148조 → 2027년은 최소 148×0.95 = 140.6조 이상 유지
...
시나리오 정의:
- **Base-case**: 컨센서스 EPS/EBITDA 수치 그대로 + 서버 계산 WACC 사용
- **Bull-case**: 매출 성장률 Base+2~5%p 상향 + WACC Base−1%p (하한: 8%)
- **Bear-case**: 매출 성장률 Base−3~5%p 하향 + WACC Base+1%p (상한: 14%)
...
**⛔ 재투자 계산 — 단 하나의 공식 (예외 없이 매년 적용):**

    당해연도 재투자 = MAX(
      매출증분 ÷ S-to-C,          ← S-to-C 공식값
      전년재투자 × 0.70,           ← Cascading Floor: 전년 대비 최대 –30% 허용
      기업별 절대 하한              ← 삼성전자 100조 / SK하이닉스 18조 / TSMC USD 25B
    )

**기업별 절대 하한 (어떤 시나리오에서도 이 밑으로 내려갈 수 없음):**
- 삼성전자(005930): **100조원/year**
- SK하이닉스(000660): **18조원/year**
- TSMC(TSM): **USD 250억/year**

**Cascading Floor 연쇄 적용 예시 (삼성전자):**
- 2026년 재투자 = 130조 → 2027년 최솟값 = MAX(S-to-C공식, 130×0.70=91조, **100조**) = MAX(공식, **100조**)
- 2027년 재투자 = 100조 → 2028년 최솟값 = MAX(S-to-C공식, 100×0.70=70조, **100조**) = MAX(공식, **100조**)
  → 절대 하한 100조가 Cascading Floor를 항상 넘으므로 삼성전자는 매년 최소 100조 유지

⛔ **표 작성 전 자가검증 (필수):**
1. 각 연도별: 전년 대비 재투자 감소율 = (당년 − 전년) ÷ 전년 × 100
2. 감소율이 –30% 초과 → MAX 공식 미적용 → 즉시 수정
3. 삼성전자인데 재투자 < 100조 → 절대 하한 미적용 → 즉시 100조로 교체
...
⚠️ 이 기업은 복합기업/지주사로 감지됐습니다. 아래 SOTP 테이블을 반드시 작성하여 사업부별 독립 가치를 합산하세요. 위 DCF·피어 목표가는 이 SOTP 결과와 함께 최종 조율에 반영하세요.
...
SOTP 작성 규칙:
- 상장 자회사: 현재 시가총액 × 지분율로 직접 산정. 별도 배수 불필요
- 비상장 자회사/사업부: 해당 업종 EV/EBITDA 또는 P/E 배수 적용 (위 업종별 벤치마크 참조)
