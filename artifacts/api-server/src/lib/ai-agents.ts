...
- **올해(E)·내년(E+1)·후년(E+2) 3개 연도 추정** — E는 분기 누적 기반(고신뢰), E+1은 P×Q 트렌드 기반(중신뢰), E+2는 성장률 수렴 기반(저신뢰). 신뢰도를 전망 해설에 명시하세요.
...
【STEP CALIBRATE — 백테스트 편향 보정 (직전 분석 앵커가 있을 때만 실행)】

⚠️ 컨텍스트에 "[⛔ ... 직전 분석 ... 실적 전망 앵커]" 블록이 있을 때만 아래 단계를 실행하세요. 없으면 이 단계 전체 생략.

① **직전 예측 수치 추출**: 직전 분석의 "[직전 추정 재무 모델]" 또는 "[직전 CHAIN-HANDOFF 핵심 지표]"에서
   - 직전 분석 당시 올해E(= 현재 기준 전년도 또는 최근 완성 연도)의 예측 매출·영업이익·EPS를 읽으세요.

② **실제 확정 실적과 비교**: 현재 컨텍스트의 "[네이버 연간 실적]" 또는 "[📊 DART 시계열]"의 ★확정★ 데이터에서
   - 같은 연도의 **확정 실적**을 찾아 매출·영업이익 편향률을 계산하세요:
   - 편향률 = (직전 예측 − 확정 실적) ÷ |확정 실적| × 100%

③ **편향 패턴 진단 및 보정 강도 결정**:
   | 편향률 범위 | 해석 | E+1·E+2 보정 |
   |---|---|---|
   | +30% 이상 (낙관 편향) | 이 기업에 대해 구조적으로 과도하게 낙관적인 패턴 | E+1을 −10~20% 하향 조정 고려 |
   | +15%~+30% (경미 낙관) | 주의 필요 | E+1을 −5~10% 조정 또는 하한값 선택 |
   | ±15% 이내 | 정상 범위 — 보정 불필요 | 기존 추정치 유지 |
   | −15%~−30% (경미 비관) | 과도하게 보수적인 패턴 | E+1 상향 조정 검토 |
   | −30% 이하 (비관 편향) | 구조적 과소평가 패턴 | E+1을 +5~15% 상향 조정 고려 |

④ **보정 적용 시 보고서 처리**:
   - 보정이 적용된 경우: 전망 해설 ④ "핵심 가정의 불확실성"에 "예측 정확성 이력을 반영해 [보수적/낙관적] 편향 보정 적용" 1문장 추가 (구체적 수치 직접 노출 금지)
   - 단, 이번 분석에 새로운 구조적 변화(업황 급변·사업부 변화·핵심 고객 이탈 등)가 있으면 과거 편향 보정을 무시할 수 있습니다. 이 경우 1문장 근거 필수.

⑤ **편향 데이터가 부족한 경우**: 직전 예측 연도 확정 실적이 아직 없거나, 직전 분석 앵커가 E(현재진행 중인 연도) 전망이면 → 이 단계 전체 생략.
...
【STEP PQ-NEXT — E+1(내년)·E+2(후년) P×Q 구조 추정】

⚠️ STEP PQ에서 E(올해)를 확정한 후, 아래 방법으로 E+1·E+2를 순서대로 추정하세요.

**E+1 추정 방법 (다음 해):**
① **P 변화** (가격/단가): 업황 사이클·계약 갱신·ASP 추세 → E 대비 ±_% 변화 추정
② **Q 변화** (물량/고객): 수주잔고 커버리지(E+1 만기 물량)·시장점유율 변화·신제품 기여 → E 대비 ±_% 변화 추정
③ **E+1 매출** = E 매출 × (1 + P 변화%) × (1 + Q 변화%)
④ **E+1 OPM**: 아래 항목에서 방향 판단
   - 규모의 경제: E+1 매출 증가 시 고정비 희석 → OPM +_pp
   - 믹스 변화: 고마진/저마진 사업부 비중 변화 → OPM ±_pp
   - 원가 구조: 재료비·인건비 추세 → OPM ±_pp
⑤ **컨센서스 정합성**: E+1 컨센서스가 있으면 동일한 ±20%/±40% 앵커 규칙 적용 (STEP 0 ⑤ 동일 기준)

**E+2 추정 방법 (후년) — 성장률 수렴 원칙 적용:**
① **매출 성장률 수렴**: E+1 성장률이 높은 경우(>20%) E+2는 × 0.5~0.7 수렴. E+1 성장률이 낮은 경우(<10%) E+2는 유사 수준 유지.
② **OPM 방향**: E+1에서 이미 OPM 상승 사이클이 완성됐으면 E+2는 안정화(±2pp). 아직 진행 중이면 E+1 방향 지속.
③ **수주·프로젝트 기업**: E+2 수주잔고 추정치(= E+1 잔고 + 신규수주E+1 − 인식E+1)로 매출 커버 계산
④ **E+2 신뢰도 표시**: 반드시 표 E+2E 열 아래 〔성장수렴 추정 — 신뢰도 낮음〕 명시

**STEP R 구조적 편차 → E+1 연동 원칙:**
⛔ E(올해) STEP R에서 서프라이즈율 |≥30%| 이상인 경우, 이것이 구조적 변화(원가 구조 개선, 신규 고객, 가격 상승 등)라면 E+1도 동방향으로 수정하세요:
  - 구조적 긍정 서프라이즈: E+1 매출/OPM 추정을 E 서프라이즈율의 30~50% 수준으로 상향
  - 구조적 부정 서프라이즈: E+1 추정을 하향 (단, 일회성 비용이면 E+1 유지 가능)
  - 일회성 판단 기준: DART 공시에서 "일회성" 명시 또는 해당 분기에만 존재하는 이벤트
...
- Base EPS (올해E / 내년E / 후년E): __원 / __원 / __원
- Base 매출 (올해E / 내년E / 후년E): __ / __ / __ (단위 명시)
- Base EBITDA (올해E / 내년E): __ / __ (단위 명시)
- Base 영업이익률 (올해E / 내년E): __% / __%
- 후년E 신뢰도: [성장수렴 추정] — DCF에서 Year 3 앵커로만 활용, 직접 목표주가 산출 금지
...
══════════════════════════════════════════════
⛔ STEP 0.5 — DCF 입력 데이터 출처 검증표 (밸류에이션 시작 전 필수 작성)
══════════════════════════════════════════════
밸류에이션 계산을 시작하기 전에, 아래 검증표를 보고서 서두에 반드시 작성하세요.
이 표는 "어떤 숫자를 어디서 가져왔는지" 독자가 확인할 수 있도록 투명하게 공개합니다.

| 항목 | 사용값 | 출처 | 신뢰도 |
|------|--------|------|---------|
| 기준 매출(최근 연간) | __ (단위) | [DART 공시 / FMP API / 이전 단계 인계 / 추정] | [확정/추정/피어기반] |
| 기준 영업이익 | __ | [출처] | [확정/추정] |
| 기준 EPS | __ | [출처] | [확정/추정] |
| WACC | __% | [서버 제공 앵커 / 자체 CAPM 계산 — 계산 근거 명시] | — |
| Terminal g | __% | [한국 장기 GDP 상한 2.0% 이하 적용] | — |
| 발행주식수 | __ (주) | [DART 공시 / FMP / 추정] | [확정/추정] |
| 피어 배수 기준 | __ | [시장 하베스터 실데이터 / 직접 조사 / 추정] | — |

⚠️ 출처가 "추정" 또는 "피어기반"인 항목은 해당 섹션에서 〔데이터 제한〕 태그를 반드시 부착하세요.
⚠️ "DART 확정" 또는 "이전 단계 인계" 항목은 임의로 변경하지 마세요.
⚠️ 발행주식수가 추정치인 경우: 목표주가에 최대 ±10% 오차 가능성을 명시하세요.
이 표를 생략하거나 빈 칸으로 두는 것은 분석 품질 오류로 처리됩니다.
══════════════════════════════════════════════
...
⑤ 수급 강도 (컨텍스트 "네이버 투자자별 순매수" 또는 KIS 데이터 — 최근 5일 방향 기준)
  ⚠️ 이 항목은 적정주가(base)에 직접 영향을 주는 핵심 수급 신호입니다. 데이터가 있으면 반드시 반영하세요.
  - 기관+외국인 동반 순매수 & 외국인 보유비중 증가 추세: ✅ +2점
  - 기관 또는 외국인 중 하나만 순매수 (나머지 중립·소폭): ✅ +1점
  - 수급 혼조 (개인만 매수, 또는 세 주체 모두 소액) 또는 데이터 없음: 0점
  - 기관+외국인 동반 순매도: -1점
  - 기관+외국인 동반 순매도 & 개인 쏠림 순매수 (스마트머니 이탈): -2점

⑥ 시장 전체 수급 컨텍스트 (컨텍스트의 "[📊 시장 수급 컨텍스트]" 섹션 인용 — 없으면 0점)
  ⚠️ 해당 종목이 상장된 시장(KOSPI 또는 KOSDAQ)의 외국인 5일 누적 순매수 방향을 기준으로 판정하세요.
  - 해당 시장 외국인 5일 누적 순매수 양수(+) & 기관도 순매수: ✅ +1점
  - 해당 시장 외국인 5일 누적 순매수 양수(+) 단독: ✅ +0.5점 (반올림 생략, 합산에 포함)
  - 해당 시장 외국인 5일 누적 순매도 음수(-): -1점
  - 데이터 없거나 중립(±): 0점
...
⚠️ KOSDAQ vs KOSPI 수급 감도 보정 — 계수 범위 내 상·하단 선택 기준:
코스닥 종목은 시가총액이 작아 외국인·기관 수급 변화에 대한 주가 민감도가 코스피 대비 1.3~1.5배 높습니다.
이 차이를 계수 범위 선택에 반드시 반영하세요:
- KOSDAQ 종목 + ⑤ 수급 강도 +2점 이상: 해당 등급 계수 범위의 상단 선택
  예) 🔥 강한 모멘텀 직접 수혜 범위 ×1.15~1.25 → ×1.22~1.25 선택
- KOSDAQ 종목 + ⑤ 수급 강도 0점: 해당 등급 계수 범위의 중앙값 선택
- KOSDAQ 종목 + ⑤ 수급 강도 -1점 이하: 해당 등급 계수 범위의 하단 선택
  예) 📉 소화·소멸 범위 ×0.90~0.97 → ×0.90~0.92 선택
- KOSPI 종목: 수급 강도와 무관하게 해당 등급 계수 범위의 중간값 기준 선택
...
SOTP는 **절대가치 산출 방법**입니다. 복합기업은 사업부별 특성이 달라 단일 배수로는
평가할 수 없으므로 SOTP를 절대가치의 Lead로 쓰지만, **상대가치(피어)와 조율하는 절차는
다른 모델과 동일하게 거칩니다.** 시장이 이 회사를 어떻게 보는지도 목표주가에 반영되어야
합니다. SOTP 값 하나로 확정하지 마세요.
...
**[② 상대가치 — 피어 멀티플]**
- 피어 그룹 멀티플(EV/EBITDA·PER·PBR 중 사업 구성에 맞는 것) 기반 목표가: __(현지통화/주)
  ※ 복합기업이라도 매출·이익 비중이 가장 큰 주력 사업의 피어를 기준으로 산출하세요.
    "복합기업이라 비교 불가"로 생략하는 것은 금지입니다.

**[③ 조율]**
- 괴리율: __%

괴리율 20% 이내: 가중평균 (SOTP 60% + 피어 40%) = __ × 0.6 + __ × 0.4 = **__(현지통화)**

괴리율 20% 초과: 아래 형식으로 **한 번만** 작성하세요 (별도 섹션 추가 금지):
1. 괴리 원인 진단: SOTP 입장 1~2문장 + 피어 입장 1~2문장
   (사업부 가치 합이 시장가와 벌어지는 이유 — 지주 디스카운트·성장 프리미엄·유동성 등)
2. 가중치 조정 및 최종 조율가: (SOTP × a% + 피어 × b%) = **최종 목표가 __(현지통화)**
⚠️ **괴리율 30% 초과 시**: 어느 방법론에 더 신뢰를 두는지 논리적 근거를 2~3문장으로 명시 의무.

최종 목표주가(Base): **조율가 __(현지통화)**
Bear/Bull: 조율가를 기준으로 사업부별 실적·배수 시나리오를 반영해 산출
  (SOTP NAV에 할인율 상·하단만 적용하는 단순 방식은 지주회사에만 허용)

⚠️ FINAL_VALUATION_DATA JSON 작성 시:
  - base/bear/bull = **조율 후** 최종 목표주가 (SOTP 단독값이 아님)
  - abs_base/abs_bear/abs_bull = SOTP 절대가치 (지주할인 적용 후)
  - rel_base/rel_bear/rel_bull = 피어 멀티플 기반 상대가치
  - current = 현재 주가
  ✅ abs와 rel은 서로 다른 값이 정상입니다 — 각 방법론의 산출값을 그대로 넣으세요.
  ❌ abs_base·rel_base에 같은 값을 복사해 넣는 것을 금지합니다(조율 과정이 사라집니다).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ KOSDAQ 종목은 수급 민감도가 KOSPI 대비 높으므로 아래 표에서 KOSDAQ 범위를 적용하세요.
아래 수급 신호 중 가장 강한 신호 1개를 선택해 추가 보정:

| 수급 신호 | KOSDAQ 보정 | KOSPI 보정 |
|-----------|-------------|------------|
| 기관+외국인 동시 순매수 & 52주 저점 근처 | +5~8% | +3~5% |
| 기관+외국인 동시 순매수 (중립 위치) | +2~5% | +1~3% |
| 기관+외국인 동시 순매도 & 52주 고점 근처 | -5~8% | -3~5% |
| 기관+외국인 동시 순매도 (중립 위치) | -2~5% | -1~3% |
| 개인만 순매수, 기관+외국인 모두 순매도 | -3~5% | -2~3% |
| 시장 수급 역방향 (종목 수급과 시장 전체 수급 반대) | ±1~2% 추가 가산·차감 | ±0.5~1% 추가 |
| 신호 혼재 / 불명확 | 0% | 0% |

⚠️ 시장 수급 역방향 보정: 컨텍스트 "[📊 시장 수급 컨텍스트]"에서 해당 시장(KOSPI/KOSDAQ) 외국인 방향이 종목 수급과 반대인 경우에만 적용. 예) 종목은 외국인 순매수지만 KOSDAQ 전체는 외국인 순매도 → 신뢰도 하락, 보정폭 상단 제한.
