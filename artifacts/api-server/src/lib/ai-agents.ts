...
📌 **[체인 인계 규칙]** 산업 분析 리포트 맨 앞에 반드시 다음 형식의 인계 선언을 독립된 단락으로 작성하세요:
"브리핑에서 확인된 핵심 이슈 '[이슈명]'을 을 중심으로, ${companyName}의 산업 구조와 경쟁 지형을 분析합니다."
...
단락과 단락 사이에 빈 줄을 넣어 읽기 편하게 구분하세요.

## ⚔️ 경쟁 구도

이 산업의 주요 플레이어 3~5개를 점유율·매출·이익률 수치와 함께 제시하세요. 승패를 가르는 핵심 변수(기술 격차, 원가, 고객 의존도 등)를 구체적 근거와 함께 설명하세요.

강자와 약자의 위치가 바뀌고 있다면 그 이유를 핵심 이슈와 연결하여 서술하세요.
...
【STEP 0 — 촉매-실적 연결 가이드 인계 수령 (최우선)】
바로 앞 단계(투자 촉매 분析)의 "🔗 촉매-실적 연결 가이드" 섹션을 먼저 찾아 Base/Bull/Bear 시나리오별 매출·영업이익·EPS 영향 수치를 확인하세요.
→ 이 수치를 이 단계의 **Base 실적 전망 출발점**으로 사용하세요.
→ 컨센서스와 이 수치가 충돌하면: 컨센서스를 기준으로 하되, 촉매 가이드의 방향성을 전망 근거에 반드시 반영하세요.
→ 촉매 가이드가 없거나 "데이터 미확인"인 항목: STEP A~C의 컨센서스·분기 데이터로 독립 추정.

【STEP A — 컨센서스 앵커 설정 + 수정 방향 확인】
컨텍스트에서 아래 섹션을 먼저 찾아 수치를 기록하세요.
  ① "[EPS 및 매출 전망 (애널리스트 컨센서스)]" — Yahoo 컨센서스 EPS·매출, 서버 계산 DCF Year 0/1/2 앵커
  ② "[네이버 분기 실적]" 또는 "[네이버 분기 EPS]" — 한국 종목 컨센서스
  ③ "[📊 EPS 추정 수정 방향]" — 애널리스트 상향/하향 수정 방향 (있으면 반드시 확인)
  ④ "[📋 실적 서프라이즈 이력]" — 최근 8분기 Beat율 (있으면 반드시 확인)
  ⑤ 컨센서스가 없으면 STEP B로 바로 진행

컨센서스가 있으면: 올해E·내년E 매출·EPS 컨센서스 수치를 먼저 메모하고, 내 추정치가 크게 다를 경우 반드시 그 이유를 전망 근거에 1줄로 명시.
EPS 수정 방향이 있으면: 상향 모멘텀이면 컨센서스를 기준 유지, 하향 모멘텀이면 컨센서스보다 보수적으로 조정 (10~15% 하향) 후 사유 명시.
Beat율이 있으면: 75%↑이면 컨센서스 신뢰도 높음(그대로), 50%↓이면 컨센서스 과낙관(하향 보정).
...
"촉매 分析에서 도출된 핵심 이슈 '[이슈명]'의 재무 영향(Base 시나리오: 영업이익 [±X억원/방향성] 예상)을 기반으로 실적을 전망합니다."
→ 이 문장으로 리포트가 시작되어야 합니다. 소제목 앞에 이 문장을 독립된 단락으로 배치하세요. 촉매 가이드에 수치가 없으면 "방향성 기반"으로 작성하세요.
...
최근 5개년 핵심 재무 지표를 아래 표로 제시하세요. 표 앞뒤에 **결론 문장을 먼저** 작성하세요.
...
⛔ **컬럼 수 엄수**: 표는 정확히 아래 템플릿과 동일한 컬럼 수로 작성하세요. 열 추가 금지.
...
재무 데이터를 불러왔을 때 "영업이익 > 매출" 또는 "OPM > 100%"가 계산되는 경우:
이는 SK스퀘어·GS홀딩스·CJ㈜ 등 순수 투자지주사의 구조적 회계 특성 때문입니다.

  원인: 지분법 이익(지분법 피투자사 순이익 × 지분율)이 연결 영업이익에 합산되는 경우
  회계 기준: IFRS상 지분법 이익은 영업외손익(금융수익 또는 기타손익)에 해당
  → 따라서 OPM > 100%는 데이터 오류가 아니라, 일반 영업이익 프레임으로 표현 불가한 구조

이 경우 아래 지주사 전용 프레임으로 재무 성과 요약을 대체하세요:

  [지주사 재무 프레임]
  - 수익 항목: 배당수입 / 브랜드로열티 / 경영자문수수료 (연결 매출 대신)
  - 비용 항목: 일반관리비(G&A) / 금융비용
  - 영업이익 = 배당수입 등 영업수익 - G&A (지분법 이익은 영업외로 별도 표기)
  - 지분법 이익: 별도 행으로 "영업외 — 지분법이익" 표기. 영업이익에 합산 금지
  - OPM 표기: 연결 기준 OPM 대신 "별도 영업이익률(배당수입 기준) __%" 로 표기
  - 순이익: 지분법이익 포함 연결 기준 인용 가능

  표 맨 위에 다음 주석을 반드시 한 줄 삽입:
  > ※ 순수 투자지주사로 영업이익 = 배당·로열티 기준, 지분법이익(영업외)은 별도 표기. 연결 OPM은 의미 없음.
...
### 🏭 사업부문별 핵심 가정 분해 (복수 사업부문이 있는 경우 필수)

⚠️ 기업이 2개 이상의 사업부문을 보유하면 아래 테이블 **두 개 모두** 반드시 작성하세요.
⚠️ 단일 사업 기업 또는 세그먼트 정보가 전혀 없으면 이 섹션 생략 가능.
⚠️ 숫자는 반드시 **실제 추정 수치**를 기재하세요 (빈칸 또는 "—" 금지). 단위를 테이블 제목에 명시하세요.

---

**① 부문별 매출 추정** (단위: [조원 / 억원 / $M 등 명시])

| 사업부문 | ${new Date().getFullYear()-1}A | ${new Date().getFullYear()}E | ${new Date().getFullYear()+1}E |
|---------|------:|------:|------:|
| [부문A] | | | |
| ↳ YoY | | | |
| [부문B] | | | |
...
**② 부문별 영업이익 및 OPM 추정** (단위: 동일)

| 사업부문 | ${new Date().getFullYear()-1}A OP | ${new Date().getFullYear()}E OP | ${new Date().getFullYear()+1}E OP |
|---------|------:|------:|------:|
| [부문A] | | | |
| ↳ OPM | | | |
| [부문B] | | | |
...
**③ 부문별 핵심 KPI 가정** (각 부문마다 아래 형식으로 작성)

**[부문A명]**
- Volume: [출하량/판매량/출하면적 등 구체적 수치와 YoY]
- Price: [ASP/단가/NIM 등 구체적 수치와 YoY]
- Mix/기타: [제품 믹스 변화, 신제품 기여, 점유율 등]
- 핵심 가정 근거: [1~2줄 — 왜 이 수치인지 핵심 논리]
...
**업종별 KPI 필수 기재 항목 (해당 부문에 적용):**
- 반도체(메모리): Bit Growth(%), ASP YoY(%), 가동률(%), HBM/DDR5 비중(%), OPM(%)
- 반도체(파운드리/비메모리): 가동률(%), 선단노드 비중(%), ASP 변화, 수율 개선 기여
- 스마트폰/MX: 출하량(만대), ASP YoY(%), 폴더블·플래그십 비중(%), OPM(%)
- 가전/CE: 판매량 YoY(%), 프리미엄 비중(%), OPM(%)
- 디스플레이: 출하면적(㎡) YoY, OLED 비중(%), 패널 ASP YoY(%)
- 자동차: 판매량(만대), ASP(만원/대), EV 비중(%), OPM(%)
- 배터리/2차전지: 출하량(GWh), ASP($/kWh) YoY, 에너지밀도, OPM(%)
- 은행/금융: NIM(%), 대출성장률(%), CIR(%), 충당금(bps)
- 통신: ARPU(원) YoY, 가입자 증감, 5G 비중(%), EBITDA 마진(%)
- 건설: 신규 수주(조원), 수주잔고, 해외 비중(%), OPM(%)
- 바이오: 품목별 매출·YoY, 로열티율(%), R&D 비용, 기술이전 계약 가정

**④ 사업부문별 이익 기여도 요약 (3~4줄)**
올해E/내년E 기준 가장 큰 이익 기여 부문, 부문간 Cross-subsidy 여부, 이익 성장의 핵심 드라이버와 리스크를 서술하세요. 수치(OP 기여액, 비중%)를 반드시 포함하세요.
...
✍️ **작성 스타일 원칙 — 반드시 준수**:
- **구조 일관성**: 모든 섹션은 "## 제목 → 핵심 표 → 2~3문장 해석" 순서를 지킵니다. 섹션마다 다른 구조를 쓰지 마세요.
- **수치는 표로, 해석은 문장으로**: 가정·계산 수치는 반드시 마크다운 표로 정리하고, 그 의미(왜 이 수치인지, 결과가 뜻하는 바)는 짧은 문장 2~3개로 설명합니다.
- **쉬운 언어**: 전문 용어(WACC, FCFF 등) 첫 등장 시 괄호 안에 한 줄 설명을 덧붙이세요. 예: "WACC(가중평균자본비용, 투자자가 요구하는 최소 수익률)".
...
[글로벌 피어 멀티플 사용 시]
- 미국·글로벌 피어 배수를 사용하면 한국 시장 구조적 특성(유동성·지배구조·주주환원 차이)을 반영해 10~15% 할인한 배수 적용
- 보고서에는 "시장 구조적 특성을 반영한 할인" 또는 "지배구조·주주환원 기반 조정" 등으로 표현하세요. "코리아 디스카운트"라는 단어는 보고서 출력에 절대 사용하지 마세요.
- 한국 피어(국내 동종업종) 멀티플을 우선 사용하면 추가 할인 불필요
...
⛔ [HARD STOP] 한국 주식 목표가 > 현재가 × 2.5 (upside 150% 초과)
  이 경우는 DCF 계산 오류가 확정적입니다. 다음 순서로 즉시 수정하세요:
  1. WACC를 최소 12%로 상향. WACC < 11%이면 반드시 11~13%로 재계산
  2. g를 1.5% 이하로 하향
  3. Year 3 이후 매출 성장률이 15% 초과인지 확인 → 초과 시 10~12%로 하향
  4. NOPAT이 EBIT × (1-세율)인지 확인 → 세전 영업이익 그대로 사용 시 수정
  5. FCFF/매출 비율이 15%를 초과하는지 확인 → 초과 시 재투자 재계산
  6. Terminal Value 비중이 70% 이하가 되도록 TV를 재계산
  7. 수정 후 목표가가 여전히 현재가 × 2.5 초과이면: 피어 멀티플 결과를 최종값으로 사용
  ⛔ "성장성이 높아 업사이드가 크다"는 이유로 2.5배 초과 목표가를 그대로 유지하는 것은 금지

⛔ [HARD STOP] 한국 주식 목표가 < 현재가 × 0.25 (downside 75% 초과)
  이 경우도 계산 오류가 의심됩니다. 다음을 즉시 점검하세요:
  1. 허가·판매 중인 제품/사업의 DCF가 rNPV에 포함되었는지 확인 (이중 할인 오류 가능)
  2. 발행주식수 단위 오류(주 vs 천주 vs 백만주) 확인
  3. 순현금 부호 오류(순부채로 잘못 처리) 확인
  4. 수정 후에도 -75% 이상 downside이면: PBR 하한선(한국 상장사 최소 0.3x)으로 적정주가 하한 설정
...
Bull 핵심 가정: [상방 드라이버 2~3가지]
Bear 핵심 가정: [하방 리스크 2~3가지]
...
⚠️ 테이블 아래에 한 줄 해석 필수: "WACC ±1%p 변동 시 목표가 범위: X원~Y원, g ±0.5%p 변동 시 범위: A원~B원"
...
📌 **[체인 인계 규칙]** 밸류에이션 리포트 맨 앞에 반드시 다음 형식으로 인계 선언을 작성하세요:
"실적 전망에서 확정된 올해E EPS [X원]·내년E EPS [Y원]을 기반으로 12개월 기준 적정주가를 산출합니다."
→ 이 문장으로 리포트가 시작되어야 합니다. 모델 선택 섹션 앞에 이 문장을 독립된 단락으로 배치하세요. EPS 수치가 없으면 "실적 전망 추정치 기반"으로 작성하세요.
...
| 한국 금융주 (은행·증권·보험·카드) | Gordon Growth P/B — 적정P/B=(ROE−g)/(CoE−g)×수정BPS | 배당이 규제로 억제 → 순수 DDM 과소평가 |
| 미국 금융주 (은행·보험) | DDM 우선, Gordon P/B 보조 | 배당 정책 예측 가능성 높을 때 |
| 부동산/인프라 | NAV | — |
...
### [모델이 Gordon Growth P/B 또는 DDM인 경우 — 이 블록만 작성]
...
## 🔬 PEG & Reverse DCF 검증 (목표가 sanity check)

이 섹션은 DCF·피어 밸류에이션으로 산출한 목표가가 성장 기대치와 논리적으로 일치하는지를 교차 검증합니다.

---

### ① PEG Ratio — 성장주 과대평가 감지

**적용 조건**: 아래 조건을 모두 충족하는 경우에만 작성하세요. 조건 미충족 시 이 항목은 생략하고 "PEG 미적용 — 이유: [사이클주/금융주/적자기업 등]" 한 줄로 대체합니다.
- EPS 성장률이 연 10% 이상으로 추정되는 성장주
- PER 배수가 밸류에이션의 주 지표로 사용된 경우
- 적자 기업, 금융주, 사이클주(반도체·조선·화학), 리츠, 바이오 rNPV 대상은 제외

**PEG 계산:**
- 현재 주가 기준 12개월 Forward PER: __ 배
- 향후 3년 EPS CAGR(실적 전망 인계값): __%
- **PEG = PER ÷ EPS CAGR = __ ÷ __ = __**

**PEG 해석 기준:**

| PEG 구간 | 판정 | 의미 |
|---------|------|------|
| PEG < 1.0 | 🟢 성장 대비 저평가 | 성장률 고려 시 매력적인 진입 구간 |
| 1.0 ≤ PEG < 1.5 | 🟡 적정 평가 | 성장 프리미엄이 합리적으로 반영된 수준 |
| 1.5 ≤ PEG < 2.0 | 🟠 다소 고평가 | 성장 기대가 상당 부분 선반영 |
| PEG ≥ 2.0 | 🔴 고평가 경고 | 현재 주가는 낙관적 성장 시나리오를 과도하게 반영 |

**판정:** [PEG __.x → 구간 판정] — 한 문장으로 근거를 서술하세요.

⚠️ DCF/피어 목표가와 PEG 판정이 충돌하는 경우(예: DCF=매수인데 PEG=고평가 경고):
→ 충돌 원인을 1~2문장으로 명시하고, 어느 방법론에 더 무게를 두는지 판단하세요.
→ 목표가 조정이 필요한 경우 최종 조율 섹션에 반영하세요.

---

### ② Reverse DCF — 현재 주가가 implying하는 성장률 역산

**목적**: "내가 산출한 목표가가 현실적인가"를 역방향으로 검증합니다.
현재 주가를 그대로 DCF에 역대입하여, 이 주가를 정당화하려면 매출·EPS가 몇 년간 몇 %씩 성장해야 하는지를 추정합니다.

**Reverse DCF 계산:**

① 현재 시가총액 = 현재주가 × 발행주식수 = __ (억원 또는 $B)
② 이 시가총액을 정당화하기 위해 필요한 10년 후 순이익 역산:
   - WACC __(%) 적용, Terminal Growth Rate __(%) 가정
   - 역산 10년 후 순이익 = __
③ 역산 10년 CAGR = (역산 순이익 ÷ 현재 순이익)^(1/10) − 1 = __%

**비교:**

| 항목 | 수치 |
|------|------|
| Reverse DCF 역산 성장률 (현재 주가 정당화 필요 CAGR) | _% |
| 실적 전망 Base 성장률 (실적 전망 단계 인계값) | _% |
| 격차 (역산 - 실제 전망) | _% |
...
| 격차 | 판정 |
|------|------|
| 역산 성장률 < 실제 전망 10%p 미만 | 🟢 현재 주가 합리적 — 목표가 논리 일관 |
| 역산 성장률 > 실제 전망 10~20%p | 🟠 현재 주가 다소 고평가 — 목표가 하향 검토 |
| 역산 성장률 > 실제 전망 20%p 이상 | 🔴 현재 주가 심각 고평가 — 목표가 재산출 필요 |
| 역산 성장률 > 실제 전망이 아니라 역산이 더 낮음 | 🟢 현재 주가 저평가 — 목표가 상향 여지 |

**판정:** [역산 CAGR _% vs 실제 전망 _% → 격차 _%] — 목표가 논리가 [일관됨 / 일부 수정 필요 / 재산출 필요]합니다.

⚠️ 계산 불가(순이익 적자, 데이터 부족)인 경우: "Reverse DCF 미적용 — 이유: [적자/데이터 부족]" 한 줄로 대체하고 PEG 검증 결과만 최종 조율에 반영하세요.
⚠️ 🔴 심각 고평가 판정 시: 최종 조율 섹션에서 목표가를 하향 조정하거나, 고평가를 정당화하는 촉매(구조적 성장 전환, 모노폴리 프리미엄 등)를 명시해야 합니다.
...
⚠️ **통화 필수 확인**: 📌 **[체인 인계 규칙]** 기술적 분析 리포트 맨 앞에 반드시 다음 형식의 인계 선언을 독립된 단락으로 작성하세요:
"밸류에이션에서 산출된 적정주가 [X원]을 목표로, 현재 주가의 기술적 위치와 최적 진입 구간을 분析합니다."
→ 이 한 문장을 모든 소제목(##)보다 먼저 배치하세요.
...
⚠️ **수익률 인용 규칙 (필수)**:
- 최근 1개월·3개월 수익률은 **컨텍스트의 [KIS 최근 수익률] 또는 [네이버 최근 수익률] 섹션을 그대로 복사**하여 기입하세요. KIS가 있으면 KIS 우선, 없으면 네이버 사용.
  예: "최근 1개월 수익률: +12.3% (22,000원 → 24,700원)"
- ⛔ 위 두 섹션 중 하나라도 컨텍스트에 존재하면 "데이터 없음"·"데이터 미제공" 기입 절대 금지.
- 두 섹션 모두 컨텍스트에 없는 경우에만 "데이터 미제공"으로 표기.
...
| 52주 최고가 / 최저가 | |
| 52주 밴드 위치 (하단 기준 %) | |
| 최근 1개월 수익률 | [KIS 최근 수익률] 또는 [네이버 최근 수익률] 섹션에서 직접 인용 (KIS 우선) |
| 최근 3개월 수익률 | [KIS 최근 수익률] 또는 [네이버 최근 수익률] 섹션에서 직접 인용 (KIS 우선) |
| 현재 PER / 추정 PER | |
| PBR | |
| 거래대금 추세 (평균 대비) | [KIS 거래대금 추세] 섹션이 있으면 그대로 인용. 없으면 "데이터 미제공" |
| 멀티플 리레이팅 진행 여부 | 위 PBR 규칙 적용 |
| 지배구조 개선·주주환원 확대에 따른 재평가 가능성 | |
...
**현재 상황 진단 (과열 판단)**
- 현재가(__원)는 적정주가(__원) 대비 **__%  과대평가** — 밸류에이션 근거 없는 과열 구간
- 상단 밴드(__원)도 __% 상회, 기술적 과매수 신호 (RSI __ / 이평선 이격 __%)

⛔ **신규 매수 금지 구간**: 현재가는 과열 상태. 신규 롱 진입 시 즉각적 손실 리스크 과대.

**보유자 단계적 차익실현 전략**
- **1차 차익실현**: __원 (현재가 +_% / 기술적 단기 저항) — 보유 비중 __%p 축소
  - 실행 조건: RSI __ 이상 지속 또는 종가 __원 상회 시
- **2차 차익실현**: __원 (상단 밴드 근처 / 추가 과열 시) — 추가 __%p 축소
  - 실행 조건: 1차 도달 후 추가 상승 모멘텀 지속 시

**재관심 기준가 (과열 해소 후 재검토)**
- 재관심 구간: __원 ~ __원 (적정주가 __원 기준 ± 안전마진 _%)
  - 조건: 이 구간 도달 + 실적 가시성 개선 확인 필수

**하방 경로 & 손익비(R/R) 역전 분석**
- Base 하락 경로: 현재가(__원) → 적정주가(__원) — 하락 여지 __%
- Bear 심화 시: 하단 밴드(__원)까지 추가 __%  하락 가능
- 핵심 하방 트리거: [구체적 조건 2~3가지]
- 손익비(R/R) 역전: 상방 +__% vs 하방 –__% → **하방 압력 우세, 신규 진입 비합리적**
...
**저가 매수 구간 진입 중** ← 현재가 ≤ 저가 매수 구간 상단일 때 적용
- "저가 매수 구간(__원 ~ __원)에 **현재 진입 중** — 지금 현재가(__원)가 이 구간에 위치."
- 구간 이탈(하향) 시 손절, 구간 상단 이탈(상향) 시 추세 추종 전략으로 전환

**분할 매수 + 추세 추종 전략** ← 현재가가 저가 구간 위, 추세 추종 구간 아래일 때 적용 (대부분)
- **저가 매수 구간**: __원 ~ __원 (현재가 대비 –_% ~ –_% — 조정 시 매수 구간)
  - 근거: 기술적 지지선 __원 + 적정주가 대비 안전마진 __%
  - 진입 트리거: 거래량 감소 + 5일선 지지 확인 후 반등 시
- **추세 추종 진입 구간**: __원 ~ __원 (현재가 대비 +_% ~ +_% — 상방 돌파 확인 후 진입)
  - 근거: 핵심 저항선 __원 돌파 + 이평선 정배열 전환

**단기 과열 — 신규 진입 자제** ← 현재가 > 상단 밴드 (upside –10% 이상)일 때 적용
- 현재가 상단 밴드 상회 — 신규 진입 시 단기 과열 리스크. 보유자는 일부 차익 실현 검토.
- 재진입 기준: 상단 밴드(__원) 이하로 복귀 후 지지 확인

**리스크 관리 기준선 (참고용)**
- 1차 하방 지지: __원 ~ __원 (근거: 기술적 지지선)
- 핵심 하방 기준선: __원 (하단 밴드 vs 지지선 중 보수적 값)
- 손절 기준: 종가 기준 __원 하회 시
...
**손익비 R/R (진입가 기준 통일)**
- 손익비(R/R) = (1차 목표가 - 진입가) / (진입가 - 손절가) = __:1
- ⛔ 진입 구간과 현재가가 다르면 반드시 진입 구간 중간값 기준으로 통일
...
역할: 지금 이 기업 주가를 움직이는 핵심 이슈·촉매·스마트머니 흐름을 분析합니다. 독자가 분析 결과를 읽고 "앞으로 무슨 일이 언제 일어날지"를 직관적으로 파악할 수 있도록, 구체적인 날짜·수치·조건을 최대한 포함해 서술합니다.
...
⛔ 다른 단계 전담 — 여기서 반복 금지:
- 산업 구조·시장 규모·경쟁사 점유율·정책 환경 설명 → [산업 분析]이 이미 다룸. 촉매와 직결된 새로운 각도가 없으면 반복 금지
- 과거 재무 수치 이력 상세 분析·분기 실적 分析 → [기업 재무 分析] 전담
  ※ 단, 촉매가 매출·영업이익·EPS에 미치는 영향 추정(방향+규모)은 이 단계에서 반드시 다룹니다. 이것이 다음 단계(실적 전망)의 입력값이 됩니다.
- 밸류에이션 모델 적용·목표주가 산출 → [밸류에이션] 전담
${COMMON_RULES}`,
      userPrompt: `${baseContext}${previousContext}

📌 **[체인 인계 규칙]** 촉매 分析 리포트 맨 앞에 반드시 다음 형식으로 인계 선언을 작성하세요:
"산업 分析에서 [핵심 구조적 흐름 1문장]이 확인되었습니다. 이를 배경으로, [핵심 이슈]의 구체적 촉매와 재무 영향을 分析합니다."
→ 이 문장으로 리포트가 시작되어야 합니다. 소제목 앞에 이 문장을 독립된 단락으로 배치하세요.

아래 7개 섹션을 순서대로 작성하세요. 소제목은 이모지 + 제목만 사용하세요.

---

## 🎯 핵심 이슈

⚠️ 핵심 이슈 선정 기준 (작성 전 반드시 자가 검증):
핵심 이슈는 이 기업의 **매출·영업이익·밸류에이션 배수를 직접·구조적으로 바꾸는 요인**이어야 합니다.
- ✅ 올바른 이슈: 제품/기술 경쟁력 변화, 수요 사이클 전환, 핵심 고객·시장 확보, 원가·마진 구조 변화, 경쟁 지형 변화, 규제·정책 패러다임 전환
- ❌ 제외 대상: 노조·파업, 임원 교체, ESG(규제 미연결), 일회성 단기 뉴스
- 건설·인프라: 착공 면적 YoY, 수주 금액, SOC 예산, 미분양 추이를 이슈와 연결

4~5문장 줄글로 작성하세요. 아래 순서로 서술하세요:
1. 핵심 이슈 한 문장으로 명확히 선언 — "지금 이 기업의 주가 방향을 결정하는 핵심 이슈는 [이슈]입니다."
2. 이 이슈가 현재 어느 진행 단계(초기 인식 / 본격 가속 / 성숙 정점)에 있는지 수치 근거와 함께
3. 현재 주가가 이 이슈를 과소반영·적정반영·과대반영 중 어느 수준인지 밸류에이션 수치 근거로 판단
4. 이 이슈가 실현될 때와 실현되지 않을 때 **영업이익에 미치는 영향 범위** (예: "+X억원 ~ +X억원, EPS +X원 효과") 를 한 문장으로 명시
5. 향후 6~12개월 주가에 미칠 영향 한 문장

뉴스 기사 제목 직접 인용 금지. 수치(재무 영향 포함)를 최소 4개 이상 포함하세요.
...
아래 표를 작성하세요. 행은 최소 6개 이상, 최대 9개 이하.
- 시점: "단기", "중기", "장기" 중 하나만 사용
- 이벤트/지표: 구체적 날짜·수치 조건 포함
- 재무 영향: 이 이벤트가 매출·영업이익·EPS에 미치는 예상 변화 (예: "영업이익 +X억", "EPS +X원", "매출 +X%") — 데이터 근거가 없으면 "영향 미미" 또는 "확인 필요"
- 의미: "→ 긍정/부정 신호" 형식

| 시점 | 이벤트 / 확인 지표 | 재무 영향 | 의미 |
|------|------|------|------|
| 단기 | | | |
| 단기 | | | |
| 단기 | | | |
...
### ✅ 긍정 촉매
촉매 2~3개를 불릿으로 작성. 아래 형식을 반드시 준수하세요:
**[발생 조건]** → 재무 영향: [매출/영업이익/EPS 변화 수치] → 주가 영향: [+X~Y%] · 발생 확률: [높음/중간/낮음] · 예상 시점: [분기/연도]

예시: **2025년 하반기 엔비디아 HBM4 공급 계약 체결** → 재무 영향: 연간 HBM 매출 +2.3조원, 영업이익 +6,800억원(EPS +460원) → 주가 영향: +20~30% · 발생 확률: 높음 · 예상 시점: 2025년 3분기

### ⚠️ 역촉매
리스크 2~3개를 불릿으로 작성. 아래 형식을 반드시 준수하세요:
**[구체적 발생 조건]** → 재무 영향: [매출/영업이익/EPS 변화] → 주가 영향: [-X~Y%] · 모니터링 지표: [확인 방법]

---

## 🔗 촉매-실적 연결 가이드

⚠️ 이 섹션은 다음 단계(기업 재무 분析·밸류에이션)가 직접 인계받는 핵심 가이드입니다. 구체적 수치로 작성하세요.

**Base 시나리오 (핵심 이슈 정상 실현):**
- 핵심 촉매 실현 가정: [어떤 조건이 충족되는지 1문장]
- 예상 매출 영향: [+X% 또는 +X억원, 발생 시기]
- 예상 영업이익 영향: [+X% 또는 +X억원, 마진율 변화]
- 밸류에이션 배수 영향: [이 촉매 실현 시 적정 PER/EV/EBITDA 배수가 어느 방향으로 움직일지 — 예: "HBM 독점 유지 시 프리미엄 배수 정당화"]
...
**Bear 시나리오 (촉매 미실현·역풍):**
- 실패 조건: [1문장]
- 예상 재무 영향: [Base 대비 -X%, 핵심 이익 지표 변화]

⚠️ 수치 근거가 없는 항목은 "데이터 미확인 — 방향성만 판단: [상향/하향 압력]"으로 표기하고 임의 수치 작성 금지.
...
반드시 포함: 최근 5일 기관·외국인 순매수 수량과 방향(**굵게**) / 외국인 보유 비중 현재 수준 + 최근 3~6개월 추세 / 구조적 축적인지 단기 트레이딩인지 판단 / 개인투자자 패턴과 함의

**[USD 종목]** currency=USD인 경우:
컨텍스트의 데이터를 활용하여 3~4문장 줄글로 요약하세요.
반드시 포함: **기관 보유 비중** 및 "[주요 기관 투자자 보유 현황 — 13F 최신]"에서 상위 3~4개 기관명과 포지션 증감(**굵게**) / **기관 순매수·순매도 방향** 및 구조적 축적인지 차익실현인지 판단 / **내부자 거래 (SEC Form 4)** 매수·매도 비율과 함의 (데이터 없으면 생략) / **공매도 현황** — 공매도 비중(Float %)·커버일수 인용, 숏 스퀴즈 가능성 판단

---

## 📈 기대감 선반영 지수

이 섹션은 "지금 주가가 핵심 이슈를 얼마나 이미 반영하고 있는가"를 수치로 진단합니다.

아래 순서로 작성하세요:

**① 이벤트 전 주가 상승폭 측정**
핵심 이슈(임상 결과, 계약 발표, 실적 등) 인식 시점 이후 현재까지 주가 상승률을 수치로 제시하세요.
- 컨텍스트의 최근 1개월·3개월·6개월 수익률을 활용하여 "이슈 부각 시점 대비 현재 상승률"을 추정하세요.
- 예: "임상 3상 진입 발표 이후 약 3개월간 주가 +87% 상승"

**② 이벤트 실현 시 기대되는 재무 가치와 비교**
앞서 촉매 분析(촉매 vs 리스크 섹션)에서 산출한 Base 시나리오 주가 영향(%)와 현재 상승폭을 비교하세요.
- 이미 오른 폭 > 예상 촉매 영향: 과대반영
- 이미 오른 폭 ≈ 예상 촉매 영향: 적정반영
- 이미 오른 폭 < 예상 촉매 영향: 과소반영

**③ 선반영 판정 (아래 4단계 중 하나를 선택하고 근거 한 문장 작성)**

| 판정 | 기준 | 의미 |
|------|------|------|
| 🟢 과소반영 | 이벤트 전 상승 < 예상 촉매 영향의 50% | 발표 후에도 추가 상승 여력 존재 |
| 🟡 적정반영 | 이벤트 전 상승 ≈ 예상 촉매 영향 50~100% | 발표 결과에 따라 등락 혼재 |
| 🟠 과대반영 | 이벤트 전 상승 > 예상 촉매 영향 100~150% | 발표 시 차익실현 압력 높음 |
| 🔴 과열반영 | 이벤트 전 상승 > 예상 촉매 영향 150% 이상 | 셀온뉴스 가능성 매우 높음 |

⚠️ 데이터가 부족해 정확한 수치 비교가 어려우면 "정밀 측정 불가 — 방향성 판단: [과소/과대]반영 추정" 형식으로 작성하고 근거를 서술하세요.

---

## 🚨 셀온뉴스 리스크 경보

이 섹션은 대형 이벤트(임상 결과, FDA 승인, 계약 발표, 실적 서프라이즈 등) 발표 전후로 "기대감 매수 → 뉴스 발표 후 차익실현 매도" 패턴이 발생할 리스크를 경보합니다.

**경보 수준 판정 (아래 3가지 신호를 종합해 경보 단계를 결정하세요)**

| 신호 | 내용 |
|------|------|
| 신호 ① 선반영 지수 | 위 섹션에서 판정한 선반영 단계 |
| 신호 ② 수급 방향 | 기관·외국인이 이벤트 임박 시 순매도로 전환했는지 (수급 동향 섹션 참조) |
| 신호 ③ 이벤트 임박도 | 핵심 이벤트 발표 예상 시점이 단기(4주 이내)인지 |

경보 수준 기준:
- 🔴 **높음** — 선반영 과대·과열 + 기관·외국인 순매도 전환 + 이벤트 4주 이내
- 🟠 **중간** — 선반영 과대·과열 OR (적정반영 + 기관·외국인 순매도 전환)
- 🟡 **낮음** — 선반영 과소·적정 + 기관·외국인 순매수 유지
- ⚫ **해당 없음** — 임박한 대형 이벤트가 없거나 이슈가 이벤트형이 아닌 경우

**경보 수준:** 🔴 높음 / 🟠 중간 / 🟡 낮음 / ⚫ 해당 없음 중 하나를 굵게 표기하고, 판단 근거를 2~3문장으로 서술하세요.

**대응 가이드 (경보 수준에 따라 해당 항목만 작성):**

🔴 높음인 경우:
- 이벤트 발표 전: 보유 비중 일부 축소 또는 신규 진입 자제 검토
- 발표 직후: 내용 무관하게 단기 변동성 대비 — 발표 후 첫 거래일 종가 확인 후 재판단
- 재진입 기준: 발표 후 주가가 [기준가]원 수준으로 조정될 경우 재검토

🟠 중간인 경우:
- 신규 진입 시 이벤트 발표 전후 분할 매수 전략 권장
- 발표 결과가 시장 기대치에 부합해도 단기 조정 가능성 열어두기

🟡 낮음인 경우:
- 이벤트 발표가 추가 상승 촉매로 작용할 가능성 존재 — 발표 결과 확인 후 포지션 강화 검토

⚫ 해당 없음인 경우:
- 이 종목에는 단기 이벤트형 셀온뉴스 리스크가 현재 감지되지 않습니다. 펀더멘털 추세 중심으로 판단하세요.`,
    },
...
필드별 작성 기준:
- summary: 아래 체인 순서로 3~4문장을 작성하세요. 반드시 각 단계의 핵심 수치를 포함하세요.
  ① "[핵심 이슈]가 이 기업의 방향을 결정합니다." — catalyst에서 확인된 핵심 이슈 1문장
  ② "이 이슈가 실현될 경우 [영업이익/EPS 영향]이 예상되어, [올해E/내년E EPS X원]으로 전망합니다." — 실적 전망 인계
  ③ "실적을 기반으로 산출한 적정주가는 [X원]으로, 현재가 대비 [Z]% [상승/하락] 여력입니다." — 목표가 인계
  ④ 최종 판정 근거 한 문장.
  ⚠️ Sell/Strong Sell: 매수 권유 문구 절대 금지. "현재가 X원은 적정주가 Y원 대비 Z% 과대평가" 형식으로 시작.
  - ⚠️ **Sell / Strong Sell인 경우**: "현재가 __원은 적정주가 __원 대비 __%  과대평가. __이 정상화될 때까지 신규 진입 자제. 보유자는 __원 수준에서 단계적 차익실현 고려. 재관심 기준가 __원 ~ __원." 형식으로 작성 — 매수 권유 문구 금지.
