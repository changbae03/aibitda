...
위 표의 수치가 나온 배경과 논리를 아래 구조로 상세히 서술하세요. 전망의 근거와 불확실성을 균형 있게 담아, 숫자만 봐서는 알 수 없는 맥락을 설명하는 것이 목적입니다.

**매출 성장 경로**: 올해E·내년E 매출 성장의 핵심 동력을 사업 메커니즘 중심으로 3~4문장으로 서술하세요. 수주 인식 시점, 신규 고객사 확보 경위, 단가·물량 기여도 등 구체적인 수치(억원 또는 %)를 최소 3개 포함하세요. 사업부문이 복수이면 부문별로 기여 구조를 나눠 설명하세요.

**이익률 가정 근거**: 영업이익률을 해당 수준으로 설정한 논리를 2~3문장으로 서술하세요. 원가 구조 변화(재료비·인건비·감가상각), 고정비 레버리지 효과, 제품 믹스 변화 등을 언급하고, 과거 이익률 추세 및 피어 대비 수준과 비교하세요.

**핵심 가정의 불확실성**: 이 전망이 크게 달라질 수 있는 변수를 불릿으로 2~3개 제시하세요. 각 항목은 "조건 변화 → 매출/이익에 미치는 방향과 규모" 형식으로, 막연한 서술 없이 수치 범위(예: ±XX억원, ±XXX원 EPS)를 포함하세요.
...
먼저 밸류에이션 접근법 전체를 2~3문장으로 안내하세요. 아래 형식을 따르세요 (별도 제목 없이 본문으로 시작):

> "[기업명]의 적정주가를 산출하기 위해 [업종] 특성에 적합한 [모델명]을 주력 절대가치 모델로 선택하고, [피어 멀티플 방법론]을 보조 상대가치 모델로 병행합니다. 두 방법론의 결과를 종합·조율하여 12개월 기준 적정주가와 상단·하단 밴드를 도출합니다. [선택 모델이 이 기업에 적합한 핵심 이유를 1문장으로 — 예: 안정적 FCF가 확인되어 현금흐름 할인이 가장 신뢰도 높은 방식이기 때문입니다.]"
