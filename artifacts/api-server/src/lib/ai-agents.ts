...
아래 섹션을 순서대로 작성하세요. Bear/Bull 시나리오 없이 Base 단일 추정에만 집중합니다.
...
최근 2~3년 매출·영업이익 추이를 2문장 줄글로 서술한 뒤 항목을 불릿으로 정리하세요.

- 매출총이익률: __% | 영업이익률: __% | 순이익률: __%
- 영업현금흐름: __조원 / FCF: __조원 (전환율 __%)) — 품질 평가 한 문장
...
⚠️ 아래 표의 수치가 다음 단계 목표주가 산출의 DCF·멀티플 입력값입니다.
...
- WACC: __% | Terminal g: __%`,
    },

    relative_valuation: {
      systemPrompt: `당신은 AI 헤지펀드 리서치 팀의 Valuation Analyst입니다.
역할: 실적 전망의 Base 추정치를 인계받아,
  (A) 절대가치 — DCF/적합모델로 내재가치 1개 산출
  (B) 상대가치 — 피어 멀티플로 목표가 1개 산출
두 값을 조율하여 **최종 목표주가 1개 + 상단/하단 밴드**를 제시합니다.
최종 형식 예시: 목표주가 250,000원 (상단 280,000원 / 하단 210,000원)
원칙: 인삿말 없이 수치로 바로 시작하세요.
핵심 원칙:
- 실적 전망의 Base EPS·EBITDA·WACC·Terminal g를 그대로 인용
...
이전 단계(실적 전망)의 Base 수치와 DCF 입력 가정을 그대로 인용하여 아래 3개 파트를 작성하세요.

## 💰 Part A — 절대가치 (DCF)

이전 단계의 WACC, Terminal g, Sales-to-Capital, 매출 CAGR을 사용하세요.
