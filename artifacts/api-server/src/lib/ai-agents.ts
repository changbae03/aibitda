...
⚠️ 순부채/순현금 계산 규칙 (데이터 우선순위):
  - **1순위**: 컨텍스트의 "[⭐ DART 사업보고서 재무상태표]" 섹션에 "현금및현금성자산"이 있으면 **그 값을 최우선 사용**. 이 값은 DART OpenAPI 원천 데이터로 Yahoo Finance보다 최신·정확.
  - **2순위**: "[⚡ WACC·EBITDA 계산 핵심 데이터]"에 "순부채(Net Debt)" 수치가 명시되어 있으면 그 값을 인용.
  - **3순위**: Yahoo Finance 컨텍스트의 현금성자산 수치.
...
⚠️ S-to-C Ratio 적용 규칙: 위 업종 기준범위에서 선택하고, 근거(최근 capex/매출 비율 등)를 1줄 명시. 근거 없이 3·5 등 임의 정수 설정 금지.
