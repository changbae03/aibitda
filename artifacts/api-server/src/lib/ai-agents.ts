...
### 💹 수익성 분석
| 지표 | ${new Date().getFullYear()-2}(실적) | ${new Date().getFullYear()-1}(실적) | TTM/최근 기준 | 추세 |
|------|------|------|------|------|
| 매출총이익률 (%) | | | | ↑/↓/→ |
| 영업이익률 (%) | | | | ↑/↓/→ |
| EBITDA 마진 (%) | | | | ↑/↓/→ |
...
- **FCF 전환율:** __% (영업CF 대비 FCF 비율 — 70% 이상이면 우량)
- **Reinvestment Rate:** __% (재투자율 = CAPEX / 영업CF) — 자본 집약도 판단
- **FCF 품질 평가:** [우량/보통/주의] — 한 문장 해설 (예: "FCF가 순이익을 초과해 이익의 현금 수반성이 높음" 또는 "CAPEX 급증으로 FCF 압박")

### 🏦 재무건전성
- **부채비율(D/E):** __% | **순부채/순현금:** __조원 | **이자보상배율:** __x
- **유동비율:** __x | **당좌비율:** __x
- **발행주식수:** __주 | **자사주 비율:** __%
- **재무 요약:** [무부채·순현금·레버리지 상태를 한 문장으로 판단 + 투자 관점 시사점]
