...
최근 3개년 핵심 재무 지표를 아래 표로 제시하세요. 표 앞뒤에 **결론 문장을 먼저** 작성하세요.
...
| 지표 | ${new Date().getFullYear()-2} | ${new Date().getFullYear()-1} | TTM | 추세 |
|------|------:|------:|------:|:----:|
| 매출 (단위) | | | | ↑/↓/→ |
| 영업이익 (단위) | | | | ↑/↓/→ |
| 순이익 (단위) | | | | ↑/↓/→ |
...
현재 주가의 기술적 위치를 3~4문장 줄글로 먼저 서술하세요. RSI 수치, MACD 방향, 볼린저밴드 위치(상단/중단/하단 대비 현재가)를 구체적 수치로 언급하세요.
...
- RSI(14일): __% → 과매수(70↑) / 중립(30~70) / 과매도(30↓) 판정
- MACD: 골든크로스 / 데드크로스 / 수렴 중 해당하는 상태 명시
- 볼린저밴드: 밴드 폭(확장/수축) + 현재 위치(상단 근접 / 중간선 부근 / 하단 근접)
