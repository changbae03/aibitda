...
역할: Macro & Industry Analyst가 파악한 산업 구도와 ${companyName}의 포지션을 전제로, 지금 이 기업 주가를 움직이는 촉매·이슈·스마트머니 흐름을 분석합니다.
원칙: 각 섹션은 8~10문장으로 충분히 상세하게 작성합니다. 실제 뉴스 헤드라인, 구체적 수치(주가 영향 %, 반영도, 순매수 수량), 체크포인트 날짜·지표를 최소 3개 이상 포함하세요. Macro & Industry Analyst의 산업 분석을 촉매 해석의 맥락 기반으로 반드시 활용하세요.
${COMMON_RULES}`,
      userPrompt: `${baseContext}${previousContext}

Macro & Industry Analyst의 산업 분석 결과를 참고하여 아래 3개 섹션을 순서대로 분석하세요.

각 섹션 작성 기준:
- 분량: 섹션당 8~10문장
- 수치: 주가 영향(%), 반영도 평가, 순매수 주식수, 외국인 소진율 등 구체적 수치를 최소 3개 이상 포함
- 사례: 실제 뉴스 헤드라인 또는 체크포인트 이벤트를 2~3개 이상 명시
- 배경: 해당 수치와 사례가 투자 판단에서 왜 중요한지 1~2문장으로 해석
- 소제목은 이모지 + 제목만 사용하고 부제 설명 텍스트는 붙이지 마세요

## 🎯 최대 이슈 식별

컨텍스트의 '최신 뉴스/공시' 섹션에 있는 뉴스 헤드라인을 먼저 검토하고, 실제 기사 제목을 1~3개 직접 인용하며 핵심 이슈를 도출하세요. Macro & Industry Analyst가 파악한 산업 구도에서 왜 이 이슈가 지금 결정적인지 맥락을 연결하세요. 핵심 이슈의 현재 진행 단계(초기/가속/성숙/소멸)와 이슈 해소 예상 시점을 명시하고, 현재 주가가 이 이슈를 얼마나 반영했는지 Bear/Base/Bull 시나리오로 구분하여 아래 표로 정리하세요. 각 시나리오에는 이슈 전개 내용, 주가 영향(%), 반영도 평가를 수치와 함께 기재하세요.
...
표 작성 후, 현재 주가가 세 시나리오 중 어디에 가깝게 형성되어 있는지 판단하고, 시장이 과대·과소·적정 반영 중 어느 상태인지 근거와 함께 서술하세요.

## 📅 이슈 체크포인트 및 촉매 이벤트

이 이슈가 어떻게 전개되는지 확인할 수 있는 구체적 지표·이벤트를 단기(1~3개월)와 중기(3~12개월)로 구분하여 날짜·수치 기반으로 서술하세요. Fundamental & Valuation Analyst가 DCF Bear/Base/Bull 가정을 설정할 때 직접 활용할 수 있도록 정량 지표(실적 발표일, 수주 계약 금액, 수율 목표치 등)를 명시하세요. 이슈가 긍정적으로 전개될 때와 부정적으로 전개될 때 주가 변화 경로를 각각 서술하세요. 최대 이슈 외에 주가에 영향을 줄 수 있는 추가 촉매(단기/중기)와 역촉매 리스크도 구체적 사례와 수치를 들어 분석하세요.

## 💰 수급 동향

컨텍스트의 "네이버 투자자별 순매수" 테이블(최근 5일 외국인·기관·개인 순매수 주식수)과 외국인 소진율을 직접 인용하여 분석하세요. 5일 누적 순매수 방향과 외인 지분율 추이를 근거로 수급 강도를 강/중/약으로 평가하고 매집 또는 이탈 여부를 판단하세요. 스마트머니(기관·외국인)의 포지셔닝 방향을 분석하고, Macro & Industry Analyst가 파악한 기업 포지션(업계 내 경쟁 위치)과 연결하여 이 수급 흐름이 구조적 변화인지 단기 노이즈인지 해석하세요. 주도주 가능성(테마 주도 여부, 모멘텀 지속 가능성)을 수급 데이터에 기반하여 최종 평가하세요.`,
    },
