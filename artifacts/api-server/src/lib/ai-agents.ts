...
⚠️ 순부채: "[⚡ WACC·EBITDA 계산 핵심 데이터]"의 "순부채(Net Debt)" 수치 있으면 직접 인용.
⚠️ 발행주식수 고정: 이 섹션에서 사용한 발행주식수를 문서 전체(실적분석·인계 요약·밸류에이션 주당가치 계산)에서 동일하게 사용. 데이터 소스(예: "최신 공시 기준 13,756,000주") 명시.

- D/E 비율: __% (총부채 ÷ 자기자본, 장부가 기준) | 순현금(+)/순부채(-): __ (단위 명시)
- 유동비율: __x | 당좌비율: __x (데이터 없으면 "—")
- 발행주식수: __주 (출처: [최신 공시 / 재무제표 / defaultKeyStatistics 중 명시])
...
- 발행주식수: __주 ← ⛔ 반드시 재무 건전성 섹션과 동일한 값·출처 사용 (임의 변경 금지)
- BPS(주당순자산): __원 (없으면 총자본 ÷ 발행주식수로 계산)
