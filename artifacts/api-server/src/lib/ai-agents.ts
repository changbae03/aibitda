...
② **촉매 인계 데이터 수령**
   ...
   - "컨센서스 조정" Base 추정치
   - Base 시나리오 핵심 가정 3개 항목

③ **핵심 이슈 영향 정량화 (이 단계의 핵심 작업)**
   촉매 인계 데이터의 "매출 기여 산식"과 "컨센서스 조정" 수치를 출발점으로 하되,
   아래 Bottom-up 검증 순서로 수치를 재확인·정제하세요.

   **[Bottom-up 검증 3단계]**
   Step ③-1 **드라이버 분해**: 이슈가 발생시키는 물량·단가·인식 시점을 각각 특정하세요.
   ...
   Step ③-2 **이익률 임팩트**: 추가 매출이 믹스·원가 구조에 미치는 영향을 %p로 추정하세요.
      고마진 제품 비중 증가 → 영업이익률 +X%p / 초기 수익화 단계 → 고정비 희석 +Y%p 등
   Step ③-3 **컨센서스 기반 최종 조정**: 위 Step ③-1~2의 결과를 STEP A 컨센서스에 가감하여
      최종 전망치를 산출하세요.

④ **조정 전망치 도출 — 반드시 아래 형식으로 명시**
   컨센서스 [매출 X조원] + 핵심 이슈 반영 조정 [+Y억원] = **당사 조정 전망 [Z조원]**
   (이슈 드라이버 산식: [Step ③-1 계산 결과 1줄 요약])
