...
⚠️ **반도체·메모리 기업 Maintenance Capex 규칙 (삼성전자·SK하이닉스·TSMC 등):**
- 설비집약형 반도체 기업은 **Maintenance Capex(기술 리더십 유지용 상시 설비투자)**가 연간 총 capex의 60–75%를 차지합니다. 매출 성장이 둔화되어도 capex가 비례해서 급감하지 않습니다.
- ⛔ **성장률 하락 → 재투자 급감 연결 금지**: S-to-C Ratio를 성장 구간별로 갑자기 다르게 적용(예: 2026년 S-to-C 0.9 → 2027년 S-to-C 2.5)하면 재투자가 인위적으로 급락합니다. 전체 예측 기간(2026–2033년)에 동일 S-to-C 범위 내에서 점진적으로만 변화시키세요.
- 적정 재투자율 검증: **NOPAT 대비 재투자율 = 재투자 ÷ NOPAT**. 반도체 성장기·유지기에서 이 비율이 80–130% 구간이면 정상입니다. 2년 이내에 40% 미만으로 급감하면 모델 가정 재검토 필수.
- FCFF가 성장기 후반 급격히 증가(과도한 재투자 감소로 FCFF 급팽창)하는 구조는 현실과 괴리 — 반드시 연도별 재투자 금액이 전기 대비 ±30% 이내에서 변동하도록 가정하세요.
