...
→ **고배수 경고선 초과 시**: "역사적 밴드 상단 초과 — 사이클 고점 과열 또는 구조적 성장 프리미엄 중 어느 쪽인지" 근거 1~2문장 의무 명시
→ **저배수 기회선 하회 시**: "역사적 밴드 하단 하회 — 사이클 저점 저평가 또는 구조적 할인 중 어느 쪽인지" 근거 1~2문장 의무 명시
→ **밴드 내 적용 시**: 현재 배수가 밴드의 상단·중단·하단 중 어느 구간에 위치하는지 1문장 명시 (예: "현 PER 12x는 역사적 밴드(8~20x) 중단 수준으로 밸류에이션 부담 제한적")
→ 이 밴드는 절대 기준이 아닌 맥락 참고용입니다. 구조적 변화(예: AI 수요 급증, 금리 급변)가 있으면 밴드를 벗어난 배수도 합리적일 수 있으며, 그 경우 근거를 명시하면 됩니다.
...
- 영업이익률·매출·EPS 등 재무 수치 분석·전망 → [기업 재무 분析]이 이미 다룸. 촉매의 재무적 영향(예: "수주 성공 시 매출 +15%")을 1문장으로 언급하는 수준만 허용
...
3~4문장 줄글로 작성하세요. 아래 순서로 서술하세요:
1. 지금 이 기업 주가에 가장 많은 영향을 미치고 있는 핵심 이슈 하나를 첫 문장에 명확히 제시
2. 이 이슈가 현재 어느 진행 단계(초기 인식 / 본격 가속 / 성숙 정점)에 있는지 수치 근거와 함께
3. 현재 주가가 이 이슈를 과소반영·적정반영·과대반영 중 어느 수준인지 밸류에이션 근거로 판단
4. 향후 6~12개월 주가에 미칠 영향 한 문장
...
올바른 핵심 이슈: 제품/기술 경쟁력 변화, 수요 사이클 전환, 핵심 고객·시장 확보, 원가·마진 구조 변화, 경쟁 지형 변화, 규제·정책 패러다임 전환
제외 대상: 노조·파업, 임원 교체, ESG(규제 미연결), 일회성 단기 뉴스

건설·인프라 기업은 착공 면적 YoY, 건설 수주 금액, SOC 예산·착공 일정, 미분양 추이, 건자재 수요 지표를 핵심 이슈와 연결하세요.
...
그 다음, 아래 표를 작성하세요. 행은 최소 6개 이상, 최대 9개 이하로 채우세요.
시점 열은 반드시 "단기", "중기", "장기" 중 하나만 사용하세요 (다른 표현 금지).
이벤트/지표 열에는 구체적 날짜·수치 조건 포함, 의미 열은 "→ 긍정/부정 신호" 형식으로 작성하세요.
...
### ✅ 긍정 촉매
이벤트·조건 2~3개를 불릿으로 작성. 형식: **발생 조건** → 예상 주가 영향 %(예: +10~15%) · 발생 시점 전망

### ⚠️ 역촉매
리스크 2~3개를 불릿으로 작성. 형식: **구체적 발생 조건** → 예상 하락 폭 · 모니터링 방법
...
→ 이 수치를 그대로 JSON 필드에 채웁니다. (단, STEP B 한국 심리 보정이 적용된 경우 보정 후 수치 사용)
→ 만약 위 단계에서 수치를 못 찾겠다면 scenarios의 target_price를 0으로 채우지 말고,
   찾을 수 있는 모든 단서("목표가", "적정가", "밸류에이션", "TP")를 재탐색하세요.
══════════════════════════════════════════

══════════════════════════════════════════
📌 STEP B — [한국 주식 전용] 수급·심리 보정 (6자리 KRX 티커인 경우에만 실행)
══════════════════════════════════════════
미국·글로벌 주식은 이 단계를 건너뜁니다.

한국 주식 시장에서는 펀더멘털 이외에 수급·투자자 심리가 단기~중기 주가에 유의미한 영향을 줍니다.
앞 단계(시장 기술적 분석)의 수급 동향 데이터를 바탕으로 STEP A의 Base target_price에 소폭 보정을 적용하세요.

▶ 보정 기준 (아래 중 가장 강한 신호 1개를 선택, 복합 신호 시 가중 평균):

| 수급·심리 신호 | 보정률 |
|---|---|
| 기관+외국인 동시 순매수 & 52주 저점 대비 현재가 +30% 이내 (저평가 인식 스마트머니 유입) | +3~5% |
| 기관+외국인 동시 순매수 (방향 일치, 52주 위치 중립) | +1~3% |
| 기관+외국인 동시 순매도 & 52주 고점 대비 현재가 -10% 이내 (고점 부담·차익실현 심리) | -3~5% |
| 기관+외국인 동시 순매도 (방향 일치, 52주 위치 중립) | -1~3% |
| 개인만 순매수, 기관+외국인 모두 순매도 (개미 역행 매수·과열 경고) | -2~3% |
| 신호 혼재 / 수급 데이터 불명확 | 0% (보정 없음) |

▶ 보정 규칙:
- 보정은 Base target_price 기준 ±5% 이내로 제한 (과도한 보정 금지)
- 보정 후 target_price는 100원(KRW) 단위로 반올림
- 보정률이 0%이면 STEP A 수치를 그대로 사용 (이 단계를 무시하지 말 것)
- target_price_rationale 마지막 문장에 반드시 아래 형식으로 명시:
  "수급·심리 보정: 기관·외국인 [방향] 신호 반영 [+N% / -N% / 보정 없음] → 최종 목표가 [금액]원"
══════════════════════════════════════════
