...
⚠️ 중요: 이 표의 수치가 이후 DCF CAGR 가정과 멀티플 크로스체크의 EPS·EBITDA 입력값의 원천입니다. 시나리오 간 수치가 논리적으로 일관되어야 합니다.

시나리오를 아래 형식으로 각각 분리해서 출력하세요. 3개 블록을 순서대로 작성합니다.

---

🔴 **Bear Case** — (핵심 전제 한 줄: 예) 수요 회복 지연, 경쟁 심화로 마진 압박 지속)

| 구분 | ${new Date().getFullYear()-1}(실적) | ${new Date().getFullYear()}E | ${new Date().getFullYear()+1}E | ${new Date().getFullYear()+2}E |
|------|------|------|------|------|
| 매출 | | | | |
| 영업이익 | | | | |
| 영업이익률 | | | | |
...
🟡 **Base Case** — (핵심 전제 한 줄: 예) 하반기 수요 회복, 점진적 마진 개선)
...
🔵 **Bull Case** — (핵심 전제 한 줄: 예) 조기 수요 반등 + 신규 계약 확대, 영업레버리지 극대화)
...
최근 3년 평균 ROIC __%, Forward ROIC __%로, WACC __%와 비교 시 스프레드 __%p를 기록한다. ROIC > WACC 여부와 이것이 현재 밸류에이션 프리미엄을 정당화하는지 2~3문장으로 결론만 서술하세요 (계산 과정 불필요).

⚠️ 아래 CAGR 가정은 반드시 위 시나리오별 실적 전망 표에서 역산한 수치를 사용하세요.

| 가정 항목 | Bear | Base | Bull |
|-----------|------|------|------|
| 시나리오 배경 | | | |
| 매출 CAGR 1~3년 (전망 테이블 역산) | | | |
| 매출 CAGR 4~10년 (장기 추정) | | | |
| 영업이익률 안정기 (전망 테이블 기반) | | | |
| 유효세율 | | | |
...
Base Case CAGR·마진 가정(위 표에서 인용)을 적용한 10년 FCFF 추정. 단위는 조원, 발행주식수는 재무 데이터에서 직접 인용.
...
- Bear Case 주당가치: __원 (Bear CAGR·마진 적용) | Bull Case 주당가치: __원 (Bull CAGR·마진 적용)

## 🔄 Reverse DCF — 현재 주가가 말하는 것

테이블 없이 3~4문단 줄글로 작성하세요. 다모다란의 철학에 따라 "주가는 미래 기대의 함수"라는 관점에서, 현재 주가가 내포하는 암묵적 성장 기대를 역산해 서술합니다.

첫 번째 문단: Base Case WACC를 고정한 채 현재 주가를 정당화하는 데 필요한 매출 CAGR, Terminal ROIC, 안정기 영업이익률을 역산한 수치를 명시하고, 이것이 Base Case 가정 대비 얼마나 높거나 낮은지 비교하세요.

두 번째 문단: 역산된 성장 기대치가 해당 기업·산업의 역사적 실적, 경쟁 강도, 구조적 한계 측면에서 달성 가능한 수준인지 판단하세요. "시장은 현재 이 기업에 대해 ___을 기대하고 있다"는 형식으로 시장 심리를 명확히 진단하세요.

세 번째 문단: 역산 가정 중 가장 도전적인 항목을 특정하고, 이것이 충족될 경우와 실패할 경우 주가에 미치는 비대칭적 영향(upside/downside)을 서술하세요. 투자 관점에서 현재 주가가 안전마진을 제공하는지, 아니면 완벽한 실행을 전제하는 가격인지 결론을 내리세요.

## 📐 멀티플 크로스체크

⚠️ Forward P/E의 EPS와 EV/EBITDA의 EBITDA는 반드시 위 시나리오별 실적 전망 표의 해당 시나리오 수치를 사용하세요.

| 방법 | Bear | Base | Bull |
|------|------|------|------|
| Forward P/E (시나리오별 EPS × 목표 멀티플) | | | |
| EV/EBITDA 역산 주가 (시나리오별 EBITDA 기반) | | | |
| DCF 내재가치 (위 DCF 결과 인용) | | | |
| 현재 주가 대비 괴리율 | | | |
...
Bear Case 내재가치와 주요 기술적 지지선을 종합한 진입 전략입니다.

- 매수 진입 구간: __원 ~ __원
- 손절가: __원 (근거: Bear Case 내재가치 __원 / 기술적 지지선 __원 중 보수적 값)
- 1차 목표가: __원 (현재가 대비 +_%)
- 2차 목표가: __원 (현재가 대비 +_%)
- 진입 트리거:
  - (단기 체크포인트 기반 트리거 1)
