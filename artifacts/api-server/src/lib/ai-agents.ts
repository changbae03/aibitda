...
## 6. 운전자본 효율성
주입된 다기간 재무 데이터를 활용해 아래 지표를 계산하고 추이를 분석합니다.
재무 데이터가 없으면 "공시 미확인"으로 표기하고 사업보고서 서술 기준으로만 설명합니다.

**재고자산 회전일수 (DIO)** = 재고자산 ÷ (매출원가 ÷ 365)
- 증가: 수요 둔화·재고 적체 신호 / 감소: 수요 호조·재고 정상화 신호
- 반도체·디스플레이: 재고 주기와 업황 사이클 연동 여부 판단

**매출채권 회전일수 (DSO)** = 매출채권 ÷ (매출액 ÷ 365)
- 증가: 채권 회수 지연·리베이트성 매출 경보 / 감소: 현금 창출력 개선 신호
- 대형 고객 편중 업종에서 DSO 급등은 거래 관계 악화 선행지표

**매입채무 회전일수 (DPO)** = 매입채무 ÷ (매출원가 ÷ 365)
- DPO 급등: 협력사 지급 지연(현금 부족 경보) / 감소: 협상력 약화 또는 재무 여유

**현금전환주기 (CCC)** = DIO + DSO − DPO
- CCC 추이(최근 3~5기)를 표로 제시하고, CCC 확대·축소가 의미하는 바를 해석
- CCC 급등: 운전자본 잠식 — 이익이 나도 현금이 안 들어오는 구조
- CCC 급락: 수요 폭발로 선수금 급증(게임체인저) 또는 공급자 압박 심화 중 하나

**운전자본 부담** = (재고자산 + 매출채권 − 매입채무) ÷ 매출액 (%)
- 동종 업종 대비 이상값(>15~20%) 여부 및 추세 방향 판단
