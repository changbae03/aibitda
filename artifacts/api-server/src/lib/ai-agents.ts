...
【STEP R — 확정 분기 서프라이즈 기반 연간 재조정 (필수)】

⚠️ 이 단계는 STEP PQ/B에서 확정한 연간E를 "실제 확정 분기 실적"으로 검증·수정하는 피드백 루프입니다.
"연간E가 먼저 확정되고, 확정 분기를 뺀 나머지를 기계적으로 낮게 맞추는" 오류를 방지합니다.

① **확정 분기 실적 vs 기대치 비교**

   컨텍스트의 "[📊 DART 시계열]"에서 ★확정★ 표시된 분기(Q1, Q2, Q3 중 최신)를 찾으세요.
   - 확정 분기 실적 (실제): A억원
   - STEP PQ/B에서 도출한 연간E에서 역산한 해당 분기 기대치: B억원
     (= 연간E ÷ 4, 또는 계절성 가중치 적용 시 연간E × 해당 분기 비중)
   - 서프라이즈율 = (A − B) ÷ |B| × 100%

② **서프라이즈 구조성 판단**

   서프라이즈가 +15% 이상(또는 -15% 이하)인 경우 아래 기준으로 "구조적 변화"인지 "일회성"인지 판단하세요:

   [구조적 개선/악화로 보는 경우 — 연간E 재조정 대상]
   - 수주잔고 증가·신규 계약으로 매출 인식 가속
   - 단가(ASP) 인상 또는 물량 증가가 지속될 사유가 있음
   - 원가 구조 개선(원재료 하락, 고마진 제품 믹스 개선)이 연중 지속
   - 전년 동기 대비 YoY 개선이 2분기 이상 연속

   [일회성으로 보는 경우 — 연간E 유지]
   - 특수 프로젝트·계약 일시 인식 (Q1에만 집중)
   - 재고 소진·선제 납품으로 Q1 이후 감소 예정
   - 계절성에 의한 Q1 강세 (예: 동절기 수요)

③ **연간E 재조정 계산**

   구조적 개선/악화로 판단되면 아래 방식으로 연간E를 수정하세요:

   [상향 조정]
   - 지속성 계수 α 설정: 구조적 확신 높음 → α=0.7 / 보통 → α=0.5 / 낮음 → α=0.3
   - 연간E 상향분 = 서프라이즈(A−B) × α × 잔여 분기 수
     예) Q1 서프라이즈 +10억, α=0.6, 잔여 3분기 → 연간E +18억 상향
   - 수정 연간E = 기존 연간E + 연간E 상향분

   [하향 조정]
   - 동일 방식으로 연간E 하향분 계산
   - 단, 확정 분기 합계 하한선은 반드시 준수 (STEP A 규칙)

④ **나머지 분기 재배분**

   수정된 연간E를 기준으로 Q2~Q4(또는 미확정 분기)를 재추정하세요:
   - 단순 균등 배분: (수정 연간E − 확정 분기 합계) ÷ 잔여 분기 수
   - 계절성 반영: 과거 분기별 비중(전년도 Q2/Q3/Q4 비중)을 적용해 배분
   ⛔ 잔여 분기를 "연간E가 이미 확정됐으니 낮게 잡는" 방식 절대 금지

⑤ **재조정 결과 보고서 내 명시 (필수)**

   연간E가 재조정된 경우, 보고서 내 실적 전망 표 직전에 아래 형식으로 한 줄 명시:
   "Q[N] 확정 실적이 초기 기대치 대비 +[서프라이즈율]% 초과 → 구조적 개선으로 판단, 연간E를 [기존]→[수정]억원으로 상향 조정"
   재조정이 없는 경우: 이 줄 생략
...
| 확정 분기가 컨센서스 대비 크게 하회 (±30% 이상) | 확정 실적 기준으로 컨센서스를 하향 조정 후 사용. 이 경우 실적 개선 반전을 가정하지 말 것 |
...
계산 후, 연간계 OPM이 STEP 2에서 결정한 **연간 OPM 앵커 ±3%p** 범위 안에 있는지 확인하세요.
범위를 크게 벗어나면 분기 배분을 조정하세요.
