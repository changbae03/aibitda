...
### 📅 ${new Date().getFullYear()}년 분기별 실적 전망

⛔ **[분기 테이블 작성 규칙 — 반드시 준수]**
1. **확정 분기(★)**: 컨텍스트 "[📊 DART 시계열]"의 "★확정★" 데이터 및 "⛔ [분기별 전망 테이블 — 확정값 직접 사용]" 섹션의 수치를 **그대로** 기입. 절대 수정 불가.
2. **미확정 분기(E)**: 계절성 가중치 적용 후 추정. 건설·인프라는 Q1 약세(20%), Q2 중립(25%), Q3 중립(25%), Q4 강세(30%) 패턴 참고.
3. **연간계**: Q1 + Q2E + Q3E + Q4E 합산. **위 연간 표의 ${new Date().getFullYear()}E 수치와 반드시 일치해야 합니다.** 불일치 시 연간 표를 수정하세요.
4. **데이터 없는 확정 분기**: 컨텍스트에 해당 분기 데이터 없으면 "확정 데이터 없음"으로 표기.
...
⛔ **자기검증 (표 작성 후 필수)**:
- 연간계 영업이익 = Q1(★확정) + Q2E + Q3E + Q4E → 위 연간 표 ${new Date().getFullYear()}E 영업이익과 동일한가?
- Q1 확정 영업이익 > 0인데 연간계 < Q1 확정값이면 즉시 수정하세요. (물리적 불가능)
