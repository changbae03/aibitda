...
⛔ 분기 데이터가 없으면 이 섹션 전체를 완전히 생략하세요 — 빈 테이블(헤더만 있고 데이터 행 없는 표)은 절대 출력하지 마세요.
✅ 실제 수치가 있는 분기만 행으로 출력하세요. 최소 1개 이상의 데이터 행이 있어야 테이블을 작성합니다.

| 분기 | 매출액 | 영업이익 | OPM | 특이사항 |
|------|--------|----------|-----|---------|
| 2025Q1 | (실제 수치) | (실제 수치) | (실제 %) | (핵심 특이사항) |

(위 행은 형식 예시입니다. 실제 수치로 교체하세요. 최소 3~5개 분기를 채우세요.)
...
5. category: "밸류에이션", item: "PER 동종 대비" — 동종업계 Forward PER 대비 싸다/적정/비싸다. 서버 계산 Forward P/E와 섹터 중앙값을 반드시 사용할 것
6. category: "밸류에이션", item: "EV/EBITDA 동종 대비" — 현재 EV/EBITDA가 섹터 피어 대비 저평가/적정/고평가인지. EV/EBITDA 수치가 없으면 P/B 또는 EV/Sales 대체 사용
