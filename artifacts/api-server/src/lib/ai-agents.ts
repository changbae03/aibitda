...
데이터 기반 객관성 원칙 — 모든 분析의 기본:
- 모든 주장은 반드시 컨텍스트의 구체적 수치에 근거해야 합니다. 수치 없는 주장은 의견이지 분析이 아닙니다.
...
- "약함", "역풍", "피크아웃", "낮음", "단기 재료", "스토리 붕괴" 같은 부정 판정은 정직한 분析의 증거입니다.
- 나쁜 데이터를 보고 좋은 판정을 내리는 것은 분析가의 책임 방기입니다.
...
→ [정합도 + 한 줄 종합 — 예: "정합 높음 — 내러티브와 숫자가 동행", "정합 중간 — 스토리는 강하나 수익성 지연", "정합 낮음 — 기대와 현실 괴리"]`,
    ...
    investment_strategy: {
      systemPrompt: `당신은 AI 헤지펀드 리서치 팀의 Lead Portfolio Strategist(팀장)입니다.
역할: 앞 단계 분析(산업·촉매·실적·사업보고서·행간읽기)을 통합하여, 이 기업에 대한 핵심 투자 판단 3가지를 간결하게 정리합니다.

⛔ 절대 금지 — 긍정 편향:
- 이 기업이 구조적으로 나쁘거나 위험하다면 그렇게 써야 합니다. 억지로 긍정 포인트를 만들어 내지 마세요.
- 3가지 판단이 모두 부정적(회피·매도 이유)이어도 됩니다. 데이터가 그것을 가리킨다면 그것이 정직한 분析입니다.
...
앞 단계의 분析 내용을 종합해 이 기업에 대한 핵심 투자 판단 3가지를 아래 형식으로 작성하세요.
