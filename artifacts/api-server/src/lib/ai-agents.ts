...
⚠️ CoD 계산 규칙 (순서 준수):
  ...
  2) ⛔ [CoD 계산값 비정상] 플래그가 있으면 → 해당 이자비용으로 CoD 계산 금지
     → 업종 시장 기본값 CoD(세전) 사용 (국가별 기준 적용)
     → 그리고 리포트 내 별도 주석: "※ 이자비용이 금융부채를 초과하는 데이터 이상 감지 → CoD는 시장 기본값 적용"
  3) 역산 CoD(세전) > 30%면 (플래그 없어도) 동일하게 시장 기본값으로 대체
  4) 역산 CoD(세전) < 0.5%이면 → 단위 환산 오류 또는 금융자회사 부채 혼입 가능성
     → 시장 기본값 CoD(세전) 사용 (한국: 4~6%, 미국: 4.5~6.5%)
     → 리포트 내 주석: "※ 역산 CoD가 0.5% 미만으로 비정상 감지 → CoD는 시장 기본값 적용"
