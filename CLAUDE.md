...
- **확정 분기가 연간 전망을 구속한다** (`lib/valuation/quarter-bridge.ts` · `quarter-audit.ts`).
  분기가 하나씩 확정될수록 연간은 정확해져야 하는데, LLM은 연간E를 먼저 정해놓고 분기를
  끼워 맞춘다. 메디포스트(1132)에는 이런 문장이 있었다 —
  *"Q1 확정 실적이 초기 기대치 대비 +1.2% → 연간E를 기존 743.7억원 → **743.7억원**으로 상향 조정"*.
  같은 숫자를 놓고 조정했다고 쓴 것이다.

  `renderQuarterBridge()`가 역산 순서를 못박고(① 남은 분기 추정 → ② 연간 = 확정 + 남은 합),
  `auditQuarters()`가 서버에서 검산한다 — 분기 합 ≠ 연간계, 확정 분기 값 변경,
  숫자가 그대로인 "조정"을 잡는다. `company_analysis` 단계 QC에 물려 있다.

  > **보고서 코드로 분기를 정렬할 것.** 번호와 시간 순서가 다르다 —
  > 11011(연간)·11012(반기)·11013(1분기)·11014(3분기). `ORDER BY reprt_code`로 정렬해
  > Q1·Q2가 확정된 종목에서 **"최신 확정 분기"로 Q1이 집혔고**, OPM 추세 판단과 권고
  > 범위가 한 분기 뒤진 값으로 돌았다. `sortChronologically()` 하나만 쓴다.
  >
  > **연간 여부도 코드로 가른다.** 라벨이 `2025 FY`와 `2025.12.` 두 가지로 저장돼 있어
  > `endsWith("FY")`로 거르면 후자가 분기로 취급된다 — 연간 수치가 분기 OPM 추세에 섞였다.
  >
  > **적자를 이상값으로 걸러내지 말 것.** OPM 권고 범위가 `filter(v => v > -20)`으로
  > 극단값을 거르다가, 적자 기업(메디포스트 OPM −86.6%)에서 배열이 비어
  > `Math.min()`이 `Infinity`가 됐고 프롬프트에 "합리 범위: Infinity% ~ -Infinity%"가 나갔다.
...
### 저장한 값을 다시 쓰려면 "정본"이 있어야 한다

분석은 종목당 여러 번 쌓이고 결과가 크게 엇갈린다. 한화시스템은 같은 날 저장된
5건의 목표가가 **1,590원 ~ 57,900원(36배)**으로 벌어져 있었다(현재가 68,200원).
그대로 갖다 쓰면 어느 것을 믿을지 알 수 없다.

- 저장 시점에 검산해 `analysis_valuations.audit_ok`·`audit_issues`에 남긴다
  (`valuation-store.ts`). 과거 9건에 소급 적용하니 **8건이 불합격**이었다.
- 조회는 뷰로 한다 — `stock_valuation_current`, `stock_segment_forecast_current`.
  규칙은 **검산을 통과한 가장 최근 분석**이며, 목표가가 현재가의 10% 미만이거나
  20배 초과인 것은 단위 오류로 보고 뺀다.
- 되읽기는 `inputs.ts`의 `getPriorValuation()`·`getPriorSegments()`.
  프롬프트에는 "이번 Base가 30% 이상 다르면 사유를 밝히라"로 들어간다 —
  **지난 결론에 묶으려는 것이 아니라**, 같은 회사가 볼 때마다 다른 가격이 되는 것을 막는다.

> **부문 이름은 `normalizeSegmentName()`을 거칠 것.** AI가 같은 사업부를 분석마다
> 다르게 적는다 — "ICT 서비스"(4건)와 "ICT 서비스 사업부"(1건)가 다른 부문으로 쌓여
> 연도·부문별 정본이 둘로 갈렸다. 뜻이 같은 꼬리말만 떼어낸다.
